...
type MockHooks = {
  ...
  onHealth?: (id: string, requestCount: number) => Record<string, unknown> | null
...
async function mockInstancesApi(page: Page, instances: MockInstance[], hooks: MockHooks = {}) {
  ...
  await page.route('**/api/org/settings', async route => {
    const payload = hooks.onOrganizationSettings?.() ?? { campaign_draft_only: false }
    await route.fulfill({
      status: 200,
  ...
  await page.route('**/api/instances**', async route => {
    ...
    if (method === 'GET' && /\/api\/instances\/[^/]+\/health$/.test(pathname)) {
      ...
      const healthPayload = hooks.onHealth?.(instanceID, requestCount) ?? HEALTH_PAYLOAD
...
async function emitWS(page: Page, eventType: string, payload: Record<string, unknown>) {
  await page.evaluate(
    ({ type, body }) => {
...
test.describe('WhatsApp Instances', () => {
  ...
  test('should send delete_chats=true when delete related chats is checked', async ({ page }) => {
    await loginAsSuperAdmin(page)
  ...
  test('should initiate connect flow and request QR regeneration', async ({ page }) => {
    ...
    await expect(page.locator('[data-sonner-toast]').filter({ hasText: /Connection initiated\. Waiting for QR code\.\.\./i })).toBeVisible()
  ...
  test('should update status on banned and logged_out websocket events', async ({ page }) => {
    await loginAsSuperAdmin(page)
  ...
  test('should show connect watchdog timeout when QR is not received', async ({ page }) => {
    await loginAsSuperAdmin(page)
    ...
    await expect(
      page.locator('[data-sonner-toast]').filter({
        hasText: 'No QR code or connection confirmation was received. Please regenerate QR code.',
      }),
  ...
  test('should save auto campaign settings payload', async ({ page }) => {
    ...
    expect(receivedUpdatePayload?.settings?.auto_campaign?.name_prefix).toBe('promo-')
    expect(receivedUpdatePayload?.settings?.auto_campaign?.interval_days).toBe(9)
    expect(receivedUpdatePayload?.settings?.auto_campaign?.min_delay_minutes).toBe(1)
    expect(receivedUpdatePayload?.settings?.auto_campaign?.max_delay_minutes).toBe(3)
    expect(receivedUpdatePayload?.settings?.auto_campaign?.target_status).toBe('run')
  ...
  test('should force target_status=draft when campaign_draft_only is enabled', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-sonner-toast]').filter({
        hasText: 'Campaign draft-only policy is active. Target status was set to draft.',
      }),
    ).toBeVisible()
    expect(receivedUpdatePayload).not.toBeNull()
    expect(receivedUpdatePayload?.settings?.auto_campaign?.target_status).toBe('draft')
  })

  test('should refresh queue depth after websocket health-triggering events', async ({ page }) => {
    await loginAsSuperAdmin(page)
