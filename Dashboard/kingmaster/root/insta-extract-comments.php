...
$page_title = "استخراج التعليقات | Kingmaster";
    ...
    <div class="modal-content">
        ...
        <form id="campaignForm">
            ...
            <div class="form-group">
               ...
               <div class="form-group">
                <label class="form-label">
                    <i class="fas fa-tag"></i> معرف البوست
                </label>
                <input type="text" class="form-input" id="id-cm-ig" placeholder="أدخل معرف البوست" required>
            </div>
      ...
      <table style="width:100%; border-collapse:collapse;">
        <thead>
          <tr style="background:#0f172a;">
            ...
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:right;">الكومنت</th>
...
document.addEventListener('DOMContentLoaded', function() {
    loadAccounts();
    loadIntervals();
    loadCampaigns('Extract Comments IG');
    loadContent();
});
...
function saveCampaign(id) {
     ...
     window.location.href = "api/save_data.php?camp_id=" + id+"&tool=Extract Comments IG";
}
...
function loadContacts() {
  ...
  fetch('api/ig_basic_info.php?' + params.toString())
    ...
    .catch(e=>{
      ...
      tb.innerHTML = `<tr><td colspan="5" style="padding:20px; text-align:center; color:#ef4444;">خطأ: ${e.message}</td></tr>`;
    });
}
...
function renderContactsTable() {
  ...
  if (!contactsState.data.length) {
    // 👉 UPDATED: colspan is now 5
    tb.innerHTML = '<tr><td colspan="5" style="padding:20px; text-align:center; color:var(--text-secondary);">لا توجد بيانات</td></tr>';
    return;
  }
  tb.innerHTML = contactsState.data.map((row, idx) => {
    ...
    // I added direction:ltr to the date so the numbers and dashes stay formatted correctly
    return `
      <tr>
        <td style="padding:8px; text-align:center;"><input type="checkbox" data-id="${id}" data-phone="${escapeHtml(row.phone||'')}" data-name="${escapeHtml((row.name||row.comment||'').toString())}" onchange="toggleSelectContact(this)" ${checked}></td>
        <td style="padding:8px;">${escapeHtml(row.name || '')}</td>
        <td style="padding:8px; direction:ltr;">${escapeHtml(row.phone || '')}</td>
        <td style="padding:8px;">${escapeHtml(row.comment || '')}</td>
        <td style="padding:8px; direction:ltr;">${escapeHtml(row.comment_date || '-')}</td> 
      </tr>`;
  }).join('');
