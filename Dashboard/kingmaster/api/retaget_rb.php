...
try {
    ...
    if ($q !== '') {
        ...
        $where .= ' AND (ty LIKE :q1 OR phone LIKE :q2)';
        $params[':q1'] = '%' . $q . '%';
        $params[':q2'] = '%' . $q . '%';
    }
    ...
    // Count total
    $stmt = $db->prepare("SELECT COUNT(*) AS total FROM retarget_rep $where");
    $stmt->execute($params);
    $total = (int)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);

    // Fetch page
    $sql = "SELECT phone, st FROM retarget_rep $where ORDER BY id DESC LIMIT :limit OFFSET :offset";
    $stmt = $db->prepare($sql);
