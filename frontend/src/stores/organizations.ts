...
export const useOrganizationsStore = defineStore('organizations', () => {
  ...
  async function fetchOrganizations(): Promise<void> {
    ...
    try {
      ...
      // 401 is handled globally by the axios interceptor (token refresh / login
      // redirect). 403 means the requester isn't a super admin / lacks
      // organizations:read — this can happen transiently when the localStorage
      // user snapshot is stale relative to the server (the OrganizationSwitcher
      // reads is_super_admin from localStorage and fires this call before
      // refreshUserData() resolves). Treat 401/403 as "no orgs to show here"
      // silently rather than surfacing a misleading error banner.
