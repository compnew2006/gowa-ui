...
import { Plus, MessageSquareText, Pencil, Trash2, Copy, Image as ImageIcon, Video as VideoIcon, X } from 'lucide-vue-next'
...
function buildMultipartPayload() {
  ...
  payload.append('keep_attachment_ids', JSON.stringify(existingAttachments.value.map(attachment => attachment.id)))
...
async function saveResponse() {
  const hasAnyAttachment = existingAttachments.value.length > 0 || newAttachmentFiles.value.length > 0
  if (!formData.value.name.trim() || (!formData.value.content.trim() && !hasAnyAttachment)) {
...
function removeNewAttachment(index: number) {
  newAttachmentFiles.value = newAttachmentFiles.value.filter((_, currentIndex) => currentIndex !== index)
}
...
function handleAttachmentSelection(event: Event) {
  ...
  for (const file of files) {
    ...
    const alreadyAdded = newAttachmentFiles.value.some(existing => existing.name === file.name && existing.size === file.size)
    if (!alreadyAdded) {
              ...
              >
                ...
                <template #cell-content="{ item: response }">
                  <div class="max-w-[300px]">
                    <p class="text-sm text-muted-foreground truncate">{{ response.content }}</p>
                  </div>
                </template>
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <Button type="button" variant="outline" size="sm" @click="openAttachmentPicker">
          ...
          />
          <p class="text-xs text-muted-foreground">{{ $t('cannedResponses.attachmentsHint') }}</p>
          <div v-if="existingAttachments.length > 0 || newAttachmentFiles.length > 0" class="space-y-2 rounded-md border p-2">
            <div
              v-for="attachment in existingAttachments"
              :key="attachment.id"
              class="flex items-center gap-2 rounded-md bg-muted/40 px-2 py-1.5"
            >
              <component :is="getAttachmentFileTypeIcon(attachment.mime_type)" class="h-4 w-4 text-muted-foreground" />
              ...
              <Button type="button" variant="ghost" size="icon" class="h-7 w-7" @click="removeExistingAttachment(attachment.id)">
                <X class="h-4 w-4" />
              </Button>
            </div>
            <div
              ...
              class="flex items-center gap-2 rounded-md bg-muted/40 px-2 py-1.5"
            >
              <component :is="getAttachmentFileTypeIcon(file.type)" class="h-4 w-4 text-muted-foreground" />
              <div class="min-w-0 flex-1">
                <p class="truncate text-sm">{{ file.name }}</p>
                <p class="text-xs text-muted-foreground">{{ formatFileSize(file.size) }}</p>
              </div>
              <Button type="button" variant="ghost" size="icon" class="h-7 w-7" @click="removeNewAttachment(index)">
                <X class="h-4 w-4" />
              </Button>
            </div>
          </div>
