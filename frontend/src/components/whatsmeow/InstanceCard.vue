...
import PerInstanceUploadsCleanup from "@/components/settings/PerInstanceUploadsCleanup.vue";
import {
  Loader2,
  Power,
  Trash2,
...
<template>
  ...
  >
    <!-- Section 1: Header / Instance Status -->
    <CardHeader class="space-y-3 border-b border-border/50 pb-4">
      <div class="flex items-start justify-between gap-3">
        <div class="flex flex-wrap items-center gap-1.5">
          <Badge variant="outline" :class="statusBadgeClass" class="text-[11px]">
            {{ $t(`instances.status.${instance.status}`) }}
          </Badge>
          <Badge
            v-if="instance.is_default"
            variant="outline"
            class="border-primary/20 bg-primary/10 text-[11px] text-primary"
          >
      ...
      <div>
        <CardTitle class="text-[15px] font-semibold leading-tight text-foreground">
          {{ instance.name }}
        </CardTitle>
      ...
      >
        <Smartphone class="h-3 w-3 shrink-0" />
        <span class="min-w-0 truncate">{{ instance.jid }}</span>
      </div>

      <div
        v-if="sendBlockedNotice"
        class="rounded-lg border border-destructive/20 bg-destructive/5 px-2.5 py-1.5 text-[11px] text-destructive"
      >
        {{ sendBlockedNotice }}
      </div>
    </CardHeader>
    ...
    <CardContent class="flex flex-1 flex-col gap-5 pt-5">
      <!-- Section 2: Stats Cards (only meaningful when connected) -->
      <div
        v-if="instance.health && isConnected"
        class="grid grid-cols-2 gap-2.5"
        data-testid="instance-health-stats"
      >
        <div class="stat-card rounded-lg border border-border/50 bg-muted/30 px-3 py-2.5">
      ...
      <!-- Section 3: Settings Grid (2-column via container query) -->
      <div class="instance-card-settings-grid grid grid-cols-1 gap-3">
        <!-- Auto-sync -->
        <div class="setting-card rounded-xl border border-border/50 bg-background/50 p-3">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
      ...
      <!-- Section 4: Retention / Uploads Cleanup -->
      <PerInstanceUploadsCleanup :instance-id="instance.id" />

      <!-- Section 5: Chat Source Tag -->
      <InstanceTagSettings
        :instance="instance"
    ...
    <CardFooter class="border-t border-border/50 bg-muted/10 pt-4">
      <Button
