...
const availableModels = computed(() => {
  const provider = aiProviders.find(
    (p) => p.value === aiSettings.value.ai_provider,
  );
...
const unselectedUsers = computed(() => {
  return availableUsers.value.filter(
    (u) => !slaSettings.value.sla_escalation_notify_ids.includes(u.id),
  );
});
  ...
  try {
    ...
    if (chatbotData.settings) {
      ...
      const mergedHours = defaultBusinessHours.map((defaultDay) => {
        const loaded = loadedHours.find(
          (h: BusinessHour) => h.day === defaultDay.day,
        );
      ...
      chatbotSettings.value = {
        ...
        session_timeout_minutes:
          chatbotData.settings.session_timeout_minutes || 30,
        business_hours_enabled:
          chatbotData.settings.business_hours_enabled || false,
        business_hours: mergedHours,
        out_of_hours_message: chatbotData.settings.out_of_hours_message || "",
        allow_automated_outside_hours:
          chatbotData.settings.allow_automated_outside_hours !== false,
        allow_agent_queue_pickup:
          chatbotData.settings.allow_agent_queue_pickup !== false,
        assign_to_same_agent:
          chatbotData.settings.assign_to_same_agent !== false,
        agent_current_conversation_only:
          chatbotData.settings.agent_current_conversation_only === true,
      };
      ...
      const clientReminderEnabledValue =
        chatbotData.settings.client_reminder_enabled === true;
      ...
      slaSettings.value = {
        ...
        sla_resolution_minutes:
          chatbotData.settings.sla_resolution_minutes || 60,
        sla_escalation_minutes:
          chatbotData.settings.sla_escalation_minutes || 30,
        sla_auto_close_hours: chatbotData.settings.sla_auto_close_hours || 24,
        sla_auto_close_message:
          chatbotData.settings.sla_auto_close_message || "",
        sla_warning_message: chatbotData.settings.sla_warning_message || "",
        sla_escalation_notify_ids:
          chatbotData.settings.sla_escalation_notify_ids || [],
        client_reminder_enabled: clientReminderEnabledValue,
        client_reminder_minutes:
          chatbotData.settings.client_reminder_minutes || 30,
        client_reminder_message:
          chatbotData.settings.client_reminder_message || "",
        client_auto_close_minutes:
          chatbotData.settings.client_auto_close_minutes || 60,
        client_auto_close_message:
          chatbotData.settings.client_auto_close_message || "",
      };
    }
  } catch (error) {
...
async function saveMessagesSettings() {
  const invalidGreetingBtn = chatbotSettings.value.greeting_buttons.find(
    (btn) => !btn.title.trim(),
  );
  ...
  const invalidFallbackBtn = chatbotSettings.value.fallback_buttons.find(
    (btn) => !btn.title.trim(),
  );
  ...
  try {
    await chatbotService.updateSettings({
      ...
      greeting_buttons: chatbotSettings.value.greeting_buttons.filter((btn) =>
        btn.title.trim(),
      ),
      fallback_message: chatbotSettings.value.fallback_message,
      fallback_buttons: chatbotSettings.value.fallback_buttons.filter((btn) =>
        btn.title.trim(),
      ),
    ...
    toast.error(
      t("common.failedSave", { resource: t("resources.chatbotSettings") }),
    );
  } finally {
...
async function saveAgentSettings() {
  ...
  try {
    await chatbotService.updateSettings({
      ...
      agent_current_conversation_only:
        chatbotSettings.value.agent_current_conversation_only,
    });
...
async function saveBusinessHoursSettings() {
  ...
  try {
    await chatbotService.updateSettings({
      ...
      allow_automated_outside_hours:
        chatbotSettings.value.allow_automated_outside_hours,
    });
              ...
              <CardContent class="space-y-4">
                <div class="space-y-2">
                  ...
                  <Textarea
                    ...
                    :placeholder="
                      $t('chatbotSettings.greetingPlaceholder') + '...'
                    "
                    :rows="2"
                  />
                  <div class="mt-2">
                    <div class="flex items-center justify-between mb-2">
                      <Label class="text-sm text-muted-foreground">{{
                        $t("chatbotSettings.quickReplyButtons")
                      }}</Label>
                      <Button
                        variant="outline"
                        size="sm"
                        ...
                        :disabled="
                          chatbotSettings.greeting_buttons.length >= 10
                        "
                      >
                    ...
                    >
                      <div
                        v-for="(
                          button, index
                        ) in chatbotSettings.greeting_buttons"
                        :key="button.id"
                      ...
                      >
                        <Input
                          ...
                          :placeholder="
                            $t('chatbotSettings.buttonPlaceholder') + '...'
                          "
                          maxlength="20"
                          class="flex-1"
                        />
                ...
                <div class="space-y-2">
                  ...
                  <Textarea
                    ...
                    :placeholder="
                      $t('chatbotSettings.fallbackPlaceholder') + '...'
                    "
                    :rows="2"
                  />
                  <div class="mt-2">
                    <div class="flex items-center justify-between mb-2">
                      ...
                      <Button
                        ...
                        :disabled="
                          chatbotSettings.fallback_buttons.length >= 10
                        "
                      >
                        <Plus class="h-4 w-4 mr-1" />
                        {{ $t("chatbotSettings.addButton") }}
                    ...
                    >
                      <div
                        v-for="(
                          button, index
                        ) in chatbotSettings.fallback_buttons"
                        :key="button.id"
                        class="flex items-center gap-2"
                      >
                        <Input
                ...
                <div class="space-y-2">
                  ...
                  />
                  <p class="text-xs text-muted-foreground">
                    {{ $t("chatbotSettings.sessionTimeoutHint") }}
                  </p>
                </div>
            ...
            <Card>
              ...
              <CardContent class="space-y-4">
                <div class="flex items-center justify-between py-2">
                  <div>
                    <p class="font-medium">
                      {{ $t("chatbotSettings.allowQueuePickup") }}
                    </p>
                    <p class="text-sm text-muted-foreground">
                      {{ $t("chatbotSettings.allowQueuePickupDesc") }}
                    </p>
                  </div>
                  <Switch
                    :checked="chatbotSettings.allow_agent_queue_pickup"
                    @update:checked="
                      chatbotSettings.allow_agent_queue_pickup = $event
                    "
                  />
                </div>
                ...
                <div class="flex items-center justify-between py-2">
                  <div>
                    <p class="font-medium">
                      {{ $t("chatbotSettings.assignSameAgent") }}
                    </p>
                    <p class="text-sm text-muted-foreground">
                      {{ $t("chatbotSettings.assignSameAgentDesc") }}
                    </p>
                  </div>
                  <Switch
                    :checked="chatbotSettings.assign_to_same_agent"
                    @update:checked="
                      chatbotSettings.assign_to_same_agent = $event
                    "
                  />
                </div>
                ...
                <div class="flex items-center justify-between py-2">
                  <div>
                    <p class="font-medium">
                      {{ $t("chatbotSettings.currentConversationOnly") }}
                    </p>
                    <p class="text-sm text-muted-foreground">
                      {{ $t("chatbotSettings.currentConversationOnlyDesc") }}
                    </p>
                  </div>
                  <Switch
                    :checked="chatbotSettings.agent_current_conversation_only"
                    @update:checked="
                      chatbotSettings.agent_current_conversation_only = $event
                    "
                  />
                </div>
            ...
            <Card>
              ...
              <CardContent class="space-y-4">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="font-medium">
                      {{ $t("chatbotSettings.enableBusinessHours") }}
                    </p>
                    <p class="text-sm text-muted-foreground">
                      {{ $t("chatbotSettings.enableBusinessHoursDesc") }}
                    </p>
                  </div>
                  <Switch
                    :checked="chatbotSettings.business_hours_enabled"
                    @update:checked="
                      chatbotSettings.business_hours_enabled = $event
                    "
                  />
                </div>
                ...
                >
                  ...
                  <div class="border rounded-lg p-4 space-y-3">
                    ...
                    >
                      ...
                      <span
                        ...
                        :class="{ 'text-muted-foreground': !hour.enabled }"
                      >
                        {{ daysOfWeek[hour.day] }}
                      </span>
                      <div
                        class="flex items-center gap-2"
                        :class="{ 'opacity-50': !hour.enabled }"
                      >
                        <Input
                  ...
                  <div class="space-y-2">
                    ...
                    <Textarea
                      ...
                      :placeholder="
                        $t('chatbotSettings.outOfHoursPlaceholder') + '...'
                      "
                      :rows="2"
                    />
                  </div>
                  ...
                  <div class="flex items-center justify-between py-2">
                    <div>
                      <p class="font-medium">
                        {{ $t("chatbotSettings.allowAutomatedOutsideHours") }}
                      </p>
                      <p class="text-sm text-muted-foreground">
                        {{
                          $t("chatbotSettings.allowAutomatedOutsideHoursDesc")
                        }}
                      </p>
                    </div>
                    <Switch
                      ...
                      @update:checked="
                        chatbotSettings.allow_automated_outside_hours = $event
                      "
                    />
                  </div>
                </div>
            ...
            <Card>
              ...
              <CardContent class="space-y-4">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="font-medium">
                      {{ $t("chatbotSettings.enableSlaTracking") }}
                    </p>
                    <p class="text-sm text-muted-foreground">
                      {{ $t("chatbotSettings.enableSlaTrackingDesc") }}
                    </p>
                  </div>
                  <Switch
                ...
                <div v-if="isSLAEnabled" class="space-y-4 pt-2">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                      ...
                      />
                      <p class="text-xs text-muted-foreground">
                        {{ $t("chatbotSettings.escalationTimeHint") }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                      ...
                      />
                      <p class="text-xs text-muted-foreground">
                        {{ $t("chatbotSettings.autoCloseHoursHint") }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div class="space-y-2">
                    ...
                    <Textarea
                      ...
                      :placeholder="
                        $t('chatbotSettings.autoClosePlaceholder') + '...'
                      "
                      :rows="2"
                    />
                  </div>
                  ...
                  <div class="space-y-2">
                    ...
                    <Textarea
                      ...
                      :placeholder="
                        $t('chatbotSettings.customerWarningPlaceholder') + '...'
                      "
                      :rows="2"
                    />
                  </div>
                  ...
                  <div class="space-y-3">
                    <div class="flex items-center justify-between">
                      <div>
                        <Label>{{
                          $t("chatbotSettings.escalationNotifyContacts")
                        }}</Label>
                        <p class="text-xs text-muted-foreground">
                          {{ $t("chatbotSettings.escalationNotifyHint") }}
                        </p>
                      </div>
                      <Popover v-model:open="escalationComboboxOpen">
                        ...
                        <PopoverContent class="w-[250px] p-0" align="end">
                          <Command>
                            <CommandInput
                              :placeholder="
                                $t('chatbotSettings.searchUsers') + '...'
                              "
                            />
                            <CommandList>
                              <CommandEmpty>{{
                                $t("chatbotSettings.noUsersFound")
                              }}</CommandEmpty>
                              <CommandGroup>
                                <CommandItem
                    ...
                    <div
                      v-if="selectedEscalationUsers.length > 0"
                    ...
                    <p v-else class="text-sm text-muted-foreground italic">
                      {{ $t("chatbotSettings.noUsersSelected") }}
                    </p>
                  </div>
                </div>
                ...
                <div class="space-y-4">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="font-medium">
                        {{ $t("chatbotSettings.clientInactivityReminders") }}
                      </p>
                      <p class="text-sm text-muted-foreground">
                        {{
                          $t("chatbotSettings.clientInactivityRemindersDesc")
                        }}
                      </p>
                    </div>
                    <Switch
                      ...
                      @update:checked="
                        (val: boolean) => (isClientReminderEnabled = val)
                      "
                    />
                  </div>
                  ...
                  <div v-if="isClientReminderEnabled" class="space-y-4 pt-2">
                    <div class="grid grid-cols-2 gap-4">
                      ...
                      <div class="space-y-2">
                        ...
                        <Input
                          v-model.number="slaSettings.client_auto_close_minutes"
                          type="number"
                          min="1"
                          max="1440"
                        />
                        <p class="text-xs text-muted-foreground">
                          {{ $t("chatbotSettings.autoCloseAfterHint") }}
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="space-y-2">
                      ...
                      <Textarea
                        ...
                        :placeholder="
                          $t('chatbotSettings.reminderPlaceholder') + '...'
                        "
                        :rows="2"
                      />
                    </div>
                    ...
                    <div class="space-y-2">
                      ...
                      <Textarea
                        ...
                        :placeholder="
                          $t('chatbotSettings.clientAutoClosePlaceholder') +
                          '...'
                        "
                        :rows="2"
                      />
            ...
            <Card>
              ...
              <CardContent class="space-y-4">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="font-medium">
                      {{ $t("chatbotSettings.enableAiResponses") }}
                    </p>
                    <p class="text-sm text-muted-foreground">
                      {{ $t("chatbotSettings.enableAiResponsesDesc") }}
                    </p>
                  </div>
                  <Switch
                    :checked="isAIEnabled"
                ...
                <div v-if="isAIEnabled" class="space-y-4 pt-2">
                    ...
                    <div class="space-y-2">
                      ...
                      <Select v-model="aiSettings.ai_provider">
                        <SelectTrigger>
                          <SelectValue
                            :placeholder="
                              $t('chatbotSettings.selectProvider') + '...'
                            "
                          />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <div class="space-y-2">
                    ...
                    <Input
                      ...
                      type="password"
                      :placeholder="
                        $t('chatbotSettings.apiKeyPlaceholder') + '...'
                      "
                    />
                    <p class="text-xs text-muted-foreground">
                      {{ $t("chatbotSettings.apiKeyHint") }}
                    </p>
                  </div>
                  ...
                  <div class="space-y-2">
                    ...
                    <Textarea
                      ...
                      :placeholder="
                        $t('chatbotSettings.systemPromptPlaceholder') + '...'
                      "
                      :rows="3"
                    />
                  </div>
