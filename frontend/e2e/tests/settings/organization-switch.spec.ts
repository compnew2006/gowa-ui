...
test.describe("Organization Switching (Super Admin)", () => {
    ...
    page
      ...
      .or(
        page
          .locator("aside")
          .locator("button")
          .filter({ hasText: /organization|org/i }),
      )
      .or(page.locator("aside select"));
  ...
  test("hover-expanded sidebar keeps org menu interactive for switching", async ({
    page,
  }) => {
    await page.goto("/login");
    await page.locator('input[type="email"]').fill(FALLBACK_ADMIN_EMAIL);
    ...
    const content = page.getByTestId("org-switcher-content");
    await expect(content).toBeVisible();

    const options = page.locator('[data-testid^="org-switcher-item-"]');
    const optionCount = await options.count();
    expect(optionCount).toBeGreaterThan(1);

    const currentOrgId = await page.evaluate(() => {
      const rawUser = localStorage.getItem("user");
      return rawUser ? JSON.parse(rawUser).organization_id : null;
    });

    let targetOrgId: string | null = null;
    let targetIndex = -1;
    for (let i = 0; i < optionCount; i += 1) {
      const optionOrgId = await options.nth(i).getAttribute("data-org-id");
      if (optionOrgId && optionOrgId !== currentOrgId) {
        targetOrgId = optionOrgId;
        targetIndex = i;
        break;
      }
    }
    ...
    await expect
      .poll(async () =>
        page.evaluate(() => localStorage.getItem("selected_organization_id")),
      )
      .toBe(targetOrgId);
  });

  test("API respects X-Organization-ID header for super admin", async ({
    request,
  }) => {
  ...
  test("API ignores X-Organization-ID header for regular user", async ({
    request,
  }) => {
    const api = new ApiHelper(request);
    try {
...
test.describe("Create Organization via Sidebar", () => {
  ...
  async function getOrgPlusButton(page: any) {
    ...
    return orgLabel
      .locator("..")
      .locator("button")
      .filter({ has: page.locator(".lucide-plus-icon") });
  }

  test("should show plus button in org switcher for super admin", async ({
    page,
  }) => {
  ...
  test("should open create organization dialog on plus click", async ({
    page,
  }) => {
    const loggedIn = await loginAsSuperAdmin(page);
    if (!loggedIn) {
  ...
  test("should create a new organization via plus button", async ({
    page,
    request,
  }) => {
    const loggedIn = await loginAsSuperAdmin(page);
...
test.describe("Organization Data Isolation", () => {
  test("users from one org are not visible in another org", async ({
    request,
  }) => {
    ...
    try {
      ...
      const org1Invite = await superAdminApi.createRegisterInvite(org1Id);
      const org2Invite = await superAdminApi.createRegisterInvite(org2Id);
      ...
      await api1.register({
        ...
        invitation_token: org1Invite.token,
      });
      ...
      await api2.register({
        ...
        invitation_token: org2Invite.token,
      });
    } catch (error) {
  ...
  test("regular user cannot access other organization data via API", async ({
    request,
  }) => {
    ...
    try {
      ...
      const invite = await superAdminApi.createRegisterInvite(myOrgId);
      ...
      const responseWithHeader = await userApi.get(
