...
Gowa-UI is multi-tenant — each organization has its own isolated data (contacts, conversations, templates, etc.). Users can belong to multiple organizations with different roles in each.
...
Create a new organization. The creator is automatically added as an admin. System roles (Admin, Manager, Agent) are seeded automatically.
...
- [Authentication](/Gowa-UI/api-reference/authentication) - Organization switching via `POST /api/auth/switch-org`
- [Users](/Gowa-UI/api-reference/users) - User management within an organization
- [Roles & Permissions](/Gowa-UI/features/roles-permissions) - Permission system details
