...
export class ApiHelper {
  ...
  async createRegisterInvite(
    orgId?: string,
    expiresInHours?: number,
  ): Promise<{ token: string; organization_id: string; expires_at: string }> {
    ...
    const response = await this.request.post(
      `${BASE_URL}/api/auth/register/invite`,
      {
        headers,
        data,
      },
    ...
    if (!response.ok()) {
      throw new Error(
        `Failed to create register invite: ${await response.text()}`,
      );
    }
  ...
  async createOrganization(name: string): Promise<Organization> {
    ...
    if (!response.ok()) {
      throw new Error(
        `Failed to create organization: ${await response.text()}`,
      );
    }
    const result = await response.json();
    return result.data;
  ...
  > {
    ...
    if (!response.ok()) {
      throw new Error(
        `Failed to get my organizations: ${await response.text()}`,
      );
    }
  ...
  async getOrgMembers(orgId?: string): Promise<any[]> {
    ...
    const response = await this.request.get(
      `${BASE_URL}/api/organizations/members`,
      {
        headers: hdrs,
      },
    );
  ...
  async removeOrgMember(userId: string, orgId?: string): Promise<void> {
    ...
    const response = await this.request.delete(
      `${BASE_URL}/api/organizations/members/${userId}`,
      {
        headers: hdrs,
      },
    );
  ...
  async listNotes(contactId: string): Promise<any[]> {
    const response = await this.request.get(
      `${BASE_URL}/api/contacts/${contactId}/notes`,
    );
    if (!response.ok()) {
  ...
  async updateNote(
    ...
    const response = await this.request.put(
      `${BASE_URL}/api/contacts/${contactId}/notes/${noteId}`,
      {
        headers: this.csrfHeaders,
        data: { content },
      },
  ...
  async getWhatsAppAccounts(): Promise<any[]> {
    ...
    if (!response.ok()) {
      throw new Error(
        `Failed to get WhatsApp accounts: ${await response.text()}`,
      );
    }
    const data = await response.json();
  ...
  async createWhatsAppAccount(data: {
    ...
    if (!response.ok()) {
      throw new Error(
        `Failed to create WhatsApp account: ${await response.text()}`,
      );
    }
    const result = await response.json();
    return result.data;
