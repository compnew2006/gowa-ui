...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login and check for a login form (email and password fields) to attempt signing in.
        await page.goto("http://localhost:8080/login", wait_until="commit", timeout=10000)
        
        # -> Fill the email field, fill the password field, then click the Sign in button to attempt login.
