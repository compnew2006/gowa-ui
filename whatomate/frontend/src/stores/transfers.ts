...
export function getSLAStatus(transfer: AgentTransfer): SLAStatus {
  ...
  if (transfer.sla_response_deadline && !transfer.picked_up_at) {
    ...
    const totalTime =
      deadline.getTime() - new Date(transfer.transferred_at).getTime();
...
export const useTransfersStore = defineStore("transfers", () => {
  ...
  const queueCount = computed(() => {
    const teamTotal = Object.values(teamQueueCounts.value).reduce(
      (sum, count) => sum + count,
      0,
    );
  ...
  const myTransfers = computed(() => {
    ...
    return transfers.value.filter(
      (t) => t.status === "active" && t.agent_id === userId,
    );
  });

  const unassignedCount = computed(
    () =>
      transfers.value.filter((t) => t.status === "active" && !t.agent_id)
        .length,
  );

  // Get active transfer for a specific contact
  function getActiveTransferForContact(
    contactId: string,
  ): AgentTransfer | undefined {
    return transfers.value.find(
      (t) => t.contact_id === contactId && t.status === "active",
    );
  }
  ...
  function resetTransferHistoryState() {
  ...
  async function fetchHistory(params?: {
    ...
    try {
      ...
      historyTotalCount.value =
        data.total_count ?? historyTransfers.value.length;
  ...
  function addTransfer(transfer: AgentTransfer) {
    ...
    if (!exists) {
      ...
      if (!transfer.agent_id) {
        if (transfer.team_id) {
          teamQueueCounts.value[transfer.team_id] =
            (teamQueueCounts.value[transfer.team_id] || 0) + 1;
        } else {
  ...
  function updateTransfer(
    ...
    if (index !== -1) {
      ...
      if (updates.agent_id !== undefined) {
        if (!oldTransfer.agent_id && updates.agent_id) {
          ...
          if (oldTransfer.team_id) {
            teamQueueCounts.value[oldTransfer.team_id] = Math.max(
              0,
              (teamQueueCounts.value[oldTransfer.team_id] || 0) - 1,
            );
          } else {
          ...
          if (teamId) {
            teamQueueCounts.value[teamId] =
              (teamQueueCounts.value[teamId] || 0) + 1;
          } else {
            generalQueueCount.value++;
          }
        }
      ...
      if (
        ...
        oldTransfer.status === "active" &&
        !oldTransfer.agent_id
      ) {
        if (oldTransfer.team_id) {
          teamQueueCounts.value[oldTransfer.team_id] = Math.max(
            0,
  ...
  function removeTransfer(id: string) {
    ...
    if (index !== -1) {
      ...
      if (transfer.status === "active" && !transfer.agent_id) {
        if (transfer.team_id) {
          teamQueueCounts.value[transfer.team_id] = Math.max(
            0,
            (teamQueueCounts.value[transfer.team_id] || 0) - 1,
          );
        } else {
