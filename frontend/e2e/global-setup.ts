...
const DB_URL = process.env.TEST_DATABASE_URL || 'postgres://gowa-ui:gowa-ui@127.0.0.1:5432/gowa-ui'
...
async function globalSetup() {
  ...
  try {
    ...
    if (loginResponse.ok()) {
      ...
      console.log(`  ℹ️  Make sure migrations have run (./gowa-ui server -migrate)`)
    }
  } catch (error) {
