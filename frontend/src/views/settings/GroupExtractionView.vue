...
const results = ref<GroupExtractionResult[]>([]);
...
const resultColumns: Column<GroupExtractionResult>[] = [
  { key: "group_name", label: t("extraction.groupName") },
  { key: "group_jid", label: "JID" },
  { key: "participant_count", label: t("extraction.members") },
...
const fetchCampaigns = async () => {
  ...
  try {
    ...
    const response = await groupExtractionService.list(params);
...
const handleCreate = async () => {
  if (!formName.value.trim() || !formInstanceId.value) {
    toast.error(t("extraction.nameAndInstanceRequired"));
    return;
  }
  try {
    await groupExtractionService.create({
...
const handleEdit = async () => {
  ...
  try {
    await groupExtractionService.update(selectedCampaign.value.id, {
      name: formName.value.trim(),
      instance_id: formInstanceId.value,
    });
...
const handleDelete = async () => {
  if (!selectedCampaign.value) return;
  try {
    await groupExtractionService.delete(selectedCampaign.value.id);
...
const handleStart = async (campaign: ExtractionCampaign) => {
  try {
    await groupExtractionService.start(campaign.id);
...
const handlePause = async (campaign: ExtractionCampaign) => {
  try {
    await groupExtractionService.pause(campaign.id);
...
const fetchResults = async () => {
  ...
  try {
    ...
    const response = await groupExtractionService.getResults(selectedCampaign.value.id, params);
...
const handleExport = async (campaign: ExtractionCampaign) => {
  try {
    const response = await groupExtractionService.exportCSV(campaign.id);
    const url = window.URL.createObjectURL(new Blob([response.data as any]));
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", `group-extraction-${campaign.name}.csv`);
...
const openStatsDialog = async (campaign: ExtractionCampaign) => {
  selectedCampaign.value = campaign;
  try {
    const response = await groupExtractionService.getStats(campaign.id);
...
<template>
  <div class="flex flex-col h-full">
    <PageHeader :title="t('extraction.groupTitle')" :description="t('extraction.groupDescription')">
      <template #actions>
    ...
    <Dialog v-model:open="showCreateDialog">
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>{{ t("extraction.createGroupDescription") }}</DialogDescription>
        </DialogHeader>
        <div class="space-y-4">
          <div>
    ...
    <Dialog v-model:open="showResultsDialog">
      <DialogContent class="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>{{ t("extraction.groupResultsDescription") }}</DialogDescription>
        </DialogHeader>
        <div class="space-y-4">
          ...
          >
            ...
            <template #cell-group_name="{ item }">
              <span class="font-medium">{{ item.group_name || "-" }}</span>
            </template>
      ...
      <DialogContent>
        ...
        <div v-if="campaignStats" class="grid grid-cols-3 gap-4">
          <div class="text-center p-4 border rounded-lg">
            <div class="text-3xl font-bold">{{ campaignStats.total_groups }}</div>
            <div class="text-sm text-muted-foreground">{{ t("extraction.totalGroups") }}</div>
          </div>
          <div class="text-center p-4 border rounded-lg">
