...
def record_comment_context(page_id, comment_id, message, from_name, sender_id, post_id, parent_id, reply_result, bot_reply_text):
...
def record_auto_public_reply_signature(page_id, parent_comment_id, reply_message):
    ...
    ensure_learning_state_loaded(page_id)
    normalized_reply = normalize_learning_text(reply_message)
    if not normalized_reply:
        return

    signature = (page_id, parent_comment_id, normalized_reply)
    if signature in AUTO_PUBLIC_REPLY_SIGNATURES:
        return

    AUTO_PUBLIC_REPLY_SIGNATURES.add(signature)
    payload = {
        "created_at": datetime_to_iso(utc_now()),
        "page_id": page_id,
    ...
    try:
        append_jsonl(get_auto_public_reply_file(page_id), payload)
    except Exception as exc:
        logger.error("Failed saving auto reply signature: %s", exc)


def is_known_auto_public_reply(page_id, parent_comment_id, reply_message):
    ensure_learning_state_loaded(page_id)
    normalized_reply = normalize_learning_text(reply_message)
    return (page_id, parent_comment_id, normalized_reply) in AUTO_PUBLIC_REPLY_SIGNATURES
            ...
            if sender_id == page_id:
                ...
                if reply:
                    ...
                    if reply_response.get('id'):
                        logger.info("AI-replied to %s: %s...", from_name, reply[:50])
                        ...
                        # Send a private Messenger reply only for allowed grounded intents.
                        if should_send_private_reply(page_config, reply_result, comment_id, sender_id, page_id):
                            ...
                            if private_reply:
                                ...
                                if private_reply_response.get("id"):
                                    ...
                                    logger.warning("Failed private reply: %s", private_reply_response)

                        # 🔔 Send notification to page owner
                        notification_system.notify_owner(page_id, from_name, message, reply)
                    else:
                        logger.warning("Failed to reply: %s", reply_response)
        except Exception as e:
...
def reply_to_facebook_comment(page_id, comment_id, message):
    try:
        ...
        logger.error("Failed to reply to %s: %s", comment_id, e)
        return {"error": str(e)}
...
def send_private_reply_to_facebook_comment(page_id, comment_id, message):
    try:
        ...
        if response.status_code == 200:
            return result
        logger.error("Facebook private reply API error for page %s: HTTP %d", page_id, response.status_code)
        return {"error": result.get("error") or response.text}
    except Exception as e:
        logger.error("Failed to send private reply to %s: %s", comment_id, e)
        return {"error": str(e)}
...
def forward_to_hermes(payload):
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning("Hermes returned: %d", response.status_code)
