...
import { rolesService, permissionsService, type Role } from "@/services/api";
...
const RESOURCE_LABEL_TRANSLATION_KEYS: Record<string, string> = {
  "settings.uploads_cleanup": "permissionsUi.resources.settingsUploadsCleanup",
};

function getPermissionGroupLabel(resource: string): string {
  const translationKey = RESOURCE_LABEL_TRANSLATION_KEYS[resource];
...
export const useRolesStore = defineStore("roles", () => {
  ...
  const permissionGroups = computed<PermissionGroup[]>(() => {
    ...
    return Object.entries(groups)
      .map(([resource, perms]) => ({
        resource,
        label: getPermissionGroupLabel(resource),
