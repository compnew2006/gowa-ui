...
import { contactsService, accountsService, instancesService, type Tag } from '@/services/api'
...
const defaultFormData: ContactFormData = { phone_number: '', profile_name: '', whatsapp_account: '', instance_id: '', tags: [] }
...
async function fetchInstances() {
  try {
    ...
    const data = response.data as any
    const responseData = data.data || data
    availableInstances.value = responseData || []
  } catch (error) {
    // Silently fail - instances are optional in legacy account mode
  }
}
  ...
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="sm:max-w-md">
      ...
      <div class="space-y-4 py-4">
        ...
        <div v-if="availableInstances.length > 0" class="space-y-2">
          ...
          <Select v-model="formData.instance_id">
            ...
            <SelectContent>
              <SelectItem v-for="instance in availableInstances" :key="instance.id" :value="instance.id">
                {{ instance.name }}
              </SelectItem>
            </SelectContent>
          </Select>
