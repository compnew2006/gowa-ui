...
func TestPluginRoutes(t *testing.T) {
	...
	require.Equal(t, "facebook-page-search", plugin.Name())
	require.Equal(t, core.ModuleManifest{
		Key: "facebook-page-search", DisplayName: "Facebook Page Search", Version: "1.0.0",
	...
	assertRouteRegistered(t, handler, fasthttp.MethodGet, "/api/facebook/page-search")
...
func newRouteTestApp(t *testing.T, rateLimit bool) *handlers.App {
	...
	return &handlers.App{
		Config: &config.Config{RateLimit: config.RateLimitConfig{Enabled: rateLimit}},
		DB:     db,
