...
async function uploadHeaderMedia() {
  ...
  try {
    const response = await templatesService.uploadMedia(
      formData.value.whatsapp_account,
...
function setSampleValue(component: string, paramName: string, value: string) {
  ...
  if (existingIndex >= 0) {
    ...
    formData.value.sample_values.push({
      component,
        ...
        <div class="space-y-4 py-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="space-y-2">
              ...
              <p class="text-xs text-muted-foreground">
                {{ $t("templates.templateNameLowercase") }}
              </p>
            </div>
          ...
          >
            <Label
              ...
              {{ formData.header_type.toLowerCase() }}</Label
            >
            <p class="text-xs text-muted-foreground">
              {{
                $t("templates.uploadSampleHint", {
                  type: formData.header_type.toLowerCase(),
                })
              }}
            </p>
            ...
            <!-- Show upload status -->
            <div
              v-if="headerMediaFilename && !headerMediaHandle"
            ...
            <p class="text-xs text-muted-foreground">
              <span v-if="formData.header_type === 'IMAGE'">{{
                $t("templates.imageFormats")
              }}</span>
              <span v-else-if="formData.header_type === 'VIDEO'">{{
                $t("templates.videoFormats")
              }}</span>
              <span v-else-if="formData.header_type === 'DOCUMENT'">{{
                $t("templates.documentFormats")
              }}</span>
            </p>
          </div>
            ...
            >
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium"
                  >{{ $t("templates.button") }} {{ index + 1 }}</span
                >
                <Button
                  type="button"
                  variant="ghost"
          ...
          >
            ...
            <div v-if="headerVariables.length > 0" class="space-y-2">
              ...
              >
                <span
                  class="text-sm font-mono bg-muted px-2 py-1 rounded min-w-[80px] text-center"
                ...
                <input
                  ...
                  "
                  :placeholder="
                    $t('templates.exampleFor', { name: paramName }) + '...'
                  "
          ...
          <!-- Info Box -->
          <div
            class="bg-blue-950 light:bg-blue-50 border border-blue-800 light:border-blue-200 rounded-lg p-4"
          >
            <div class="flex gap-3">
              <AlertCircle
                class="h-5 w-5 text-blue-400 light:text-blue-600 flex-shrink-0"
              />
              <div class="text-sm text-blue-200 light:text-blue-800">
                <p class="font-medium">
                  {{ $t("templates.templateSubmission") }}
                </p>
                <p class="mt-1">
        ...
        <div v-if="previewTemplate" class="py-4">
          ...
          <div class="bg-gray-800 light:bg-[#e5ddd5] rounded-lg p-4">
            ...
            >
              ...
              >
                <div
                  v-if="previewTemplate.header_type === 'TEXT'"
                  class="font-semibold"
                >
                ...
                <div
                  v-else
                  class="h-32 bg-gray-600 light:bg-gray-200 rounded flex items-center justify-center"
                >
                  <component
                    :is="getHeaderIcon(previewTemplate.header_type)"
              ...
              <div class="p-3">
                <p class="text-sm whitespace-pre-wrap">
                  <template
                    v-for="(part, idx) in parseFormatPreview(
              ...
              >
                <div
                  v-for="(btn, idx) in previewTemplate.buttons"
                  :key="idx"
                  class="border-b last:border-b-0"
                >
                  <button
                    class="w-full py-2 text-sm text-blue-500 hover:bg-gray-600 light:hover:bg-gray-50"
                  >
          ...
          <div class="mt-4 space-y-2 text-sm">
            <div class="flex justify-between">
              <span class="text-muted-foreground"
                >{{ $t("templates.status") }}:</span
              >
              <span
                :class="[
                  'px-2 py-0.5 rounded text-xs font-medium',
                  getStatusBadgeClass(previewTemplate.status),
                ]"
              >
            ...
            <div class="flex justify-between">
              <span class="text-muted-foreground"
                >{{ $t("templates.category") }}:</span
              >
              <span>{{ previewTemplate.category }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted-foreground"
                >{{ $t("templates.language") }}:</span
              >
              <span>{{ previewTemplate.language }}</span>
            </div>
            <div
              v-if="previewTemplate.meta_template_id"
              class="flex justify-between"
            >
              <span class="text-muted-foreground"
                >{{ $t("templates.metaId") }}:</span
              >
