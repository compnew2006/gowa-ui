...
try {
    ...
    // Count total
    $stmt = $db->prepare("SELECT COUNT(*) AS total FROM fb_serch $where");
    $stmt->execute($params);
    $total = (int)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);

    // Fetch page
    $sql = "SELECT name, page_id, followers_count FROM fb_serch $where ORDER BY id DESC LIMIT :limit OFFSET :offset";
    $stmt = $db->prepare($sql);
