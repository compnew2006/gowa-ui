...
// GOWA device endpoints respond via fastglue's SendEnvelope, which wraps the
// payload as { status, data: <inner> }. The service definitions in api.ts type
// only the inner payload, so unwrap the envelope here to read the real fields
// while keeping the access type-checked.
function unwrap<T>(resp: { data: T }): T {
  const envelope = resp.data as unknown as { data?: T };
  return (envelope.data ?? resp.data) as T;
}
...
const canWriteDevices = computed(() =>
  authStore.hasPermission("devices", "write"),
);
const canDeleteDevices = computed(() =>
  authStore.hasPermission("devices", "delete"),
);
...
const connectTab = ref<"qr" | "pair">("qr");
const qrLink = ref("");
...
async function refreshDevices() {
  try {
    ...
    toast.error(
      getErrorMessage(e, t("common.failedLoad", { resource: "devices" })),
    );
  }
}
...
async function submitCreate() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        e,
        t("gowaServers.deviceCreated", "Failed to create device"),
      ),
    );
  } finally {
...
async function fetchPair() {
  ...
  pairCode.value = "";
  try {
    const resp = await gowaServersService.devicePairCode(
      serverId.value,
      connectDevice.value.id,
    ...
    toast.error(
      getErrorMessage(
        e,
        t("accounts.gowaPairFailed", "Failed to get pair code"),
      ),
    );
  } finally {
...
async function openWebhook(d: GowaDevice) {
  ...
  try {
    const resp = await gowaServersService.getDeviceWebhook(
      serverId.value,
      d.id,
    );
    const data = unwrap(resp);
    if (data.webhook) {
      webhookForm.value.webhook_url =
        data.webhook.webhook_url || webhookForm.value.webhook_url;
      webhookForm.value.webhook_events =
        data.webhook.webhook_events || webhookForm.value.webhook_events;
      webhookForm.value.webhook_insecure_skip_verify =
        !!data.webhook.webhook_insecure_skip_verify;
    }
  } catch {
    /* keep defaults */
  }
...
async function saveWebhook() {
  ...
  try {
    await gowaServersService.setDeviceWebhook(
      serverId.value,
      webhookDevice.value.id,
      webhookForm.value,
    );
...
async function confirmDelete() {
  ...
  try {
    await gowaServersService.deleteDevice(
      serverId.value,
...
<template>
  <div class="flex flex-col h-full bg-[#0a0a0b] light:bg-gray-50">
        ...
        <Card>
          <CardHeader>
            <div class="flex items-center justify-between">
              <div>
                <CardTitle>{{
                  $t("gowaServers.devices", "Devices")
                }}</CardTitle>
                <CardDescription>{{
                  store.currentServer?.base_url
                }}</CardDescription>
              </div>
          ...
          <CardContent>
            ...
            >
              ...
              <p class="text-sm">
                {{ $t("gowaServers.noDevices", "No devices on this server") }}
              </p>
              <p class="text-xs mt-1">
                {{
                  $t(
                    "gowaServers.noDevicesDesc",
                    "Create a device to connect a WhatsApp number.",
                  )
                }}
              </p>
            </div>
              ...
              <Card v-for="d in devices" :key="d.id" class="border-border/60">
                <CardContent class="pt-4 space-y-3">
                  <div class="flex items-start justify-between gap-2">
                    ...
                    <Badge
                      variant="outline"
                      :class="stateClass(d.state) + ' flex-shrink-0'"
                    >
                      <CheckCircle2
                        v-if="d.state === 'logged_in' || d.is_connected"
                        class="h-3 w-3 mr-1"
                      />
                  ...
                  <div class="flex flex-wrap items-center gap-1.5">
                    ...
                    >
                      ...
                      {{ $t("gowaServers.pairCode", "Pair Code") }}
                    </Button>
                    <Button
                      v-if="canWriteDevices && d.is_connected"
                      size="sm"
    ...
    <Dialog :open="connectOpen" @update:open="(v) => !v && closeConnect()">
      <DialogContent
        class="max-w-lg"
        @escape-key-down="closeConnect"
        @pointer-down-outside="closeConnect"
      >
        <DialogHeader>
          <DialogTitle>{{
          ...
          <DialogDescription>{{
            $t(
              "accounts.gowaConnectDesc",
              "Scan the QR code or use a pair code to link your WhatsApp account.",
            )
          }}</DialogDescription>
        </DialogHeader>
        <Tabs :model-value="connectTab" @update:model-value="(v) => (connectTab = v as 'qr' | 'pair')">
          <TabsList class="grid w-full grid-cols-2">
            <TabsTrigger value="qr"
              ><QrCode class="h-4 w-4 mr-1.5" />
              {{ $t("gowaServers.qrCode", "QR Code") }}</TabsTrigger
            >
            <TabsTrigger value="pair"
              ><Link2 class="h-4 w-4 mr-1.5" />
              {{ $t("gowaServers.pairCode", "Pair Code") }}</TabsTrigger
            >
          </TabsList>
          <TabsContent value="qr" class="flex flex-col items-center gap-3 py-4">
            <div
              class="relative w-64 h-64 bg-white rounded-lg flex items-center justify-center border border-border shadow-inner"
            >
              <Loader2
                v-if="qrLoading && !qrLink"
          ...
          <TabsContent value="pair" class="space-y-4 py-4">
            <div class="space-y-2">
              <Label class="text-xs">{{
                $t("gowaServers.phoneNumber", "Phone Number")
              }}</Label>
              ...
              <p class="text-xs text-muted-foreground">
                {{ $t("gowaServers.pairCodeInstructions") }}
              </p>
            </div>
            <div
              v-if="pairCode"
              class="flex flex-col items-center gap-2 p-4 bg-muted rounded-lg"
            >
              <span class="text-xs text-muted-foreground">{{
                $t("gowaServers.yourPairCode", "Your Pair Code")
              }}</span>
              <span class="text-3xl font-bold font-mono tracking-[0.3em]">{{
                pairCode
              }}</span>
    ...
    <Dialog :open="webhookOpen" @update:open="(v) => (webhookOpen = v)">
      <DialogContent class="max-w-lg">
        ...
        <div class="space-y-4 py-2">
          <div class="space-y-2">
            <Label>{{
              $t("gowaServers.webhookUrlLabel", "Webhook URL")
            }}</Label>
            <Input
          ...
          <div class="space-y-2">
            <Label>{{
              $t("gowaServers.webhookEventsLabel", "Webhook Events")
            }}</Label>
            <Input
              v-model="webhookForm.webhook_events"
              class="font-mono text-xs"
            />
          </div>
        </div>
        <div class="flex justify-end gap-2">
          <Button variant="outline" @click="webhookOpen = false">{{
            $t("common.cancel", "Cancel")
          }}</Button>
          <Button :disabled="webhookSaving" @click="saveWebhook">{{
            $t("gowaServers.saveWebhook", "Save Webhook")
          }}</Button>
        </div>
      </DialogContent>
    </Dialog>
