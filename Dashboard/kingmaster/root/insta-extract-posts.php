...
$page_title = "استخراج البوستات | Kingmaster";
    ...
    <div class="modal-content">
        ...
        <form id="campaignForm">
            ...
            <div class="form-group">
               ...
               <div class="form-group">
                <label class="form-label">
                    <i class="fas fa-tag"></i> معرف البروفايل
                </label>
                <input type="text" class="form-input" id="id-cm-ig" placeholder="أدخل معرف البروفايل" required>
            </div>
...
<div id="contactsModal" class="modal">
  <div class="modal-content" style="max-width: 900px;">
    ...
    <div class="filters-section" style="margin-top:0; gap:10px; align-items:center;">
      <input type="text" id="contactsSearch" class="form-input" placeholder="بحث بالاسم أو المعرف" oninput="debouncedSearchContacts()" style="flex:1;">
      ...
      <table style="width:100%; border-collapse:collapse;">
       <thead>
          <tr style="background:#0f172a;">
            ...
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:center;">النوع</th>
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:right;">المحتوى (الكابشن)</th>
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:center;">إعجابات <i class="fas fa-heart" style="color:#ef4444;"></i></th>
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:center;">تعليقات <i class="fas fa-comment" style="color:#3b82f6;"></i></th>
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:right;">التاريخ</th>
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:center;">عرض</th>
          </tr>
        </thead>
        <tbody id="contactsTableBody">
          <tr><td colspan="5" style="padding:20px; text-align:center; color:var(--text-secondary);">جاري التحميل...</td></tr>
        </tbody>
      </table>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    loadAccounts();
    loadIntervals();
    loadCampaigns('Extract Posts IG');
    loadContent();
});
...
function closeModal() {
    ...
    editingCampaignId = null;
        ...
        document.getElementById('id-cm-ig').innerHTML = '';
...
document.getElementById('campaignForm').addEventListener('submit', function(e) {
    ...
    if (editingCampaignId) {
        ...
        const data = {
            name: document.getElementById('campaign-name').value,
            accounts: selectedAccounts,
            tools: "Extract Posts IG",
            paltform: "Instagram",
            id_action: document.getElementById('id-cm-ig').value,
...
function editCampaign(id) {
    ...
    document.getElementById('id-cm-ig').value = campaign.pram1;
...
function saveCampaign(id) {
     ...
     window.location.href = "api/save_data.php?camp_id=" + id+"&tool=Extract Posts IG";
}
...
function renderContactsTable() {
  ...
  if (!contactsState.data.length) {
    // تم تغيير colspan إلى 7
    tb.innerHTML = '<tr><td colspan="7" style="padding:20px; text-align:center; color:var(--text-secondary);">لا توجد بيانات</td></tr>';
    return;
  }
  
  tb.innerHTML = contactsState.data.map((row, idx) => {
    // استخدمنا shortcode كمعرف بدلاً من الهاتف
    const id = `${contactsState.page}-${idx}-${row.shortcode}`;
    const checked = contactsState.selected.has(id) ? 'checked' : '';
    
    // تحديد الأيقونة حسب نوع البوست
    let typeIcon = '<i class="fas fa-image" style="color:#a855f7;" title="صورة"></i>';
    if(row.post_type === 'Video') typeIcon = '<i class="fas fa-video" style="color:#ef4444;" title="فيديو"></i>';
    if(row.post_type === 'Album') typeIcon = '<i class="fas fa-images" style="color:#3b82f6;" title="ألبوم"></i>';
    
    // اختصار الكابشن الطويل حتى لا يفسد شكل الجدول
    let shortContent = (row.content || '').length > 60 ? (row.content.substring(0, 60) + '...') : row.content;

    return `
      <tr style="border-bottom: 1px solid var(--border-color);">
        <td style="padding:8px; text-align:center;">
          <input type="checkbox" data-id="${id}" data-phone="${escapeHtml(row.shortcode||'')}" data-name="${escapeHtml((row.content||'').toString())}" onchange="toggleSelectContact(this)" ${checked}>
        </td>
        <td style="padding:8px; text-align:center; font-size:1.2rem;">${typeIcon}</td>
        <td style="padding:8px; max-width: 250px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${escapeHtml(row.content || '')}">${escapeHtml(shortContent)}</td>
        <td style="padding:8px; text-align:center; font-weight:bold;">${row.likes_count}</td>
        <td style="padding:8px; text-align:center; font-weight:bold;">${row.comments_count}</td>
        <td style="padding:8px; direction:ltr;">${escapeHtml(row.post_date || '-')}</td>
        <td style="padding:8px; text-align:center;">
          <a href="https://instagram.com/p/${row.shortcode}" target="_blank" class="action-btn-small" style="background: linear-gradient(135deg, #10b981, #059669); text-decoration:none; display:inline-block;">
...
document.getElementById('commentsForm').addEventListener('submit', function(e) {
    ...
    console.log(data);
    fetch('api/create_campaign_insta.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(data)
    })
