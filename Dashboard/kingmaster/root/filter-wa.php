...
$page_title = "التحقق من الهواتف | Kingmaster";
...
function saveCampaign(id) {
     ...
     window.location.href = "api/save_data.php?camp_id=" + id+"&tool=filter-WA-send";
}
...
function loadContacts() {
  ...
  fetch('api/wa_filter.php?' + params.toString())
    .then(r=>r.json())
    .then(j=>{
...
function renderContactsTable() {
  ...
  tb.innerHTML = contactsState.data.map((row, idx) => {
    ...
    return `
      <tr>
        <td style="padding:8px; text-align:center;"><input type="checkbox" data-id="${id}" data-phone="${escapeHtml(row.phone||'')}" data-name="${escapeHtml((row.name||row.ty||'').toString())}" onchange="toggleSelectContact(this)" ${checked}></td>
        <td style="padding:8px;">${escapeHtml(row.name || '')}</td>
        <td style="padding:8px; direction:ltr;">${escapeHtml(row.phone || '')}</td>
        <td style="padding:8px;">${escapeHtml(row.ty || '')}</td>
      </tr>`;
  }).join('');
