...
try {
    ...
    // Count total
    $stmt = $db->prepare("SELECT COUNT(*) AS total FROM wa_contacts $where");
    $stmt->execute($params);
    $total = (int)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);

    // Fetch page
    $sql = "SELECT name, phone, pushname FROM wa_contacts $where ORDER BY id DESC LIMIT :limit OFFSET :offset";
    $stmt = $db->prepare($sql);
