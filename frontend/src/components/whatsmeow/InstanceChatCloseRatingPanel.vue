...
function handleSave() {
  emit("save", cloneInstanceChatCloseRatingSettings(localSettings.value));
           ...
           <div class="flex items-center justify-between gap-2">
            <div>
              <Label class="text-white/80 light:text-gray-800">{{
                $t("instances.chat_close_rating.overrideOrg")
              }}</Label>
              <p class="text-xs text-white/45 light:text-gray-500">
                {{ $t("instances.chat_close_rating.overrideOrgDesc") }}
              </p>
            </div>
            <Switch v-model:checked="localSettings.override_org_settings" />
          </div>
        </div>

        <template v-if="localSettings.override_org_settings">
             <div class="space-y-4">
              <div v-for="lang in ['ar', 'en', 'es']" :key="lang" class="space-y-2">
                <Label
                  >{{ $t(`settings.chatCloseRatingTemplate${lang.charAt(0).toUpperCase() + lang.slice(1)}`) }}</Label
                >
                <Textarea
