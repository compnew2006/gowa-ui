...
const canViewOrganizationAnalytics = computed(() =>
  authStore.hasPermission("analytics", "read"),
);
const canUseAgentFilter = computed(
  () =>
    canViewOrganizationAnalytics.value &&
...
const openRatingChat = async (record: AgentRatingRecord) => {
  ...
  await router.push({
    ...
    params: { contactId: record.contact_id },
  });
};

const extractFollowupComments = (
...
const formatRatingMessage = (record: AgentRatingRecord): string => {
  const base =
    typeof record.rating_message === "string"
      ? record.rating_message.trim()
      : "";
...
const formatContextMessages = (
  ...
  if (
    ...
    "content" in ratingValue
  ) {
    ratingPart = String(
      (ratingValue as { content?: unknown }).content ?? "",
    ).trim();
  }
...
const fetchAgents = async () => {
  ...
  try {
    ...
    agents.value = usersStore.users.map((u) => ({
      id: u.id,
...
const sourceChartData = computed(() => {
  ...
  const labels = Object.keys(sources).map(
  ...
  <div class="flex flex-col h-full">
    ...
    >
      <template #actions>
        ...
        <div
          v-if="canViewOrganizationAnalytics"
          class="flex items-center gap-2 mr-4"
        >
          <Select v-model="selectedInstanceId">
            <SelectTrigger
              class="w-[220px]"
              data-testid="agent-analytics-instance-filter"
            >
              <SelectValue :placeholder="$t('chat.instance')">
                <span v-if="selectedInstanceId === 'all'">{{
                  $t("common.all")
                }}</span>
                <span v-else>
                  {{
          ...
          <Select v-model="selectedRange">
            ...
            <SelectContent>
              <SelectItem value="today">{{
                $t("agentAnalytics.today")
              }}</SelectItem>
              <SelectItem value="7days">{{
                $t("agentAnalytics.last7Days")
              }}</SelectItem>
              <SelectItem value="30days">{{
                $t("agentAnalytics.last30Days")
              }}</SelectItem>
              <SelectItem value="this_month">{{
                $t("agentAnalytics.thisMonth")
              }}</SelectItem>
              <SelectItem value="custom">{{
                $t("agentAnalytics.customRange")
              }}</SelectItem>
            </SelectContent>
          </Select>
      ...
      <div class="p-6 space-y-6">
                  ...
                  {{
                    formatMinutes(
                      analytics.my_stats?.total_break_time_mins ??
                        analytics.summary?.total_break_time_mins ??
                        0,
                    )
                  }}
                </div>
                <p class="text-xs text-white/40 light:text-gray-500 mt-1">
                  {{
                    $t("agentAnalytics.breaksTaken", {
                      count:
                        analytics.my_stats?.break_count ??
                        analytics.summary?.break_count ??
                        0,
                    })
                  }}
                </p>
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{{
                $t("agentAnalytics.transfersOverTime")
              }}</CardDescription>
            </CardHeader>
            <CardContent>
              <div class="h-64">
          ...
          <Card>
            <CardHeader>
              <CardTitle>{{
                $t("agentAnalytics.conversationSources")
              }}</CardTitle>
              <CardDescription>{{
                $t("agentAnalytics.howConversationsInitiated")
              }}</CardDescription>
            </CardHeader>
            <CardContent>
              <div class="h-64">
        ...
        >
          <Card>
            <CardHeader>
              ...
              <CardDescription>{{
                $t("agentAnalytics.performanceComparison")
              }}</CardDescription>
            </CardHeader>
            <CardContent>
              <div class="h-64">
        ...
        <template v-if="canViewOrganizationAnalytics">
          <Card>
            <CardHeader>
              <CardTitle>{{
                $t("agentAnalytics.ratingsTableTitle")
              }}</CardTitle>
              <CardDescription>{{
                $t("agentAnalytics.ratingsTableSubtitle")
              }}</CardDescription>
            </CardHeader>
            <CardContent>
              <template v-if="isLoading">
                ...
                >
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>{{
                          $t("agentAnalytics.agentColumn")
                        }}</TableHead>
                        <TableHead>{{
                          $t("agentAnalytics.chatIdColumn")
                        }}</TableHead>
                        <TableHead>{{
                          $t("agentAnalytics.contactColumn")
                        }}</TableHead>
                        <TableHead>{{
                          $t("agentAnalytics.ratingColumn")
                        }}</TableHead>
                        <TableHead>{{
                          $t("agentAnalytics.ratedAtColumn")
                        }}</TableHead>
                        <TableHead>{{
                          $t("agentAnalytics.closingAgentColumn")
                        }}</TableHead>
                        <TableHead>{{
                          $t("agentAnalytics.ratingMessageColumn")
                        }}</TableHead>
                        <TableHead>{{
                          $t("agentAnalytics.contextMessagesColumn")
                        }}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      ...
                      >
                        ...
                        <TableCell>{{
                          record.contact || record.contact_phone
                        }}</TableCell>
                        <TableCell>{{ record.rating }}</TableCell>
                        <TableCell>{{
                          formatDateTime(record.rated_at)
                        }}</TableCell>
                        <TableCell>{{
                          record.closing_agent_name || "-"
                        }}</TableCell>
                        <TableCell
                          class="max-w-[220px] truncate"
                          :title="formatRatingMessage(record)"
                        >
