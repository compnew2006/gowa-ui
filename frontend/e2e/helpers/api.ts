...
export class ApiHelper {
  ...
  // Requires users:write permission in the selected org context.
  async createRegisterInvite(orgId?: string, expiresInHours?: number): Promise<{ token: string; organization_id: string; expires_at: string }> {
    const headers: Record<string, string> = { ...this.csrfHeaders }
    if (orgId) headers['X-Organization-ID'] = orgId

    const data: Record<string, number> = {}
    if (expiresInHours) data.expires_in_hours = expiresInHours

    const response = await this.request.post(`${BASE_URL}/api/auth/register/invite`, {
      headers,
      data
    })
    if (!response.ok()) {
      throw new Error(`Failed to create register invite: ${await response.text()}`)
    }
  ...
  // Activity Logs
  async createActivityLog(data: ActivityLogPayload): Promise<any> {
    const response = await this.request.post(`${BASE_URL}/api/activity-logs`, {
      headers: this.csrfHeaders,
      data
    })
    if (!response.ok()) {
  ...
  async listActivityLogs(params?: Record<string, string | number>): Promise<any> {
    const response = await this.request.get(`${BASE_URL}/api/activity-logs`, { params })
    if (!response.ok()) {
      throw new Error(`Failed to list activity logs: ${await response.text()}`)
    }
    const result = await response.json()
    return result.data
  }
