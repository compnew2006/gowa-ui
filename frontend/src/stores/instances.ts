...
export const useInstancesStore = defineStore("instances", () => {
  ...
  async function fetchInstances() {
    ...
    try {
      ...
      const message =
        err.response?.data?.message || t("instances.toast.fetchFailed");
      error.value = message;
      toast.error(message);
    } finally {
      loading.value = false;
  ...
  async function fetchAllHealth() {
    ...
    await Promise.all(
      instances.value.map((instance) => fetchInstanceHealth(instance.id)),
    );
  }
  ...
  async function fetchInstance(id: string) {
    ...
    try {
      ...
      const message =
        err.response?.data?.message || t("instances.toast.fetchOneFailed");
      error.value = message;
      return null;
    }
  }
  ...
  function startHealthPolling(
    intervalMs = 30000,
    options: { refreshInstances?: boolean } = {},
  ) {
    stopHealthPolling();
  ...
  async function createInstance(data: {
    ...
    try {
      ...
      const newInstance = (response.data.data ||
        response.data) as WhatsAppInstance;
      instances.value.push(newInstance);
      toast.success(t("instances.toast.createSuccess"));
      return newInstance;
    } catch (err: any) {
      const msg =
        err.response?.data?.message || t("instances.toast.createFailed");
      toast.error(msg);
      throw err;
    } finally {
      loading.value = false;
  ...
  async function updateInstance(id: string, data: Partial<WhatsAppInstance>) {
    ...
    try {
        ...
        instances.value[index] = {
          ...
          settings:
            data.settings !== undefined
              ? data.settings
              : updatedInstance.settings || current.settings,
      ...
      const msg =
        err.response?.data?.message || t("instances.toast.updateFailed");
      toast.error(msg);
      throw err;
    } finally {
      loading.value = false;
  ...
  async function deleteInstance(
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || t("instances.toast.deleteFailed");
      toast.error(msg);
      throw err;
    } finally {
      loading.value = false;
  ...
  async function connectInstance(id: string) {
    try {
      ...
      const msg =
        err.response?.data?.message || t("instances.toast.connectFailed");
      toast.error(msg);
      return false;
    }
  }
  ...
  async function disconnectInstance(id: string) {
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || t("instances.toast.disconnectFailed");
      toast.error(msg);
    } finally {
      loading.value = false;
    }
  ...
  async function reconnectInstance(id: string) {
    try {
      ...
      const msg =
        err.response?.data?.message || t("instances.toast.reconnectFailed");
      toast.error(msg);
      throw err;
    }
  }
  ...
  async function requestPhonePairCode(id: string, phoneNumber: string) {
    try {
      ...
      const msg =
        err.response?.data?.message || t("instances.toast.pairingCodeFailed");
      toast.error(msg);
      throw err;
    }
  }
