...
$page_title = "بحث الأماكن | Kingmaster";
...
<div class="tools-container">
    <div class="tools-header">
        <div class="tools-title"><i class="fas fa-map-location-dot fa-spin" style="--fa-animation-duration: 3s; color: #ef4444;"></i> البحث عن أماكن (Location Search)</div>
    ...
    <div class="filters-section">
        ...
        <select class="filter-select" id="status-filter" onchange="applyFilters()">
            <option value="all">جميع الحالات</option><option value="pending">قيد الانتظار</option><option value="running">قيد التشغيل</option>
            <option value="paused">متوقف مؤقتاً</option><option value="stopped">متوقف</option><option value="finished">منتهي</option>
        </select>
...
<div id="campaignModal" class="modal">
    <div class="modal-content">
        ...
        <form id="campaignForm">
            <div class="form-group"><label class="form-label"><i class="fas fa-tag"></i> اسم الحملة</label><input type="text" class="form-input" id="campaign-name" required></div>
            ...
            <div class="form-group"><label class="form-label"><i class="fas fa-map-marker-alt"></i> المدينة أو المعلم</label><input type="text" class="form-input" id="keyword" placeholder="مثال: Cairo, Dubai" required></div>
...
<div id="contactsModal" class="modal">
  <div class="modal-content" style="max-width: 900px; background: #1e293b; border-radius: 16px; border: 1px solid #334155; padding: 24px;">
    <div class="modal-header" style="border-bottom: 1px solid #334155; padding-bottom: 15px;"><div class="modal-title" style="color: #f8fafc;"><i class="fas fa-map-marker-alt" style="color: #ef4444;"></i> الأماكن المستخرجة</div><span class="close-modal" onclick="closeContactsModal()" style="cursor: pointer; color: #94a3b8;">&times;</span></div>
    <div class="filters-section" style="display: flex; gap: 15px; margin-bottom: 20px; border:none; padding:0; background:transparent;">
      <div style="flex: 1; position: relative;">
         ...
         <input type="text" id="contactsSearch" class="form-input" placeholder="بحث باسم المكان أو العنوان..." oninput="debouncedSearchContacts()" style="width: 100%; padding: 10px 40px 10px 15px; background: #0f172a; border: 1px solid #334155; border-radius: 8px; color: #fff; outline: none;">
      </div>
    ...
    <div style="overflow-x: auto; background: #0f172a; border-radius: 12px; border: 1px solid #334155;">
      <table style="width: 100%; border-collapse: collapse; text-align: right; color: #e2e8f0; table-layout: fixed;">
        <thead><tr style="background: #1e293b; border-bottom: 2px solid #334155;"><th style="width: 30%; padding: 15px; color: #94a3b8;">اسم المكان</th><th style="width: 40%; padding: 15px; color: #94a3b8;">العنوان التفصيلي</th><th style="width: 15%; padding: 15px; color: #94a3b8; text-align:center;">خريطة</th><th style="width: 15%; padding: 15px; color: #94a3b8; text-align:center;">المعرف</th></tr></thead>
        <tbody id="contactsTableBody"><tr><td colspan="4" style="padding:30px; text-align:center; color:#94a3b8;">جاري التحميل...</td></tr></tbody>
      </table>
    </div>
    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px;"><div id="contactsInfo" style="color: #94a3b8;">—</div><div id="contactsPagination" style="display: flex; gap: 6px;"></div></div>
  </div>
</div>

<script>
let selectedAccounts = [], allCampaigns = [], editingCampaignId = null;
const TOOL_NAME = 'Search Location IG';
const DB_TABLE = 'ig_search_locations';
...
document.getElementById('campaignForm').addEventListener('submit', function(e) {
    ...
    fetch(editingCampaignId ? 'api/manage_campaign.php' : 'api/create_campaign.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data) }).then(r=>r.json()).then(res => { if(res.success){ Swal.fire({icon:'success', title:'تم!', timer:1500, showConfirmButton:false, background:'#111827', color:'#fff'}); closeModal(); loadCampaigns(TOOL_NAME); } });
});
...
function renderCampaigns(campaigns) {
    ...
    grid.innerHTML = campaigns.map(c => `
        <div class="campaign-card">
            <div class="campaign-name"><i class="fas fa-map-marker-alt"></i> ${c.name}</div>
            <div class="campaign-info">
                <div class="campaign-info-item"><span class="campaign-info-label"><i class="fas fa-check-double" style="color: #10b981;"></i> النتائج:</span><span>${c.true_count||0}</span></div>
                <div class="campaign-info-item"><span class="campaign-info-label"><i class="fas fa-map-marker-alt" style="color: #ef4444;"></i> المكان:</span><span style="font-weight:bold;">${c.pram1 ||c.id_action || ''}</span></div>
            ...
            <div class="accordion-content" id="acc-${c.id}">
                ...
                <div class="accordion-section"><div class="accordion-section-title"><i class="fas fa-tasks"></i> الإجراء</div><div class="action-btns"><button class="action-btn-full btn-send" onclick="viewResults('${c.campaign_id||c.id}')"><i class="fa-solid fa-table-cells fa-flip"></i> عرض في جدول</button></div></div>
            </div>
        </div>
    `).join('');
}
...
function loadContacts() {
    fetch(`api/ig_basic_info.php?table=${DB_TABLE}&campaign_id=${contactsState.campaign_id}&page=${contactsState.page}&per_page=${contactsState.per_page}&q=${contactsState.q}`).then(r=>r.json()).then(j => {
        ...
        if(!j.data.length) return tb.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:30px; color:#94a3b8;">لا توجد بيانات</td></tr>';
        tb.innerHTML = j.data.map(r => `
            <tr style="border-bottom: 1px solid #1e293b;">
                <td style="padding:12px; font-weight:bold; color:#f8fafc;">${escapeHtml(r.location_name||'--')}</td>
                <td style="padding:12px; font-size:12px; color:#94a3b8;">${escapeHtml(r.address||'--')}</td>
                <td style="padding:12px; text-align:center;"><a href="https://www.google.com/maps/search/?api=1&query=${r.lat},${r.lng}" target="_blank" style="color:#10b981;"><i class="fas fa-map-marked-alt"></i></a></td>
                <td style="padding:12px; text-align:center; font-family:monospace; color:#cbd5e1;">${r.location_id}</td>
            </tr>
        `).join('');
