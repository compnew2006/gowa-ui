...
<Aside type="caution">
  User management endpoints require the `users:read`, `users:write`, or
  `users:delete` permissions respectively.
</Aside>
...
<Aside type="note">Requires `users:read` permission.</Aside>
...
<Aside type="tip">
  The `is_member` field is `true` for users who belong to another organization
  but have been added as members to the current organization.
</Aside>
...
<Aside type="note">Requires `users:write` permission.</Aside>
...
<Aside type="note">
  For **cross-org members** (`is_member: true`), only `role_id` can be updated.
  The role is changed in the `user_organizations` table, not the user's account.
  Other fields like `email`, `password`, `full_name`, and `is_active` cannot be
  modified for members.
</Aside>
...
<Aside type="note">Requires `users:delete` permission.</Aside>

<Aside type="caution">
  - You cannot delete yourself - You cannot delete the last admin user -
  Deleting a native user will remove their account and unassign all their
  contacts - Deleting a **cross-org member** only removes them from the current
  organization — their account remains intact in their home organization
</Aside>
