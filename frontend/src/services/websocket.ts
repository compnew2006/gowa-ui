...
class WebSocketService {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case WS_TYPE_CHAT_CLAIMED:
          ...
          break
        case WS_TYPE_CHAT_CLOSED:
          this.handleChatClosed(store, message.payload)
          break
  ...
  private handleStatusUpdate(store: ReturnType<typeof useContactsStore>, payload: any) {
    store.updateMessageStatus(payload.message_id, payload.status, payload.error_message, payload.wamid)
  }
  ...
  private handleChatClaimed(store: ReturnType<typeof useContactsStore>, payload: any) {
    const assignedTo = payload.assigned_to || payload.assigned_user_id
    const c = store.contacts.find(c => c.id === payload.contact_id)
    if (c) {
      c.assigned_user_id = assignedTo
    ...
    if (current && current.id === payload.contact_id) {
      current.assigned_user_id = assignedTo
      current.assigned_user_name = payload.assigned_to_name
      current.chat_status = payload.chat_status || 'open'
      if (Array.isArray(payload.collaborators)) current.collaborators = payload.collaborators
      // Re-fetch messages now that the chat is claimed/reopened/closed
  ...
  private handleChatReopened(store: ReturnType<typeof useContactsStore>, payload: any) {
    const c = store.contacts.find(c => c.id === payload.contact_id)
    if (c) c.chat_status = payload.chat_status || 'open'
    const current = store.currentContact
    if (current && current.id === payload.contact_id) {
      current.chat_status = payload.chat_status || 'open'
      store.fetchMessages(payload.contact_id)
  ...
  private handleCollaboratorJoined(store: ReturnType<typeof useContactsStore>, payload: any) {
    const newCollab = {
    ...
    if (c) {
      ...
      if (!c.collaborators.find(x => x.user_id === payload.user_id)) {
        c.collaborators.push(newCollab)
      }
    }
    // Update currentContact too for real-time UI
    const current = store.currentContact
    if (current && current.id === payload.contact_id) {
      if (!current.collaborators) current.collaborators = []
      if (!current.collaborators.find(x => x.user_id === payload.user_id)) {
        current.collaborators.push(newCollab)
      }
      store.fetchMessages(payload.contact_id)
    }
  }

  private handleCollaboratorLeft(store: ReturnType<typeof useContactsStore>, payload: any) {
    const c = store.contacts.find(c => c.id === payload.contact_id)
    if (c?.collaborators) {
      c.collaborators = c.collaborators.filter(x => x.user_id !== payload.user_id)
    }
    // Update currentContact too
    const current = store.currentContact
    if (current && current.id === payload.contact_id) {
      if (current.collaborators) {
        current.collaborators = current.collaborators.filter(
          x => x.user_id !== payload.user_id
        )
      }
      store.fetchMessages(payload.contact_id)
    }
