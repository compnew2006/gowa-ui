...
type CannedResponseRequest struct {
	...
	KeepAttachmentIDs []string `json:"keep_attachment_ids,omitempty"`
}
...
type CannedResponseResponse struct {
	...
	Shortcut    string                           `json:"shortcut"`
	Content     string                           `json:"content"`
	Attachments models.CannedResponseAttachments `json:"attachments"`
	Category    string                           `json:"category"`
	IsActive    bool                             `json:"is_active"`
...
func (a *App) CreateCannedResponse(r *fastglue.Request) error {
	...
	req := parsedRequest.Request

	name := strings.TrimSpace(req.Name)
	content := strings.TrimSpace(req.Content)
	if name == "" || (content == "" && len(parsedRequest.AttachmentFiles) == 0) {
		return r.SendErrorEnvelope(fasthttp.StatusBadRequest,
			"name and content are required (or add at least one attachment)", nil, "")
	}
...
func (a *App) UpdateCannedResponse(r *fastglue.Request) error {
	...
	if updatedName != "" {
		cannedResponse.Name = updatedName
	}
	...
	nextAttachments, removedAttachments, err := a.mergeCannedResponseAttachments(
		...
		parsedRequest.HasAttachmentChanges,
	)
	if err != nil {
		return r.SendErrorEnvelope(fasthttp.StatusBadRequest, err.Error(), nil, "")
	}
	if parsedRequest.HasAttachmentChanges {
		cannedResponse.Attachments = nextAttachments
	}
	if req.IsActive != nil {
	...
	if parsedRequest.HasAttachmentChanges {
		a.cleanupCannedResponseAttachments(removedAttachments)
	}
