...
const selectedAccount = ref<string>(
  localStorage.getItem("flows_selected_account") || "all",
);
...
async function createFlow() {
  ...
  try {
    ...
    if (flowBuilderData.value.screens.length > 0) {
      const sanitizedScreens = sanitizeScreensForMeta(
        flowBuilderData.value.screens,
      );
      payload.flow_json = {
    ...
    toast.error(
      getErrorMessage(
        e,
        t("common.failedCreate", { resource: t("resources.flow") }),
      ),
    );
  } finally {
...
function openEditDialog(flow: WhatsAppFlow) {
  ...
  editFlowBuilderData.value = {
    screens: Array.isArray(flow.screens) ? flow.screens : [],
  };
...
async function updateFlow() {
  ...
  try {
    ...
    toast.success(
      t("common.updatedSuccess", { resource: t("resources.Flow") }),
    );
    ...
    toast.error(
      getErrorMessage(
        e,
        t("common.failedUpdate", { resource: t("resources.flow") }),
      ),
    );
  } finally {
...
async function saveFlowToMeta(flow: WhatsAppFlow) {
  ...
  try {
    ...
    toast.success(t("flows.flowSavedToMeta"));
    await fetchFlows();
  } catch (e) {
    toast.error(getErrorMessage(e, t("flows.saveToMetaFailed")));
  } finally {
...
async function publishFlow(flow: WhatsAppFlow) {
  ...
  try {
    ...
    toast.success(t("flows.flowPublished"));
    await fetchFlows();
  } catch (e) {
    toast.error(getErrorMessage(e, t("flows.publishFailed")));
  } finally {
...
async function duplicateFlow(flow: WhatsAppFlow) {
  ...
  try {
    ...
    toast.error(getErrorMessage(e, t("flows.duplicateFailed")));
  } finally {
...
async function syncFlows() {
  if (!selectedAccount.value || selectedAccount.value === "all") {
    toast.error(t("flows.selectAccountToSync"));
    return;
  }
  ...
  try {
    ...
    toast.error(getErrorMessage(e, t("flows.syncFailed")));
  } finally {
  ...
  return screens.map((screen) => ({
    ...
    layout: {
      ...
      children: (screen.layout?.children || []).map((comp: any) => {
        const rest = { ...comp };
        delete rest._id;
        return componentsWithoutId.includes(comp.type) ? rest : comp;
      }),
    },
  }));
}
    ...
    >
      <template #actions>
        <Button
          ...
          ><RefreshCw
            :class="['h-4 w-4 mr-2', isSyncing && 'animate-spin']"
          />{{ $t("flows.syncFromMeta") }}</Button
        >
        <Button variant="outline" size="sm" @click="openCreateDialog"
          ><Plus class="h-4 w-4 mr-2" />{{ $t("flows.createFlow") }}</Button
        >
      </template>
    </PageHeader>
          ...
          <Card>
            <CardHeader>
              <div class="flex items-center justify-between flex-wrap gap-4">
                <div>
                  ...
                  <CardDescription>{{
                    $t("flows.yourFlowsDesc")
                  }}</CardDescription>
                </div>
                <div class="flex items-center gap-2">
                  <Label class="text-sm text-muted-foreground"
                    >{{ $t("flows.account") }}:</Label
                  >
                  <Select
                  ...
                  >
                    <SelectTrigger class="w-[180px]"
                      ><SelectValue :placeholder="$t('flows.allAccounts')"
                    /></SelectTrigger>
                    <SelectContent
            ...
            <CardContent>
              <DataTable
                ...
                :empty-title="
                  searchQuery
                    ? $t('flows.noMatchingFlows')
                    : $t('flows.noFlowsYet')
                "
                :empty-description="
                  searchQuery
                    ? $t('flows.noMatchingFlowsDesc')
                    : $t('flows.noFlowsYetDesc')
                "
                server-pagination
              ...
              >
                ...
                <template #cell-status="{ item: flow }">
                  <Badge
                    v-if="flow.status?.toUpperCase() === 'DEPRECATED'"
                    variant="destructive"
                    class="text-xs"
                  >
                  ...
                  <Badge
                    v-else
                    variant="outline"
                    :class="[getStatusClass(flow.status), 'text-xs']"
                    >{{ flow.status }}</Badge
                  >
                </template>
                ...
                <template #cell-actions="{ item: flow }">
                  <div class="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      :title="$t('flows.duplicateTooltip')"
                    >
                      <Loader2
                        v-if="duplicatingFlowId === flow.id"
                    ...
                    <Button
                      ...
                      target="_blank"
                      :title="$t('flows.previewTooltip')"
                    >
                      <ExternalLink class="h-4 w-4" />
                    </Button>
                    <Button
                      v-if="
                        flow.status?.toUpperCase() !== 'DEPRECATED' &&
                        (flow.has_local_changes || !flow.meta_flow_id)
                      "
                      variant="ghost"
                      size="icon"
                      class="h-8 w-8"
                      ...
                      :title="
                        flow.meta_flow_id
                          ? $t('flows.updateOnMeta')
                          : $t('flows.saveToMeta')
                      "
                    >
                      <Loader2
                        v-if="savingToMetaFlowId === flow.id"
                        class="h-4 w-4 animate-spin"
    ...
    <Dialog v-model:open="showCreateDialog">
      <DialogContent class="max-w-6xl h-[85vh] flex flex-col">
        ...
        <div class="flex gap-4 py-2 border-b">
          ...
          <div class="flex items-center gap-2">
            <Label class="text-sm whitespace-nowrap"
              >{{ $t("flows.category") }}:</Label
            >
            <Select v-model="formData.category" :disabled="isCreating"
              ><SelectTrigger class="w-[180px]"
                ><SelectValue
                  :placeholder="$t('flows.selectCategory')" /></SelectTrigger
              ><SelectContent
                ><SelectItem
    ...
    <Dialog v-model:open="showEditDialog">
      <DialogContent class="max-w-6xl h-[85vh] flex flex-col">
        ...
        <div class="flex gap-4 py-2 border-b">
          ...
          <div
            v-if="flowToEdit?.meta_flow_id"
            class="flex items-center gap-2 ml-auto"
          >
            <Badge variant="outline"
              >Meta ID: {{ flowToEdit.meta_flow_id }}</Badge
            >
          </div>
        </div>
        <div class="flex-1 overflow-hidden py-4">
