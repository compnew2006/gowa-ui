...
function sanitizeScreensForMeta(screens: any[]): any[] {
  return screens.map(screen => ({
    ...
    layout: { type: screen.layout?.type || 'SingleColumnLayout', children: (screen.layout?.children || []).map((comp: any) => { const rest = { ...comp }; delete rest._id; return componentsWithoutId.includes(comp.type) ? rest : comp }) }
  }))
}
</script>
