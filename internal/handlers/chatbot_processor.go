...
func (a *App) processIncomingMessageFull(phoneNumberID string, msg IncomingTextMessage, profileName string, isGroup, isNewsletter bool, senderName, senderJID string) {
	...
	// In groups, reactions must attribute to the group contact (msg.From is already the
	// group JID in that case, set by processGowaMessage).
	...
	a.saveIncomingMessage(account, contact, msg.ID, messageType, messageText, mediaInfo, replyToWAMID, senderName, senderJID)
