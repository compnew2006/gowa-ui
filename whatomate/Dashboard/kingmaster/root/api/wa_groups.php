...
try {
    ...
    // Count total
    $stmt = $db->prepare("SELECT COUNT(*) AS total FROM gb_wa $where");
    $stmt->execute($params);
    $total = (int)($stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);

    // Fetch page
    $sql = "SELECT name, gb_id, participantsCount FROM gb_wa $where ORDER BY id DESC LIMIT :limit OFFSET :offset";
    $stmt = $db->prepare($sql);
