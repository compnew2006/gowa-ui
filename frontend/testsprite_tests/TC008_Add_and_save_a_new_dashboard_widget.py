...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login (http://localhost:8080/login) to access the login form.
        ...
        # -> Click the 'Add Widget' button to open the widget creation/configuration dialog (index 478).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/main/div/header/div/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the Widget Name and Description fields, then open the Data Source dropdown so an option can be selected.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[4]/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Test Widget')
        
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[4]/div[2]/div[2]/textarea').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Automated widget created during test')
        ...
        # -> Select the 'Sessions' data source option, create the widget, then verify that 'Test Widget' appears on the dashboard.
        frame = context.pages[-1]
        # Click element
