...
func TestWorker_HandleRecipientJob_SkipsAlreadyProcessedRecipient(t *testing.T) {
	w := testWorker(t)
	org, _, _, campaign, recipient := createTestCampaignData(t, w)

	require.NoError(t, w.DB.Model(recipient).Updates(map[string]interface{}{
	...
	require.NoError(t, w.DB.Model(&models.Message{}).Where("metadata->>'campaign_id' = ?", campaign.ID.String()).Count(&messageCount).Error)
