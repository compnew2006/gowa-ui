...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign in' button to submit the login form and wait for the app to redirect to the dashboard where the Templates navigation link can be accessed.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div/form/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Template Successfully Updated').first).to_be_visible(timeout=3000)
