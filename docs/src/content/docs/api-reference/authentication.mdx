...
Gowa-UI supports two authentication methods:

1. **JWT Tokens** - For user sessions and frontend applications
2. **API Keys** - For server-to-server integrations and automation

After logging in, you receive an access token and refresh token. Include the access token in the `Authorization` header for all protected API requests. Alternatively, use an [API key](/Gowa-UI/api-reference/api-keys) in the `X-API-Key` header.
