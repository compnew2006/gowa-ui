...
const canWriteRoles = computed(() => authStore.hasPermission("roles", "write"));
const canDeleteRoles = computed(() =>
  authStore.hasPermission("roles", "delete"),
);
const isReadOnlyDialog = computed(() => {
  if (!editingRole.value) {
    return !canWriteRoles.value;
  }
  ...
  return editingRole.value.is_system && !isSuperAdmin.value;
});
const canSaveRole = computed(
  () => !isReadOnlyDialog.value && canWriteRoles.value,
);
const canEditPermissions = computed(() => canSaveRole.value);
const dialogTitle = computed(() => {
  if (!editingRole.value) {
    return t("roles.createRole");
  }
  return isReadOnlyDialog.value ? t("roles.viewRole") : t("roles.editRole");
});
const dialogDescription = computed(() => {
  if (!editingRole.value) {
    return t("roles.createRoleDesc");
  }
...
function getRoleActionLabel(role: Role): string {
  if (role.is_system) {
    return canWriteRoles.value && isSuperAdmin.value
      ? t("roles.editPermissions")
      : t("roles.viewPermissions");
  }
  return canWriteRoles.value ? t("roles.editRole") : t("roles.viewRole");
}
...
async function saveRole() {
  ...
  try {
    if (editingRole.value) {
      const updateData: UpdateRoleData = editingRole.value.is_system
        ? {
    ...
    >
      <template #actions>
        <Button
          ...
          ><Plus class="h-4 w-4 mr-2" />{{ $t("roles.addRole") }}</Button
        >
      </template>
    </PageHeader>
          ...
          <Card>
            ...
            <CardContent>
              <DataTable
                ...
                :empty-title="
                  searchQuery
                    ? $t('roles.noMatchingRoles')
                    : $t('roles.noRolesYet')
                "
                :empty-description="
                  searchQuery
                    ? $t('roles.noMatchingRolesDesc')
                    : $t('roles.noRolesYetDesc')
                "
              ...
              >
                ...
                <template #cell-description="{ item: role }">
                  <span class="text-muted-foreground max-w-xs truncate block">{{
                    role.description || "-"
                  }}</span>
                </template>
                ...
                <template #cell-users="{ item: role }">
                  <div class="flex items-center justify-center gap-1">
                    <Users class="h-4 w-4 text-muted-foreground" /><span>{{
                      role.user_count
                    }}</span>
                  </div>
                </template>
                    ...
                    <Tooltip
                      ><TooltipTrigger as-child
                        ><Button
                          ...
                          size="icon"
                          class="h-8 w-8"
                          :aria-label="getRoleActionLabel(role)"
                          @click="openEditDialog(role)"
                          ><Pencil class="h-4 w-4" /></Button></TooltipTrigger
                      ><TooltipContent>{{
                        getRoleActionLabel(role)
                      }}</TooltipContent></Tooltip
                    >
                    <Tooltip v-if="canDeleteRoles && !role.is_system"
                      ><TooltipTrigger as-child
                        ><Button
                          variant="ghost"
                          size="icon"
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto space-y-4 py-4 pr-2">
          <div class="space-y-2">
            ...
            ><Input
              ...
              :disabled="isReadOnlyDialog || editingRole?.is_system"
            />
          </div>
          <div class="space-y-2">
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between">
              ...
              ><span class="text-xs text-muted-foreground"
                ...
                {{ $t("common.selected") || "selected" }}</span
              >
            </div>
        ...
        <DialogFooter class="pt-4 border-t">
          <Button variant="outline" size="sm" @click="isDialogOpen = false">{{
            isReadOnlyDialog ? $t("common.close") : $t("common.cancel")
          }}</Button>
          <Button
            v-if="canSaveRole"
            size="sm"
