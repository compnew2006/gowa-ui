# Graph Report - whatomate  (2026-07-25)

## Corpus Check
- 790 files · ~716,804 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8403 nodes · 22278 edges · 337 communities (274 shown, 63 thin omitted)
- Extraction: 72% EXTRACTED · 27% INFERRED · 0% AMBIGUOUS · INFERRED: 6126 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8203e654`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Accounts Page Object
- Chat View Components
- Contact Info Panel
- Canned Response Picker
- Framework Auth Utils
- Accounts API Tests
- App Layout & Nav
- WebSocket Client
- API Utils & Chatbot
- Organization Switcher
- AI Context Services
- API Keys Tests
- Main Server Config
- Gowa Mock Server
- Base Page Objects
- Audit Log & Detail Panels
- API Helpers
- Campaign & Header Utils
- Users Store & View
- Chatbot Graph Tests
- Teams Store & View
- Auth Flow Tests
- Auth & Token Gaps Tests
- Radix UI Primitives
- Webhooks View
- Flow Canvas Builder
- Agent Transfers Tests
- Radix UI Components
- Gowa Message Handler
- SSO Providers
- Radix UI Buttons
- WebSocket Middleware
- Menubar Components
- Template Engine
- Template Picker
- Database Models
- Connection Settings
- Agent Load Balancer
- Chatbot Flows Page Object
- Context Menu Components
- Command Components
- Agent Transfers View
- Conversation Notes Tests
- Dashboard & Widgets
- User & Auth Handlers
- SLA Processors
- Campaigns Tests
- Account Detail View
- Message Handlers Tests
- Page Object Helpers
- Catalogs Service
- Widget Data Services
- Keywords View
- Cache Utils
- API Envelope Helpers
- Campaigns Page Object
- Campaign Queue Worker
- Catalog API Tests
- Chatbot AI Handler
- Template Worker Tests
- Date Picker Components
- AI Contexts View
- Campaign & Flow Handlers
- Canned Response Tests
- Media Access Tests
- Queue Tests
- Dialog Components
- Custom Actions View
- Toast Components
- Chatbot Settings View
- Gowa Server Settings
- Chatbot Migration
- Template Utils
- Calendar Components
- User & Sync Tests
- PubSub System
- Build Config
- Message Preview
- Flow Builder Nodes
- Contact Handlers
- Chatbot Processor Tests
- Test Helpers
- Node Property Config
- Nav Menu Components
- Range Calendar Components
- Contact Detail View
- Gowa Client Tests
- Agent Transfer Types
- App Dependencies
- Custom Action Handlers
- Message Dispatchers
- Chatbot Models
- IVR & Calling Cache
- Import/Export Dialog
- Canned Response Handlers
- Chatbot Flow Execution
- Dialog Components
- Contacts View
- Custom Actions Tests
- Gowa API Mocks
- IVR Call Flows
- Meta API Credentials
- Client Base Requests
- IVR Audio Bridging
- Flow Builder UI
- Database Models
- Agent Transfers E2E
- GOWA Client Interfaces
- Flow Interactive Preview
- Database Seeding Unit
- IVR Call Session
- Data Table E2E
- Sheet UI Component
- TypeScript Compiler Config
- Organization Settings Unit
- Account Management Service
- Organization Service Handlers
- Templates Unit Tests
- WebRTC DTMF
- Conversation Notes UI
- Chatbot Flows Page
- Meta Webhook Processor
- Audit Logging System
- Message DTOs
- System Configuration
- General Settings E2E
- Internationalization
- Drawer UI Component
- Media Storage Handlers
- Business Profile Mocks
- GOWA Webhook Utils
- Contacts Page E2E
- Media Burst Dialog
- Message Buttons Editor
- Core App Handlers
- Rate Limiting Middleware
- Call Server Client
- Analytics Fixtures
- Agent Transfer Session
- Project Build Config
- Data Table Component
- Group Management
- Agent Transfer Dispatcher
- Import Export Handlers
- Flow API Client
- Theme Switcher UI
- Toggle UI Component
- Template Management
- GOWA Instance Devices
- Webhook Dispatcher
- Organization Members Unit
- Message Status Unit
- Test Fixtures
- Card Grid Page E2E
- Role Permissions Handlers
- GOWA Webhook Processor
- Meta API Types
- Database Migration Models
- Template Documentation
- Form UI Component
- Number Field Component
- Sticky Agent Calls
- Tag Handlers
- Meta API Mocks
- Config Loading Unit
- Docs Build Config
- Collapsible Metadata UI
- Pin Input Component
- Chat Input Helper
- GOWA Webhook Payloads
- GOWA Device Mocks
- S3 Storage Client
- Panel Config Editor
- Agent Analytics Service
- IVR Flow Handlers
- Audit Logs Unit
- Piper TTS Unit
- Canned Responses E2E
- Flow Debug Panel
- Accordion UI Component
- Tabs UI Component
- Media Export Utilities
- Template Engine
- Bulk Campaign Models
- Flow Builder E2E
- Permission Matrix UI
- Device Management Handlers
- API Key Handlers
- Collaborative Chat Actions
- Contact Utilities
- Webhook Dispatcher Unit
- Commerce Catalog API
- Meta API Responses
- Template API Client
- Audio Player
- Outbound Call Service
- Conversation Notes Handlers
- Dropdown Menu Component
- GOWA App Auth
- campaign-templates.spec.ts
- Chat Status
- Message Builder
- Redis Config
- Tailwind Config
- Build Scripts
- Time
- Gowa Backfill
- Webhooks
- Call Handling
- Flow Graph
- Account Tabs Spec
- API Mock Dialog
- Hover Card
- Resizable Panels
- User Profiles
- Flow Validation
- Template Builder
- User Builder
- WhatsApp Account Builder
- Meta Registry Tests
- Message Sending
- Documentation
- API Docs
- Input Bar Component
- Radio Group
- useInfiniteScroll.ts
- Node Config
- Chat Fetching
- Gowa Device Pairing
- Contact Builder
- CRUD Form Dialog
- ConfirmDialog.vue
- Checkbox.vue
- CannedResponse
- SelectContent.vue
- onTabKeydown
- .UpdateBusinessProfile
- Model Tests
- Organization Builder
- Team Builder
- TestClient_ExchangeCodeForToken
- Frontend Config
- Dashboard Actions
- Device Connection
- TestClient_Capabilities_AllTrue
- Role Models
- CSRFProtection
- chart.js
- Auth Cookies
- Template Tests
- RequestHandler
- HTTP Callbacks
- Transfer Callbacks
- Brand Assets
- Context
- I18n Validation
- WSMessage
- Type Declarations
- Media Redownload
- Embedded App
- Flows Documentation
- GOWA QA Reports
- Message Parsers
- Connection Tests
- Media Upload
- Dashboard Screenshots
- Docs Config
- Aspect Ratio Component
- Call Rejection
- Read Receipts
- Recipient Utilities
- Tab Implementation Plan
- Call Cache
- Chat Handlers
- Design System
- GOWA Specs
- Canned Responses
- Content Config
- Grid Layout
- Date Utilities
- UI Components
- Drawer Component
- Vue Framework
- Emoji Picker
- Chart.js Integration
- Flow Background
- Flow Controls
- ESLint Config
- Context Menu
- Sidebar Layout
- Media URL Handling
- Tailwind Configuration
- Message Extensions
- Pending/Me Tabs Specs
- Project Documentation
- piper_test.go
- Crowdin Config
- Quickstart Guide
- Keyword Rule Editor UI
- AI Contexts UI
- AI Context Editor UI
- WhatsApp Flows UI
- WhatsApp Flow Builder UI
- Template Editor UI
- Custom Actions Docs
- Crowdin Configuration
- E2E Test Architecture
- Frontend Entry HTML
- Favicon Asset
- Frontend README
- i18n Documentation
- GoReleaser Config
- LogAudit
- organization.go
- factories.go
- PaginationControls.vue
- External Dependency
- Typing & Revoke Report
- Catalog Restoration Plan
- Tabs Release Plan
- Slider.vue
- Spinner.vue

## God Nodes (most connected - your core abstractions)
1. `CreateTestOrganization()` - 608 edges
2. `newTestApp()` - 580 edges
3. `GetResponseStatusCode()` - 535 edges
4. `SetAuthContext()` - 482 edges
5. `CreateTestUser()` - 471 edges
6. `SetPathParam()` - 314 edges
7. `NewGETRequest()` - 313 edges
8. `page` - 278 edges
9. `NewJSONRequest()` - 271 edges
10. `GetResponseBody()` - 260 edges

## Surprising Connections (you probably didn't know these)
- `Keyword Rules Screenshot` --conceptually_related_to--> `README`  [INFERRED]
  docs/public/images/03-keyword-rules.png → README.md
- `Conversation Flows Screenshot` --conceptually_related_to--> `README`  [INFERRED]
  docs/public/images/07-conversation-flows.png → README.md
- `Chatbot Dark Screenshot` --conceptually_related_to--> `README`  [INFERRED]
  docs/public/images/chatbot-dark.png → README.md
- `Chatbot Light Screenshot` --conceptually_related_to--> `README`  [INFERRED]
  docs/public/images/chatbot-light.png → README.md
- `Conversation Flow Dark Screenshot` --conceptually_related_to--> `README`  [INFERRED]
  docs/public/images/conversation-flow-dark.png → README.md

## Import Cycles
- 2-file cycle: `frontend/src/components/ui/toast/index.ts -> frontend/src/components/ui/toast/use-toast.ts -> frontend/src/components/ui/toast/index.ts`

## Hyperedges (group relationships)
- **Whatomate Project Identity** — serena_project, docs_src_assets_logo_dark_svg, docs_src_assets_logo_light_svg [EXTRACTED 0.90]
- **Chatbot Automation Components** — docs_src_content_docs_features_chatbot, keyword_rules, conversation_flows, ai_contexts [EXTRACTED 0.90]
- **Role-Based Access Control System** — docs_src_content_docs_api_reference_users, docs_src_content_docs_api_reference_roles, permissions, docs_src_content_docs_api_reference_organizations [EXTRACTED 0.95]
- **Message Delivery Channels** — docs_src_content_docs_api_reference_messages, docs_src_content_docs_api_reference_templates, whatsapp_flows, docs_src_content_docs_features_canned_responses [INFERRED 0.75]

## Communities (337 total, 63 thin omitted)

### Community 0 - "Accounts Page Object"
Cohesion: 0.02
Nodes (21): AccountsPage, ChatbotSettingsPage, ChatPage, TemplatesPage, getSidebarMenuItems(), descriptionInput(), getAriaSort(), gotoCreateRole() (+13 more)

### Community 1 - "Chat View Components"
Cohesion: 0.01
Nodes (94): actionIconMap, activeTransfer, activeTransferId, assignableUsers, assignSearchQuery, authStore, brokenMediaIds, burstTimeMs (+86 more)

### Community 2 - "Contact Info Panel"
Cohesion: 0.02
Nodes (101): authStore, canEditTags, collapsedSections, contactsStore, contactTags, emit, hasMetadata, isAdminOrManager (+93 more)

### Community 3 - "Canned Response Picker"
Cohesion: 0.02
Nodes (105): emit, handleCancel(), handleConfirm(), open, props, emit, emits, forwarded (+97 more)

### Community 4 - "Framework Auth Utils"
Cohesion: 0.03
Nodes (80): Credentials, loginAs(), loginAsSuperAdmin(), SUPER_ADMIN, BaseOpts, createFlowBody(), deleteFlowBody(), editFlowBody() (+72 more)

### Community 5 - "Accounts API Tests"
Cohesion: 0.07
Nodes (87): T, TestApp_CreateAccount_Success(), TestApp_CreateAccount_Unauthorized(), TestApp_CreateAccount_ValidationErrors(), TestApp_CreateAccount_WithOptionalFields(), TestApp_DeleteAccount_CrossOrgIsolation(), TestApp_DeleteAccount_InvalidID(), TestApp_DeleteAccount_NotFound() (+79 more)

### Community 6 - "App Layout & Nav"
Cohesion: 0.17
Nodes (6): burstMinutes, emit, hasFiles, open, props, { t }

### Community 7 - "WebSocket Client"
Cohesion: 0.06
Nodes (78): Conn, UUID, NewClient(), NewUnauthenticatedClient(), ClientAuthenticated(), ClientHandleAuthMessage(), ClientOrgID(), ClientSendChan() (+70 more)

### Community 8 - "API Utils & Chatbot"
Cohesion: 0.05
Nodes (43): Account, accounts, columns, componentsWithoutId, confirmDeleteFlow(), confirmPublishFlow(), confirmSaveToMeta(), createFlow() (+35 more)

### Community 9 - "Organization Switcher"
Cohesion: 0.07
Nodes (87): Server, T, newChatsMock(), TestSyncGowaInstanceDeviceContacts_AgentDenied(), TestSyncGowaInstanceDeviceContacts_Idempotent(), TestSyncGowaInstanceDeviceContacts_ImportsChats(), TestSyncGowaInstanceDeviceContacts_NeedsAccountProvisioning(), T (+79 more)

### Community 10 - "AI Context Services"
Cohesion: 0.12
Nodes (47): createSessionForChatbotTest(), createTestAIContext(), createTestChatbotFlow(), createTestKeywordRule(), getChatbotFlowPermissions(), AIContext, App, ChatbotFlow (+39 more)

### Community 11 - "API Keys Tests"
Cohesion: 0.10
Nodes (67): APIKey, TestApp_GetAgentAnalytics_EmptyData(), TestApp_GetAgentAnalytics_Success(), T, TestApp_CreateAPIKey_EmptyNameRejected(), TestApp_CreateAPIKey_ExpiresAtParsedAndStored(), TestApp_CreateAPIKey_HashIsBcryptOfFullKey(), TestApp_CreateAPIKey_InvalidExpiresAtFormat() (+59 more)

### Community 12 - "Main Server Config"
Cohesion: 0.12
Nodes (54): mockCatalogServer, createCatalogTestAccount(), createTestCatalog(), createTestCatalogProduct(), App, Catalog, CatalogProduct, Server (+46 more)

### Community 13 - "Gowa Mock Server"
Cohesion: 0.05
Nodes (85): downloadResponse, gowaExtMock, mediaCacheItem, mockAPIServer, mockGowaServer, sendResponse, Server, T (+77 more)

### Community 14 - "Base Page Objects"
Cohesion: 0.04
Nodes (8): BasePage, AIContextsPage, KeywordsPage, DialogPage, escapeCssSelector(), escapeRegex(), ChatbotFlowBuilderPage, LoginPage

### Community 15 - "Audit Log & Detail Panels"
Cohesion: 0.02
Nodes (80): connectionStore, isTesting, password, serverUrl, username, actionConfig, isLoading, loadLogs() (+72 more)

### Community 16 - "API Helpers"
Cohesion: 0.09
Nodes (17): addNote(), authStore, editingContent, editingNoteId, emit, isSaving, newNoteContent, notesEndRef (+9 more)

### Community 17 - "Campaign & Header Utils"
Cohesion: 0.03
Nodes (67): clear(), useHeaderMedia(), Account, accounts, addRecipients(), addRecipientsFromCSV(), addRecipientsTab, auditRefreshKey (+59 more)

### Community 18 - "Users Store & View"
Cohesion: 0.03
Nodes (65): CreateUserData, FetchUsersParams, FetchUsersResponse, UpdateUserData, User, UserRole, useUsersStore, authStore (+57 more)

### Community 19 - "Chatbot Graph Tests"
Cohesion: 0.11
Nodes (67): ChatGraph, ChatNodeType, chatGraphPath(), createChatbotSettings(), AIConfig, App, ChatbotFlow, Contact (+59 more)

### Community 20 - "Teams Store & View"
Cohesion: 0.03
Nodes (59): Team, TeamMember, teamsService, CreateTeamData, FetchTeamsParams, FetchTeamsResponse, UpdateTeamData, useTeamsStore (+51 more)

### Community 21 - "Auth Flow Tests"
Cohesion: 0.06
Nodes (81): T, TestApp_GeneratedTokensAreValid(), TestApp_Login_InactiveUser(), TestApp_Login_InvalidRequestBody(), TestApp_Login_Success(), TestApp_Login_UserNotFound(), TestApp_Login_UserWithRole(), TestApp_Login_WrongPassword() (+73 more)

### Community 22 - "Auth & Token Gaps Tests"
Cohesion: 0.10
Nodes (19): ApiResponse, ExtractedMessage, MediaInfo, Reaction, getStringFromMap(), ChatbotFlow, ChatbotSettings, Contact (+11 more)

### Community 23 - "Radix UI Primitives"
Cohesion: 0.03
Nodes (39): delegatedProps, emits, forwarded, props, delegatedProps, props, props, delegatedProps (+31 more)

### Community 24 - "Webhooks View"
Cohesion: 0.04
Nodes (59): Webhook, WebhookEvent, webhooksService, authStore, availableEvents, breadcrumbs, canDelete, canWrite (+51 more)

### Community 25 - "Flow Canvas Builder"
Cohesion: 0.03
Nodes (49): activityOpen, addNodeFromPalette(), auditRefreshKey, availableFlows, availableVariables, cancelDialogOpen, completionConfig, completionConfigOpen (+41 more)

### Community 26 - "Agent Transfers Tests"
Cohesion: 0.21
Nodes (28): auditCount(), auditCountIs(), claimForTest(), Contact, DB, Organization, T, UUID (+20 more)

### Community 27 - "Radix UI Components"
Cohesion: 0.13
Nodes (64): createTestAgent(), createTestTeam(), createTestTransfer(), AgentTransfer, App, T, Team, User (+56 more)

### Community 28 - "Gowa Message Handler"
Cohesion: 0.14
Nodes (16): deviceID(), Account, toJID(), Account, Context, Client, Account, Context (+8 more)

### Community 29 - "SSO Providers"
Cohesion: 0.17
Nodes (11): AccountRequest, AccountResponse, accountToResponse(), generateNumericPIN(), generateVerifyToken(), Context, App, Request (+3 more)

### Community 30 - "Radix UI Buttons"
Cohesion: 0.04
Nodes (43): delegatedProps, props, delegatedProps, props, Props, ButtonVariants, delegatedProps, forwardedProps (+35 more)

### Community 31 - "WebSocket Middleware"
Cohesion: 0.07
Nodes (28): activeTab, allColumns, closeDialog(), defaultColumns, downloadSampleCsv(), emit, exportColumns, handleExport() (+20 more)

### Community 32 - "Menubar Components"
Cohesion: 0.03
Nodes (43): delegatedProps, emits, forwarded, props, delegatedProps, emits, forwarded, props (+35 more)

### Community 33 - "Template Engine"
Cohesion: 0.09
Nodes (57): copyMap(), evaluateCondition(), getNestedValue(), processConditionals(), processForLoops(), splitPath(), T, TestCopyMap_Empty() (+49 more)

### Community 34 - "Template Picker"
Cohesion: 0.19
Nodes (4): ChatbotStatsResponse, App, Request, UUID

### Community 35 - "Database Models"
Cohesion: 0.07
Nodes (35): DeletedAt, Template, Account, ActionType, Message, MessageType, Contact, TeamMember (+27 more)

### Community 36 - "Connection Settings"
Cohesion: 0.23
Nodes (23): Auth(), generateTestToken(), generateTokenWithSecret(), Duration, Request, T, UUID, newTestRequest() (+15 more)

### Community 37 - "Agent Load Balancer"
Cohesion: 0.11
Nodes (46): agentLoad, LoadCounter, Assigner, Client, DB, Logger, UUID, New() (+38 more)

### Community 39 - "Context Menu Components"
Cohesion: 0.04
Nodes (40): emits, forwarded, props, delegatedProps, emits, forwarded, props, delegatedProps (+32 more)

### Community 40 - "Command Components"
Cohesion: 0.04
Nodes (42): allGroups, allItems, { contains }, delegatedProps, emits, filterState, forwarded, props (+34 more)

### Community 41 - "Agent Transfers View"
Cohesion: 0.05
Nodes (43): chatbotService, AgentTransfer, getSLAStatus(), SLAStatus, activeTab, agents, allActiveTransfers, allowQueuePickup (+35 more)

### Community 42 - "Conversation Notes Tests"
Cohesion: 0.09
Nodes (67): fakeOAuthProvider, chatRWRole(), DB, T, UUID, TestApp_CreateConversationNote_EmptyContentRejected(), TestApp_CreateConversationNote_PermissionDenied(), TestApp_CreateConversationNote_Success() (+59 more)

### Community 43 - "Dashboard & Widgets"
Cohesion: 0.04
Nodes (52): useAppToast(), DashboardWidget, LayoutItem, WidgetData, widgetsService, applyCustomRange(), authStore, availableFields (+44 more)

### Community 44 - "User & Auth Handlers"
Cohesion: 0.28
Nodes (6): App, Request, User, parseSuperAdminField(), userAuditSnapshot(), userToResponse()

### Community 45 - "SLA Processors"
Cohesion: 0.11
Nodes (28): SLAProcessor, SLASendOptions(), AgentTransfer, App, ChatbotSettings, Contact, Context, Duration (+20 more)

### Community 46 - "Campaigns Tests"
Cohesion: 0.26
Nodes (49): createTestCampaign(), createTestRecipient(), App, T, UUID, TestApp_Campaign_CrossOrgIsolation(), TestApp_CancelCampaign_AlreadyFinished(), TestApp_CancelCampaign_Success() (+41 more)

### Community 47 - "Account Detail View"
Cohesion: 0.05
Nodes (45): account, accountId, authStore, basePath, breadcrumbs, canDelete, canWrite, canWriteDevices (+37 more)

### Community 48 - "Message Handlers Tests"
Cohesion: 0.19
Nodes (47): mockWhatsAppServer, ChatbotSendOptions(), createTestAccount(), App, Server, T, UUID, WhatsAppAccount (+39 more)

### Community 49 - "Page Object Helpers"
Cohesion: 0.06
Nodes (4): ApiKeysPage, CustomActionsPage, TableSettingsPage, TagsPage

### Community 50 - "Catalogs Service"
Cohesion: 0.14
Nodes (25): AuthWithDB(), CORS(), GetOrganization(), GetOrganizationID(), GetUser(), GetUserID(), DB, FastMiddleware (+17 more)

### Community 51 - "Widget Data Services"
Cohesion: 0.14
Nodes (24): ChartPoint, DataPoint, FilterInput, GroupedSeriesData, GroupedSeriesDataset, TableRow, WidgetDataResponse, WidgetRequest (+16 more)

### Community 52 - "Keywords View"
Cohesion: 0.29
Nodes (11): App, Server, T, newMockTemplateServer(), newTemplateTestApp(), TestExtParamNames_DuplicateParams(), TestExtParamNames_MixedParams(), TestExtParamNames_NamedParams() (+3 more)

### Community 53 - "Cache Utils"
Cohesion: 0.09
Nodes (14): chatbotSettingsCache, UserPermissions, whatsAppAccountCache, AIContext, ChatbotFlow, ChatbotSettings, Context, DB (+6 more)

### Community 54 - "API Envelope Helpers"
Cohesion: 0.09
Nodes (40): Pagination, App, Request, endOfDay(), DB, Request, Time, listEnvelope() (+32 more)

### Community 56 - "Campaign Queue Worker"
Cohesion: 0.14
Nodes (41): T, TestClient_CreateCatalog_APIError(), TestClient_CreateCatalog_Success(), TestClient_CreateProduct_Success(), TestClient_DeleteCatalog_Success(), TestClient_DeleteProduct_APIError(), TestClient_DeleteProduct_Success(), TestClient_ListCatalogProducts_Empty() (+33 more)

### Community 57 - "Catalog API Tests"
Cohesion: 0.05
Nodes (35): categoryLabels, emit, extractParamNames(), filteredTemplates, getBodyContent(), groupedTemplates, isLoading, isOpen (+27 more)

### Community 58 - "Chatbot AI Handler"
Cohesion: 0.14
Nodes (42): ResolveParams(), createMinimalCampaignData(), createTestCampaignData(), Organization, T, Template, User, WhatsAppAccount (+34 more)

### Community 59 - "Template Worker Tests"
Cohesion: 0.04
Nodes (54): emit, props, { t }, DateRangeResult, TimeRangePreset, useDateRange(), UseDateRangeOptions, agentAnalyticsService (+46 more)

### Community 60 - "Date Picker Components"
Cohesion: 0.16
Nodes (15): appInfo, connectionStore, errorMsg, isLoading, loadInfo(), basicAuthHeader(), normalizeBaseUrl(), sameOriginBaseUrl() (+7 more)

### Community 61 - "AI Contexts View"
Cohesion: 0.10
Nodes (20): ExportConfig, ExportRequest, ImportConfig, ImportDataRequest, AgentTransfer, ChatbotSettings, Contact, App (+12 more)

### Community 62 - "Campaign & Flow Handlers"
Cohesion: 0.12
Nodes (12): App, Request, App, Request, flowToResponse(), App, Request, WhatsAppFlow (+4 more)

### Community 63 - "Canned Response Tests"
Cohesion: 0.15
Nodes (13): SSOProviderPublic, SSOProviderRequest, SSOProviderResponse, SSOState, UserInfo, generateRandomString(), getString(), App (+5 more)

### Community 64 - "Media Access Tests"
Cohesion: 0.25
Nodes (9): WebhookRequest, WebhookResponse, App, Request, UUID, Webhook, validateWebhookURL(), webhookAuditSnapshot() (+1 more)

### Community 65 - "Queue Tests"
Cohesion: 0.20
Nodes (39): cleanStream(), Client, T, makeRecipientJob(), skipIfNoRedis(), TestConsume_EmptyQueue(), TestConsume_MultipleJobs(), TestConsume_ProcessesJob() (+31 more)

### Community 66 - "Dialog Components"
Cohesion: 0.25
Nodes (12): Duration, NewWithTimeout(), Request, Response, T, TestClient_DownloadMedia(), TestClient_GetMediaURL(), TestClient_MarkMessageRead() (+4 more)

### Community 67 - "Custom Actions View"
Cohesion: 0.04
Nodes (47): computedDescription, computedSubmitLabel, computedTitle, emit, handleCancel(), handleSubmit(), open, props (+39 more)

### Community 68 - "Toast Components"
Cohesion: 0.07
Nodes (30): ToastProps, toastVariants, delegatedProps, emits, forwarded, props, delegatedProps, props (+22 more)

### Community 69 - "Chatbot Settings View"
Cohesion: 0.03
Nodes (55): displayItems, effectiveTotalItems, emit, handlePageChange(), handleSort(), hasSortableColumns, needsPagination, props (+47 more)

### Community 70 - "Gowa Server Settings"
Cohesion: 0.03
Nodes (75): getErrorMessage(), isAxiosError(), GowaDevice, GowaServer, gowaServersService, CreateGowaServerData, useGowaServersStore, revokeMessage() (+67 more)

### Community 71 - "Chatbot Migration"
Cohesion: 0.23
Nodes (8): ConversationNote, ConversationNoteRequest, ConversationNoteResponse, App, Request, Time, UUID, noteToResponse()

### Community 72 - "Template Utils"
Cohesion: 0.11
Nodes (17): Time, UUID, Context, Mutex, mockHandler, RecipientJob, Account, Context (+9 more)

### Community 73 - "Calendar Components"
Cohesion: 0.13
Nodes (37): ExtParamNames(), isPositionalParam(), ReplaceWithJSONBParams(), ReplaceWithStringParams(), ResolveParamsFromMap(), T, TestExtParamNames_DuplicateParams(), TestExtParamNames_MixedParams() (+29 more)

### Community 74 - "User & Sync Tests"
Cohesion: 0.05
Nodes (27): delegatedProps, emits, forwarded, props, delegatedProps, forwardedProps, props, delegatedProps (+19 more)

### Community 75 - "PubSub System"
Cohesion: 0.06
Nodes (121): TestApp_GetAgentAnalytics_AgentSeesOwnStats(), createTestCannedResponse(), App, CannedResponse, T, UUID, TestApp_CannedResponse_FullLifecycle(), TestApp_CreateCannedResponse() (+113 more)

### Community 76 - "Build Config"
Cohesion: 0.19
Nodes (17): Client, Context, Logger, UUID, NewPublisher(), NewSubscriber(), T, TestPublisher_PublishCampaignStats_NoSubscribersIsNotAnError() (+9 more)

### Community 77 - "Message Preview"
Cohesion: 0.05
Nodes (37): autoprefixer, eslint, eslint-plugin-vue, devDependencies, autoprefixer, eslint, eslint-plugin-vue, pg (+29 more)

### Community 78 - "Flow Builder Nodes"
Cohesion: 0.06
Nodes (39): emit, handlePlayPause(), props, statusColor, statusLabel, stepsExpanded, timelineExpanded, variableEntries (+31 more)

### Community 79 - "Contact Handlers"
Cohesion: 0.06
Nodes (25): OutputHandle, props, summary, urlTitle, buttons, outputHandles, props, outputHandles (+17 more)

### Community 80 - "Chatbot Processor Tests"
Cohesion: 0.08
Nodes (30): Collaborator, AssignContactRequest, ButtonContent, ContactResponse, ContactSessionDataResponse, CreateContactRequest, InteractiveContent, MessageResponse (+22 more)

### Community 81 - "Test Helpers"
Cohesion: 0.06
Nodes (33): agentsLogKey, aiLogKey, aiProviders, aiSettings, authStore, availableModels, availableUsers, BusinessHour (+25 more)

### Community 82 - "Node Property Config"
Cohesion: 0.09
Nodes (33): addHeader(), addPhoneButton(), addReplyButton(), addResponseMapping(), addUrlButton(), config, ctaCount, defaultSchedule (+25 more)

### Community 83 - "Nav Menu Components"
Cohesion: 0.06
Nodes (25): navigationMenuTriggerStyle, delegatedProps, emits, forwarded, props, delegatedProps, emits, forwarded (+17 more)

### Community 84 - "Range Calendar Components"
Cohesion: 0.06
Nodes (24): delegatedProps, emits, forwarded, props, delegatedProps, forwardedProps, props, delegatedProps (+16 more)

### Community 85 - "Contact Detail View"
Cohesion: 0.08
Nodes (66): legacyFlowMeta, BackfillChatbotFlowGraph(), branchEdges(), buildEdges(), buildNodeConfig(), buttonsEdges(), extractCanvasPositions(), getStringFromJSONB() (+58 more)

### Community 86 - "Gowa Client Tests"
Cohesion: 0.17
Nodes (24): AutoButtonComponents(), BodyParamsToComponents(), BuildTemplateComponents(), ButtonURLParamsToComponents(), HeaderTextParamsComponent(), sortParamKeys(), T, TestBodyParamsToComponents_NamedParamsRetainLexicalOrder() (+16 more)

### Community 87 - "Agent Transfer Types"
Cohesion: 0.07
Nodes (33): TeamConfig, AIContextResponse, ChatbotFlowResponse, ChatbotSettingsResponse, KeywordResponse, KeywordRuleResponse, TeamMemberRequest, TeamMemberResponse (+25 more)

### Community 88 - "App Dependencies"
Cohesion: 0.06
Nodes (33): axios, chart.js, clsx, dependencies, axios, chart.js, clsx, lucide-vue-next (+25 more)

### Community 89 - "Custom Action Handlers"
Cohesion: 0.11
Nodes (21): ActionResult, CustomActionRequest, CustomActionResponse, ExecuteActionRequest, redirectToken, ToastConfig, ValidationError, boolToToastType() (+13 more)

### Community 90 - "Message Dispatchers"
Cohesion: 0.17
Nodes (15): MessageSendOptions, OutgoingMessageRequest, SendTemplateMessageRequest, APISendOptions(), DefaultSendOptions(), Account, Contact, Context (+7 more)

### Community 91 - "Chatbot Models"
Cohesion: 0.11
Nodes (21): containsEvent(), Contact, Organization, Team, Time, User, AgentAssignmentConfig, AgentTransfer (+13 more)

### Community 92 - "IVR & Calling Cache"
Cohesion: 0.19
Nodes (15): Contact, Organization, Team, Time, User, UUID, CallDirection, CallLog (+7 more)

### Community 93 - "Import/Export Dialog"
Cohesion: 0.03
Nodes (71): authStore, bottomSections, isCollapsed, isMobileMenuOpen, mainSections, navSections, route, router (+63 more)

### Community 94 - "Canned Response Handlers"
Cohesion: 0.23
Nodes (13): CannedResponseButton, CannedResponseRequest, CannedResponseResponse, buttonsToAuditString(), buttonsToJSONBArray(), cannedResponseAuditSnapshot(), cannedResponseToResponse(), CannedResponse (+5 more)

### Community 95 - "Chatbot Flow Execution"
Cohesion: 0.20
Nodes (14): chatNodeCtx, nodeOutcome, scheduleLogger, appendChatPath(), buttonsFromConfig(), evaluateConditionExpression(), evaluateTimingSchedule(), ChatbotFlow (+6 more)

### Community 96 - "Dialog Components"
Cohesion: 0.25
Nodes (20): cacheTestApp(), App, T, UUID, WhatsAppAccount, makeAccount(), TestDecryptAccountSecrets_DecryptsEncryptedValues(), TestDecryptAccountSecrets_LeavesLegacyPlaintextUnchanged() (+12 more)

### Community 97 - "Contacts View"
Cohesion: 0.03
Nodes (70): delegatedProps, props, emits, forwarded, props, props, props, CrudState (+62 more)

### Community 98 - "Custom Actions Tests"
Cohesion: 0.03
Nodes (47): props, props, props, props, AlertVariants, props, props, props (+39 more)

### Community 99 - "Gowa API Mocks"
Cohesion: 0.14
Nodes (24): mockGowaAPI, Header, Logger, Server, T, newMockGowaAPI(), nopLogger(), TestE2E_GOWAAccountResolvesGOWAProvider() (+16 more)

### Community 100 - "IVR Call Flows"
Cohesion: 0.20
Nodes (14): CallSession, IVRContext, getConfigInt(), Account, Manager, Duration, IVRFlowGraph, CancelFunc (+6 more)

### Community 101 - "Meta API Credentials"
Cohesion: 0.17
Nodes (4): Account, Context, Client, Recipient

### Community 102 - "Client Base Requests"
Cohesion: 0.23
Nodes (6): doJSON(), Account, Context, Recipient, T, Client

### Community 103 - "IVR Audio Bridging"
Cohesion: 0.12
Nodes (31): Agent Transfers, AI Contexts, config.toml Configuration, Contact Info Panel, Contact Metadata, Conversation Flows, Chatbot Sessions, Contacts (+23 more)

### Community 104 - "Flow Builder UI"
Cohesion: 0.12
Nodes (12): AudioBridge, callerLeg, CallRecorder, File, Mutex, TrackLocalStaticRTP, TrackRemote, WaitGroup (+4 more)

### Community 105 - "Database Models"
Cohesion: 0.08
Nodes (17): addComponent(), addOption(), addScreen(), componentsWithoutId, componentTypes, emit, FlowComponent, FlowScreen (+9 more)

### Community 107 - "GOWA Client Interfaces"
Cohesion: 0.11
Nodes (17): DeviceInfo, GOWA Devices, DeviceStatus, GOWA Instances, LoginResponse, WebhookConfig, gowaInstanceInput, truncateString() (+9 more)

### Community 108 - "Flow Interactive Preview"
Cohesion: 0.03
Nodes (64): useSearchPagination(), UseSearchPaginationOptions, formatDate(), apiKeysService, closeDeleteDialog(), columns, confirmDeleteRule(), deleteDialogOpen (+56 more)

### Community 109 - "Database Seeding Unit"
Cohesion: 0.11
Nodes (15): CallingConfig, ICEServerConfig, Account, Assigner, Manager, Client, Contact, DB (+7 more)

### Community 110 - "IVR Call Session"
Cohesion: 0.10
Nodes (5): TablePage, gotoCreateApiKey(), nameInput(), saveButton(), scope

### Community 111 - "Data Table E2E"
Cohesion: 0.08
Nodes (17): SheetVariants, emits, forwarded, props, props, delegatedProps, emits, forwarded (+9 more)

### Community 112 - "Sheet UI Component"
Cohesion: 0.08
Nodes (25): compilerOptions, allowImportingTsExtensions, isolatedModules, jsx, lib, module, moduleResolution, noEmit (+17 more)

### Community 113 - "TypeScript Compiler Config"
Cohesion: 0.39
Nodes (4): Account, Context, App, Request

### Community 114 - "Organization Settings Unit"
Cohesion: 0.11
Nodes (18): B.1 New package shape (mirrors `internal/assignment/` exactly), B.2 The API — pure decision + side-effect methods, no `*fastglue.Request`, B.3 What moves where, B.4 Migration sequence (file-by-file, test-gated), B.5 Risk mitigations (P0), C.1 Target structure (mirrors `ContactInfoPanel.vue` conventions), C.2 Props/emits contracts (tuple-form `defineEmits`, per `ContactInfoPanel.vue:73-76`), C.3 The two cross-region reach-throughs (preserved, lowest risk) (+10 more)

### Community 115 - "Account Management Service"
Cohesion: 0.06
Nodes (86): appOption, generateRefreshTokenWithJTI(), Duration, T, User, TestApp_GetWSToken_MissingOrgID(), TestApp_GetWSToken_MissingUserID(), TestApp_GetWSToken_Success() (+78 more)

### Community 116 - "Organization Service Handlers"
Cohesion: 0.23
Nodes (25): cleanAll(), DB, T, TestCreateDefaultAdmin_CreatesOrgAndUser(), TestCreateDefaultAdmin_Idempotent(), TestCreateDefaultAdmin_UsesExistingOrg(), TestSeedPermissionsAndRoles_CreatesAllDefaultPermissions(), TestSeedPermissionsAndRoles_Idempotent() (+17 more)

### Community 117 - "Templates Unit Tests"
Cohesion: 0.31
Nodes (17): Load(), T, TestLoad_AppliesDefaultsForMissingFields(), TestLoad_DevelopmentDoesNotForceSecureCookie(), TestLoad_EmptyConfigPathStillLoadsDefaults(), TestLoad_EnvMapsMultiWordKeys(), TestLoad_EnvVarsOverrideFile(), TestLoad_FileValuesOverrideDefaults() (+9 more)

### Community 118 - "WebRTC DTMF"
Cohesion: 0.12
Nodes (16): decodeDTMFEvent(), Manager, Logger, TrackRemote, sendDTMFDigit(), createOpusTrack(), Account, Manager (+8 more)

### Community 119 - "Conversation Notes UI"
Cohesion: 0.09
Nodes (14): chatScrollRef, currentApiStep, debugSteps, flowDataRef, graphRef, lastButtonMessage, props, {
  state,
  currentStep,
  isWaitingForInput,
  expectedInputType,
  canUndo,
  startSimulation,
  pauseSimulation,
  resumeSimulation,
  resetSimulation,
  processUserInput,
  processWhatsAppFlowCompletion,
  undo,
  stepForward,
  goToStep,
  apiMocker,
} (+6 more)

### Community 120 - "Chatbot Flows Page"
Cohesion: 0.05
Nodes (37): emits, handleChange(), props, organizationService, ChatbotFlow, columns, confirmDeleteFlow(), currentPage (+29 more)

### Community 121 - "Meta Webhook Processor"
Cohesion: 0.13
Nodes (12): IncomingTextMessage, TemplateStatusUpdate, WebhookPayload, WebhookStatus, WebhookStatusError, Number, App, RawMessage (+4 more)

### Community 122 - "Audit Logging System"
Cohesion: 0.32
Nodes (11): ComputeChanges(), extractSubField(), jsonEqual(), T, TestComputeChanges_CreateRecordsAllFields(), TestComputeChanges_DeleteRecordsAllFields(), TestComputeChanges_FlattenedField_NoDiff(), TestComputeChanges_FlattenedJSONBField() (+3 more)

### Community 123 - "Message DTOs"
Cohesion: 0.17
Nodes (13): Client, Context, DB, Logger, Template, UUID, WhatsAppAccount, New() (+5 more)

### Community 124 - "System Configuration"
Cohesion: 0.26
Nodes (12): Decrypt(), DecryptFields(), deriveKey(), Encrypt(), EncryptFields(), IsEncrypted(), T, TestDecrypt_LegacyUnencrypted() (+4 more)

### Community 125 - "General Settings E2E"
Cohesion: 0.19
Nodes (17): AIConfig, AppConfig, Config, CookieConfig, DatabaseConfig, DefaultAdminConfig, GOWAConfig, GOWAInstance (+9 more)

### Community 127 - "Drawer UI Component"
Cohesion: 0.32
Nodes (4): Organization, UUID, GowaInstance, GowaInstanceResponse

### Community 128 - "Media Storage Handlers"
Cohesion: 0.09
Nodes (15): emits, forwarded, props, delegatedProps, emits, forwardedProps, props, delegatedProps (+7 more)

### Community 129 - "Business Profile Mocks"
Cohesion: 0.23
Nodes (8): addFileToZip(), defaultZipEntryName(), App, Message, Request, UUID, uniqueZipName(), Writer

### Community 130 - "GOWA Webhook Utils"
Cohesion: 0.25
Nodes (22): fakeProfileServer, App, DB, Request, ResponseWriter, Server, T, UUID (+14 more)

### Community 131 - "Contacts Page E2E"
Cohesion: 0.07
Nodes (42): AckPayload, ChatPresencePayload, ConnectionPayload, EditedPayload, MediaField, MessagePayload, ReactionPayload, RevokedPayload (+34 more)

### Community 133 - "Message Buttons Editor"
Cohesion: 0.13
Nodes (13): { colorMode }, { colorMode, setColorMode }, ColorMode, getSystemTheme(), isDark, updateTheme(), useColorMode(), i18n (+5 more)

### Community 134 - "Core App Handlers"
Cohesion: 0.11
Nodes (18): addButton(), ButtonType, ctaCount, ctaLimitReached, effectiveMax, emit, emitButtons(), hasCtaButtons (+10 more)

### Community 135 - "Rate Limiting Middleware"
Cohesion: 0.16
Nodes (10): Assigner, CancelFunc, Client, DB, App, Logger, Request, UUID (+2 more)

### Community 136 - "Call Server Client"
Cohesion: 0.17
Nodes (19): withRateLimit(), FastRequestHandler, enforce(), extractClientIP(), Client, Duration, FastMiddleware, Logger (+11 more)

### Community 137 - "Analytics Fixtures"
Cohesion: 0.25
Nodes (21): Client, Mutex, Request, ResponseWriter, Server, T, newCallServer(), newCallTestClient() (+13 more)

### Community 139 - "Project Build Config"
Cohesion: 0.23
Nodes (7): NewAudioPlayer(), safeClose(), buildTransferVars(), Manager, Context, TrackRemote, UUID

### Community 140 - "Data Table Component"
Cohesion: 0.10
Nodes (19): entry, ignore, ignoreDependencies, project, $schema, autoprefixer, e2e/**, eslint-plugin-vue (+11 more)

### Community 141 - "Group Management"
Cohesion: 0.06
Nodes (26): emits, forwarded, props, emits, forwarded, props, props, props (+18 more)

### Community 142 - "Agent Transfer Dispatcher"
Cohesion: 0.20
Nodes (6): GroupInfo, GroupInfoFromLink, ParticipantRequest, ParticipantResult, Context, Client

### Community 143 - "Import Export Handlers"
Cohesion: 0.20
Nodes (11): Account, Context, Client, FlowAssetsResponse, FlowCreateRequest, FlowCreateResponse, FlowGetResponse, FlowJSON (+3 more)

### Community 144 - "Flow API Client"
Cohesion: 0.24
Nodes (4): generateSlug(), callingConfigDefault(), App, Request

### Community 145 - "Theme Switcher UI"
Cohesion: 0.12
Nodes (15): delegatedProps, emits, forwarded, props, ToggleGroupVariants, context, delegatedProps, forwardedProps (+7 more)

### Community 146 - "Toggle UI Component"
Cohesion: 0.10
Nodes (16): TemplateRequest, TemplateResponse, convertFromJSONBArray(), convertToJSONBArray(), diffButtons(), App, Request, Template (+8 more)

### Community 147 - "Template Management"
Cohesion: 0.15
Nodes (12): ensureGowaAccountOpts, gowaInstanceBundle, Client, Context, App, Request, UUID, WhatsAppAccount (+4 more)

### Community 148 - "GOWA Instance Devices"
Cohesion: 0.14
Nodes (14): ContactEventData, MessageEventData, OutboundWebhookPayload, TransferEventData, WebhookError, computeHMACSignature(), Context, App (+6 more)

### Community 149 - "Webhook Dispatcher"
Cohesion: 0.16
Nodes (10): currentLocale, currentLocaleName, { locale }, localeModules, localeNames, messages, MessageSchema, setLocale() (+2 more)

### Community 150 - "Organization Members Unit"
Cohesion: 0.24
Nodes (19): App, Message, Organization, T, TestUpdateMessageStatus_DeliveredBroadcastsViaWebSocket_NoErrorMessage(), TestUpdateMessageStatus_DeliveredUpdatesRecipient(), TestUpdateMessageStatus_FailedBroadcastsErrorMessageViaWebSocket(), TestUpdateMessageStatus_FailedUpdatesMessage() (+11 more)

### Community 151 - "Message Status Unit"
Cohesion: 0.14
Nodes (12): ContactFixtures, createRoleFixture(), createTeamFixture(), createUserFixture(), createWebhookFixture(), generateUniqueEmail(), generateUniqueName(), RoleFixtures (+4 more)

### Community 153 - "Card Grid Page E2E"
Cohesion: 0.22
Nodes (10): PermissionResponse, RoleRequest, RoleResponse, App, Permission, Request, UUID, roleAuditSnapshot() (+2 more)

### Community 154 - "Role Permissions Handlers"
Cohesion: 0.39
Nodes (10): RedisConfig, Client, NewRedis(), T, parseTestRedisConfig(), TestNewRedis_ConnectsWithDefaultOptions(), TestNewRedis_EmptyUsernameUsesDefaultUser(), TestNewRedis_InvalidHostReturnsError() (+2 more)

### Community 155 - "GOWA Webhook Processor"
Cohesion: 0.15
Nodes (18): Number, ParseMetaAPIError(), Account, BusinessProfileInput, CatalogInfo, CatalogListResponse, MetaAPIError, MetaAPIResponse (+10 more)

### Community 156 - "Meta API Types"
Cohesion: 0.29
Nodes (16): MigrationModel, AutoMigrate(), BackfillLastInboundAt(), CreateIndexes(), FixSystemRolePermissions(), getIndexes(), GetMigrationModels(), DB (+8 more)

### Community 157 - "Database Migration Models"
Cohesion: 0.11
Nodes (17): File limits, Header Variables, Media Headers (Image, Video, Document), Named Parameters, Overview, Positional Parameters, Publishing Templates, Syncing Templates (+9 more)

### Community 158 - "Template Documentation"
Cohesion: 0.19
Nodes (10): { error, formItemId, formDescriptionId, formMessageId }, { formDescriptionId }, props, id, props, { error, formItemId }, props, { name, formMessageId } (+2 more)

### Community 159 - "Form UI Component"
Cohesion: 0.11
Nodes (12): delegatedProps, emits, forwarded, props, props, delegatedProps, forwarded, props (+4 more)

### Community 160 - "Number Field Component"
Cohesion: 0.22
Nodes (8): CallPermissionReplyData, Contact, Context, App, Time, UUID, WhatsAppAccount, stickyCallKey()

### Community 161 - "Sticky Agent Calls"
Cohesion: 0.16
Nodes (11): TagRequest, TagResponse, App, Request, Tag, tagToResponse(), Organization, Time (+3 more)

### Community 162 - "Tag Handlers"
Cohesion: 0.16
Nodes (12): fakeMetaServer, createTestAccountForValidation(), App, DB, Mutex, Request, ResponseWriter, Server (+4 more)

### Community 163 - "Meta API Mocks"
Cohesion: 0.23
Nodes (12): AvailabilityRequest, ChangePasswordRequest, MyOrganizationResponse, PermissionInfo, RoleInfo, superAdminField, UserRequest, UserResponse (+4 more)

### Community 164 - "Config Loading Unit"
Cohesion: 0.12
Nodes (16): astro, @astrojs/starlight, dependencies, astro, @astrojs/starlight, sharp, name, scripts (+8 more)

### Community 165 - "Docs Build Config"
Cohesion: 0.13
Nodes (10): isArrayOfObjects(), isArrayOfPrimitives(), isObject(), isOpen, props, emits, forwarded, props (+2 more)

### Community 166 - "Collapsible Metadata UI"
Cohesion: 0.12
Nodes (12): delegatedProps, emits, forwarded, props, delegatedProps, forwardedProps, props, forwardedProps (+4 more)

### Community 167 - "Pin Input Component"
Cohesion: 0.11
Nodes (20): AUTO_RESOLVED_CONTEXT_TOKENS, closeMediaDialog(), extractButtonUrlParams(), extractCannedTokens(), extractCannedTokensFromResponse(), getMediaType(), handleCannedSelect(), handleTemplateWithParams() (+12 more)

### Community 168 - "Chat Input Helper"
Cohesion: 0.15
Nodes (12): compareEqual(), compareNumeric(), extractResponseMapping(), isTruthy(), TestCompareEqual(), TestCompareNumeric(), TestExtractResponseMapping_EmptyMapping(), TestExtractResponseMapping_MissingKey() (+4 more)

### Community 169 - "GOWA Webhook Payloads"
Cohesion: 0.13
Nodes (20): AppStatus, LoginWithCodeResponse, PasskeyStatus, WebAuthnAssertion, mockGowaDeviceAPI, authedGowaRequest(), createGowaAccountInDB(), App (+12 more)

### Community 170 - "GOWA Device Mocks"
Cohesion: 0.21
Nodes (11): Client, Context, Duration, Reader, NewS3Client(), T, TestNewS3Client_AcceptsStaticCredentials(), TestNewS3Client_RequiresBucket() (+3 more)

### Community 171 - "S3 Storage Client"
Cohesion: 0.18
Nodes (15): addField(), addSection(), assignedKeys, AvailableVariable, emit, PanelConfig, PanelField, PanelSection (+7 more)

### Community 172 - "Panel Config Editor"
Cohesion: 0.33
Nodes (8): AgentAnalyticsResponse, AgentAnalyticsSummary, AgentPerformanceStats, TrendPoint, App, Request, Time, UUID

### Community 173 - "Agent Analytics Service"
Cohesion: 0.19
Nodes (9): Logger, Time, UUID, MustParseUUID(), NewTestUUID(), RandomUUID(), TestLogger(), TimePtr() (+1 more)

### Community 174 - "IVR Flow Handlers"
Cohesion: 0.35
Nodes (15): auditLogsRole(), DB, T, Time, UUID, makeAuditLog(), TestApp_GetAuditLog_CrossOrgIsolation(), TestApp_GetAuditLog_NotFound() (+7 more)

### Community 175 - "Audit Logs Unit"
Cohesion: 0.24
Nodes (8): { t }, BURST_MEDIA_TYPES, isBurstMedia(), useMediaBurst(), UseMediaBurstOptions, UseMediaBurstResult, UseMediaExportResult, Message

### Community 177 - "Canned Responses E2E"
Cohesion: 0.32
Nodes (3): orgCallingSettings, Manager, UUID

### Community 178 - "Flow Debug Panel"
Cohesion: 0.06
Nodes (23): actionLabels, emit, groupsWithSelections, isSelected(), openGroups, props, sortedPermissionGroups, toggleGroup() (+15 more)

### Community 179 - "Accordion UI Component"
Cohesion: 0.20
Nodes (10): isLoggedIn(), login(), loginAsAgent(), loginAsManager(), logout(), TestUser, ADMIN_USER, cannedScope (+2 more)

### Community 180 - "Tabs UI Component"
Cohesion: 0.41
Nodes (4): App, Request, User, refreshTokenKey()

### Community 181 - "Media Export Utilities"
Cohesion: 0.32
Nodes (4): buildTeamResponse(), App, Request, Team

### Community 182 - "Template Engine"
Cohesion: 0.15
Nodes (21): AgentTransferResponse, agentTransferRow, AssignTransferRequest, CreateAgentTransferRequest, Time, createAnalyticsTestCampaign(), createTestAgentTransfer(), createTestChatbotSession() (+13 more)

### Community 183 - "Bulk Campaign Models"
Cohesion: 0.19
Nodes (9): Client, Context, Logger, JobHandler, JobType, Queue, RedisConsumer, RedisQueue (+1 more)

### Community 184 - "Flow Builder E2E"
Cohesion: 0.27
Nodes (9): BulkResult, ClaimOutcome, InviteOutcome, InviteResult, JoinOutcome, JoinResult, LeaveOutcome, LeaveResult (+1 more)

### Community 185 - "Permission Matrix UI"
Cohesion: 0.17
Nodes (13): IVRFlowRequest, UUID, diffIVRMenuNodes(), extractLabel(), DB, App, Request, menuHasGreetingText() (+5 more)

### Community 187 - "API Key Handlers"
Cohesion: 0.21
Nodes (8): APIKeyCreateResponse, APIKeyRequest, APIKeyResponse, generateAPIKey(), App, Request, Time, UUID

### Community 188 - "Collaborative Chat Actions"
Cohesion: 0.40
Nodes (4): delegatedProps, emits, forwarded, props

### Community 189 - "Contact Utilities"
Cohesion: 0.27
Nodes (11): FindContact(), GetOrCreateContact(), Contact, DB, UUID, T, TestGetOrCreateContact_CreatesNew(), TestGetOrCreateContact_FindsExisting() (+3 more)

### Community 190 - "Webhook Dispatcher Unit"
Cohesion: 0.33
Nodes (13): clearWebhookCache(), Client, T, UUID, TestApp_DispatchWebhook_CompletesSuccessfully(), TestApp_DispatchWebhook_ConcurrencyLimit(), TestApp_DispatchWebhook_EventFiltering(), TestApp_DispatchWebhook_HTTPTimeout() (+5 more)

### Community 191 - "Commerce Catalog API"
Cohesion: 0.38
Nodes (4): Account, Context, Client, ProductInput

### Community 192 - "Meta API Responses"
Cohesion: 0.15
Nodes (12): BusinessProfile, BusinessProfileResponse, CredentialsValidationResult, MediaURLResponse, ResumableUploadFinishResponse, ResumableUploadResponse, SharedWABAResponse, SubscribeAppResponse (+4 more)

### Community 193 - "Template API Client"
Cohesion: 0.29
Nodes (8): extractExamplesForComponent(), extractNamedExamplesForComponent(), extractNamedParamsInOrder(), Account, Context, Client, hasNamedParams(), TemplateSubmission

### Community 194 - "Audio Player"
Cohesion: 0.21
Nodes (4): AudioPlayer, Reader, TrackLocalStaticRTP, readOpusPackets()

### Community 195 - "Outbound Call Service"
Cohesion: 0.21
Nodes (8): Account, Manager, PeerConnection, TrackLocalStaticRTP, TrackRemote, UUID, durationSince(), Time

### Community 196 - "Conversation Notes Handlers"
Cohesion: 0.20
Nodes (9): formatValue(), processVariables(), TestFormatValue(), TestProcessVariables_ArrayIndex(), TestProcessVariables_MissingVariable(), TestProcessVariables_MultipleVariables(), TestProcessVariables_NestedPath(), TestProcessVariables_NoPlaceholders() (+1 more)

### Community 197 - "Dropdown Menu Component"
Cohesion: 0.15
Nodes (6): emit, props, props, props, props, props

### Community 198 - "GOWA App Auth"
Cohesion: 0.29
Nodes (7): CampaignRequest, CampaignResponse, RecipientRequest, getMimeTypeFromExtension(), Time, UUID, sanitizeFilename()

### Community 199 - "campaign-templates.spec.ts"
Cohesion: 0.40
Nodes (3): MOCK_ACCOUNTS, TEMPLATES_ALPHA, TEMPLATES_BETA

### Community 200 - "Chat Status"
Cohesion: 0.26
Nodes (4): Contact, Time, ChatStatus, Collaborator

### Community 201 - "Message Builder"
Cohesion: 0.17
Nodes (4): MessageBuilder, Message, MessageType, NewMessage()

### Community 202 - "Redis Config"
Cohesion: 0.22
Nodes (8): AuditLogResponse, DB, UUID, LogAudit(), Time, UUID, App, AuditAction

### Community 203 - "Tailwind Config"
Cohesion: 0.17
Nodes (11): aliases, components, utils, $schema, style, tailwind, baseColor, config (+3 more)

### Community 204 - "Build Scripts"
Cohesion: 0.17
Nodes (12): scripts, build, dev, format, lint, preview, test, test:debug (+4 more)

### Community 205 - "Time"
Cohesion: 0.31
Nodes (5): FastHTTPUpgrader, App, Request, newUpgrader(), ParseAllowedOrigins()

### Community 206 - "Gowa Backfill"
Cohesion: 0.14
Nodes (17): corsWrapper(), DB, RequestHandler, main(), printUsage(), resolveGowaCreds(), runServer(), runWorker() (+9 more)

### Community 207 - "Webhooks"
Cohesion: 0.22
Nodes (9): processTemplate(), TestProcessTemplate_ConditionalsOnly(), TestProcessTemplate_EmptyTemplate(), TestProcessTemplate_LoopsOnly(), TestProcessTemplate_Mixed(), TestProcessTemplate_NestedLoopsWithConditionals(), TestProcessTemplate_NilData(), TestProcessTemplate_NoPlaceholders() (+1 more)

### Community 208 - "Call Handling"
Cohesion: 0.45
Nodes (4): Account, Context, Recipient, Client

### Community 209 - "Flow Graph"
Cohesion: 0.29
Nodes (6): Edge, Graph, Graph[T], Node, Position, T

### Community 210 - "Account Tabs Spec"
Cohesion: 0.24
Nodes (9): CONTACT, contactEnvelope(), contactsEnvelope(), messagesEnvelope(), MULTI_ACCOUNT_MESSAGES, ONE_ORG_ACCOUNT, setupMockRoutes(), SINGLE_ACCOUNT_MESSAGES (+1 more)

### Community 211 - "API Mock Dialog"
Cohesion: 0.22
Nodes (10): delay, emit, handleClose(), handleSubmit(), parsedResponse, parseError, props, responseBody (+2 more)

### Community 212 - "Hover Card"
Cohesion: 0.18
Nodes (7): emits, forwarded, props, delegatedProps, forwardedProps, props, props

### Community 213 - "Resizable Panels"
Cohesion: 0.18
Nodes (8): delegatedProps, emits, forwarded, props, delegatedProps, emits, forwarded, props

### Community 214 - "User Profiles"
Cohesion: 0.29
Nodes (6): BusinessProfileInfo, Newsletter, PrivacySettings, UserAvatar, Context, Client

### Community 215 - "Flow Validation"
Cohesion: 0.35
Nodes (10): FlowRequest, FlowResponse, collectFormFieldNames(), collectFormFieldsPerScreen(), UUID, hasCompleteAction(), sanitizeComponentsWithPayload(), sanitizeID() (+2 more)

### Community 216 - "Template Builder"
Cohesion: 0.20
Nodes (3): TemplateBuilder, Template, NewTemplate()

### Community 218 - "WhatsApp Account Builder"
Cohesion: 0.20
Nodes (3): WhatsAppAccountBuilder, WhatsAppAccount, NewWhatsAppAccount()

### Community 219 - "Meta Registry Tests"
Cohesion: 0.27
Nodes (9): Logger, New(), T, TestClient_Capabilities_AllTrue(), TestErrNotSupported(), T, TestRegistry_ReturnsMetaForEmptyProviderType(), TestRegistry_ReturnsMetaForMetaAccount() (+1 more)

### Community 220 - "Message Sending"
Cohesion: 0.55
Nodes (5): parseMessageID(), Account, Context, Recipient, Client

### Community 221 - "Documentation"
Cohesion: 0.20
Nodes (10): Contributing Guide, Docker Compose, Keyword Rules Screenshot, Conversation Flows Screenshot, Chatbot Dark Screenshot, Chatbot Light Screenshot, Conversation Flow Dark Screenshot, Conversation Flow Light Screenshot (+2 more)

### Community 222 - "API Docs"
Cohesion: 0.20
Nodes (10): Campaigns Screenshot, Campaign Details Screenshot, Agent Analytics Dark Screenshot, Agent Analytics Light Screenshot, Accounts API Docs, Analytics API Docs, API Keys Docs, Authentication API Docs (+2 more)

### Community 223 - "Input Bar Component"
Cohesion: 0.24
Nodes (9): emit, handleKeydown(), handleSubmit(), inputPlaceholder, inputString, inputTypeAttr, inputValue, isEnabled (+1 more)

### Community 224 - "Radio Group"
Cohesion: 0.20
Nodes (7): delegatedProps, emits, forwarded, props, delegatedProps, forwardedProps, props

### Community 225 - "useInfiniteScroll.ts"
Cohesion: 0.33
Nodes (4): AgentTransferBuilder, AgentTransfer, UUID, NewAgentTransfer()

### Community 226 - "Node Config"
Cohesion: 0.20
Nodes (9): compilerOptions, allowSyntheticDefaultImports, composite, module, moduleResolution, skipLibCheck, strict, include (+1 more)

### Community 227 - "Chat Fetching"
Cohesion: 0.31
Nodes (7): Chat, ChatMessage, ChatMessagesOptions, ListChatsOptions, buildChatsPath(), Context, Client

### Community 228 - "Gowa Device Pairing"
Cohesion: 0.27
Nodes (6): gowaAccount, Client, App, Request, UUID, WhatsAppAccount

### Community 229 - "Contact Builder"
Cohesion: 0.22
Nodes (3): ContactBuilder, Contact, NewContact()

### Community 230 - "CRUD Form Dialog"
Cohesion: 0.17
Nodes (12): CatalogProductRequest, CatalogProductResponse, CatalogRequest, CatalogResponse, SyncCatalogsRequest, catalogToResponse(), Catalog, CatalogProduct (+4 more)

### Community 231 - "ConfirmDialog.vue"
Cohesion: 0.47
Nodes (5): emit, handleCancel(), handleConfirm(), open, props

### Community 232 - "Checkbox.vue"
Cohesion: 0.28
Nodes (4): CampaignBuilder, defaultPasswordHash(), NewCampaign(), NewUser()

### Community 233 - "CannedResponse"
Cohesion: 0.29
Nodes (7): CookieAuthResponse, LoginRequest, LogoutRequest, RefreshRequest, RegisterRequest, SwitchOrgRequest, UUID

### Community 234 - "SelectContent.vue"
Cohesion: 0.40
Nodes (4): delegatedProps, emits, forwarded, props

### Community 237 - "Model Tests"
Cohesion: 0.43
Nodes (7): T, TestJSONB_Scan(), TestJSONB_Value(), TestJSONBArray_Scan(), TestJSONBArray_Value(), TestStringArray_Scan(), TestStringArray_Value()

### Community 238 - "Organization Builder"
Cohesion: 0.03
Nodes (74): categoryLabels, emit, filteredResponses, groupedResponses, internalOpen, isLoading, isOpen, props (+66 more)

### Community 239 - "Team Builder"
Cohesion: 0.29
Nodes (3): OrganizationBuilder, Organization, NewOrganization()

### Community 240 - "TestClient_ExchangeCodeForToken"
Cohesion: 0.60
Nodes (4): T, TestClient_ExchangeCodeForToken(), TestClient_GetPhoneNumberInfo(), TestClient_RegisterPhoneNumber()

### Community 241 - "Frontend Config"
Cohesion: 0.43
Nodes (5): CLEANUP_STATEMENTS, cleanupE2EData(), CreateUser, extractCSRFToken(), globalSetup()

### Community 242 - "Dashboard Actions"
Cohesion: 0.29
Nodes (6): name, overrides, minimatch, private, type, version

### Community 243 - "Device Connection"
Cohesion: 0.36
Nodes (5): CatalogProduct, Organization, UUID, Catalog, CatalogProduct

### Community 244 - "TestClient_Capabilities_AllTrue"
Cohesion: 0.29
Nodes (3): TeamBuilder, Team, NewTeam()

### Community 245 - "Role Models"
Cohesion: 0.29
Nodes (5): Contact, Organization, User, UUID, ConversationNote

### Community 246 - "CSRFProtection"
Cohesion: 0.33
Nodes (4): testServerTransport, Request, Response, ResponseWriter

### Community 247 - "chart.js"
Cohesion: 0.40
Nodes (4): delegatedProps, emits, forwarded, props

### Community 248 - "Auth Cookies"
Cohesion: 0.40
Nodes (3): generateCSRFToken(), App, Request

### Community 249 - "Template Tests"
Cohesion: 0.33
Nodes (5): createTestTemplate(), App, T, Template, UUID

### Community 250 - "RequestHandler"
Cohesion: 0.40
Nodes (4): delegatedProps, emits, forwarded, props

### Community 251 - "HTTP Callbacks"
Cohesion: 0.50
Nodes (4): HTTPCallbackResult, executeHTTPCallback(), Duration, interpolateTemplate()

### Community 252 - "Transfer Callbacks"
Cohesion: 0.43
Nodes (5): IVRNodeType, TransferCallbacks, TransferHTTPCallback, parseOneCallback(), parseTransferCallbacks()

### Community 253 - "Brand Assets"
Cohesion: 0.40
Nodes (5): Dark Mode Logo, Light Mode Logo, Serena Project Configuration, Whatomate Branding, Whatomate Project

### Community 258 - "Type Declarations"
Cohesion: 0.40
Nodes (4): ImportMeta, ImportMetaEnv, *.vue, vue3-emoji-picker/css

### Community 260 - "Media Redownload"
Cohesion: 0.40
Nodes (3): classifyRedownloadError(), App, Request

### Community 261 - "Embedded App"
Cohesion: 0.27
Nodes (9): App, Client, Logger, setupRoutes(), Fastglue, RequestHandler, Handler(), IsEmbedded() (+1 more)

### Community 264 - "Flows Documentation"
Cohesion: 0.50
Nodes (4): WhatsApp Flows Documentation, WhatsApp Flows REST API Endpoint, Meta WhatsApp Business API, WhatsApp Flows Feature

### Community 265 - "GOWA QA Reports"
Cohesion: 0.67
Nodes (4): GOWA Instances Device Management Plan, GOWA OpenAPI Spec v9.0.0, Full Feature Test Report Round 2, QA Feature Test Report Round 1

### Community 266 - "Message Parsers"
Cohesion: 0.50
Nodes (4): extractAgentFromLegacy(), getMessageContent(), getSystemMessageText(), SYSTEM_MESSAGE_TYPES

### Community 268 - "Connection Tests"
Cohesion: 0.67
Nodes (3): T, TestConnectionPayload_Unmarshal(), TestWebhookPayload_ConnectionEvent()

### Community 269 - "Media Upload"
Cohesion: 0.50
Nodes (3): Account, Context, Client

### Community 270 - "Dashboard Screenshots"
Cohesion: 0.67
Nodes (3): Dashboard Dark Screenshot, Dashboard Light Screenshot, Docs Introduction

### Community 277 - "Tab Implementation Plan"
Cohesion: 1.00
Nodes (3): Pending/Me Tabs Implementation Plan, Pending/Me Tabs Task Checklist, Pending/Me Tabs Feature

### Community 305 - "piper_test.go"
Cohesion: 0.35
Nodes (13): fileExists(), sha256Short(), fakeBin(), fakeOpusenc(), fakePiper(), T, TestPiperTTS_Generate_CachesExistingFile(), TestPiperTTS_Generate_CreatesFile() (+5 more)

### Community 345 - "LogAudit"
Cohesion: 0.34
Nodes (9): Service, GetUserName(), Contact, Context, DB, Logger, UUID, WSMessage (+1 more)

### Community 350 - "organization.go"
Cohesion: 0.23
Nodes (10): AddMemberRequest, CreateOrganizationRequest, MemberResponse, OrganizationResponse, OrganizationSettings, UpdateMemberRoleRequest, callingSettingsSnapshot(), generalSettingsSnapshot() (+2 more)

### Community 352 - "factories.go"
Cohesion: 0.23
Nodes (9): Message, Organization, Template, Time, User, UUID, BulkMessageCampaign, BulkMessageRecipient (+1 more)

### Community 357 - "PaginationControls.vue"
Cohesion: 0.32
Nodes (6): emit, goToPage(), pageNumbers, paginationInfo, props, getPageNumbers()

### Community 383 - "Slider.vue"
Cohesion: 0.33
Nodes (4): delegatedProps, emits, forwarded, props

### Community 394 - "Spinner.vue"
Cohesion: 0.40
Nodes (3): props, sizeClass, SpinnerProps

## Ambiguous Edges - Review These
- `Canned Responses API Docs` → `Templates Screenshot`  [AMBIGUOUS]
  docs/public/images/11-templates.png · relation: conceptually_related_to

## Knowledge Gaps
- **2167 isolated node(s):** `name`, `type`, `version`, `dev`, `start` (+2162 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **63 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **What is the exact relationship between `Canned Responses API Docs` and `Templates Screenshot`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `page` connect `Accounts Page Object` to `Framework Auth Utils`, `Media Burst Dialog`, `Chatbot Flows Page Object`, `Agent Transfers E2E`, `Agent Transfer Session`, `Base Page Objects`, `IVR Call Session`, `Piper TTS Unit`, `Page Object Helpers`, `Account Tabs Spec`, `Accordion UI Component`, `Audit Log & Detail Panels`, `Campaigns Page Object`, `Test Fixtures`, `Device Management Handlers`, `Internationalization`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `newTestApp()` connect `Account Management Service` to `GOWA Webhook Utils`, `Accounts API Tests`, `Organization Switcher`, `AI Context Services`, `API Keys Tests`, `Main Server Config`, `Auth Flow Tests`, `Radix UI Components`, `Tag Handlers`, `GOWA Webhook Payloads`, `Conversation Notes Tests`, `IVR Flow Handlers`, `Campaigns Tests`, `Message Handlers Tests`, `Keywords View`, `Template Engine`, `Webhook Dispatcher Unit`, `Queue Tests`, `PubSub System`, `Build Config`, `Organization Service Handlers`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `JSONB` connect `Permission Matrix UI` to `Chatbot Graph Tests`, `Auth & Token Gaps Tests`, `Template Picker`, `Meta API Mocks`, `Database Models`, `Widget Data Services`, `API Envelope Helpers`, `Campaign & Flow Handlers`, `Conversation Notes Handlers`, `Template Utils`, `Message Builder`, `Chatbot Processor Tests`, `Contact Detail View`, `Agent Transfer Types`, `Custom Action Handlers`, `LogAudit`, `Chatbot Models`, `Message Dispatchers`, `IVR & Calling Cache`, `organization.go`, `Chatbot Flow Execution`, `factories.go`, `Team Builder`, `Account Management Service`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Are the 604 inferred relationships involving `CreateTestOrganization()` (e.g. with `TestAssignToTeam_LoadBalanced_AllUnavailableReturnsNil()` and `TestAssignToTeam_LoadBalanced_PicksLowestLoad()`) actually correct?**
  _`CreateTestOrganization()` has 604 INFERRED edges - model-reasoned connections that need verification._
- **Are the 577 inferred relationships involving `newTestApp()` (e.g. with `TestApp_CreateAccount_Success()` and `TestApp_CreateAccount_Unauthorized()`) actually correct?**
  _`newTestApp()` has 577 INFERRED edges - model-reasoned connections that need verification._
- **Are the 532 inferred relationships involving `GetResponseStatusCode()` (e.g. with `TestApp_CreateAccount_Success()` and `TestApp_CreateAccount_Unauthorized()`) actually correct?**
  _`GetResponseStatusCode()` has 532 INFERRED edges - model-reasoned connections that need verification._