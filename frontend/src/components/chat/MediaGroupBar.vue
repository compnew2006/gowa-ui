 ...
 */
async function fetchMediaBlob(message: Message): Promise<{ blob: Blob; filename: string }> {
  ...
  if (cachedUrl) {
    ...
    if (!resp.ok) throw new Error(`Failed to fetch media for ${message.id}: ${resp.status}`)
    blob = await resp.blob()
  }

  // Determine filename
  let filename = message.media_filename || `${message.message_type}_${message.id}`
  // Ensure extension
  ...
  try {
    ...
    if (eligibleMessages.length === 1) {
      ...
      for (let i = 0; i < eligibleMessages.length; i++) {
        ...
        downloadProgress.value = Math.round(((i + 1) / eligibleMessages.length) * 80)
      }
...
<template>
  ...
  <div v-if="variant === 'start'" class="media-group-bar media-group-bar--start">
  ...
  <div v-else class="media-group-bar media-group-bar--end">
    ...
    >
      ...
      <CheckCircle v-else-if="downloadDone" class="h-3.5 w-3.5 text-emerald-400" />
