...
func (a *App) CreateAgentTransfer(r *fastglue.Request) error {
	...
	if agentID != nil {
		a.DB.Model(contact).Updates(chatAssignmentUpdates(agentID))
	}
...
func (a *App) PickNextTransfer(r *fastglue.Request) error {
	...
	pickedBy := userID
	if err := tx.Model(&models.Contact{}).Where("id = ?", transfer.ContactID).Updates(chatAssignmentUpdates(&pickedBy)).Error; err != nil {
		tx.Rollback()
...
func (a *App) ReturnAgentTransfersToQueue(userID, orgID uuid.UUID) int {
	...
	for i := range transfers {
		...
		if transfer.ContactID != uuid.Nil {
			a.DB.Model(&models.Contact{}).Where("id = ?", transfer.ContactID).Updates(chatAssignmentUpdates(nil))
		}
