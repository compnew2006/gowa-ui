...
$page_title = "استخراج مجموعات الواتساب | Kingmaster";
      ...
      <table style="width:100%; border-collapse:collapse;">
        <thead>
          <tr style="background:#0f172a;">
            ...
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:right;">عدد الاعضاء</th>
          </tr>
        </thead>
...
document.addEventListener('DOMContentLoaded', function() {
    loadAccounts();
    loadIntervals();
    loadCampaigns('Extract Groups WA');
    loadContent();
});
...
function saveCampaign(id) {
     ...
     window.location.href = "api/save_data.php?camp_id=" + id+"&tool=Extract-Groups-WA";
}
...
function loadContacts() {
  ...
  fetch('api/wa_groups.php?' + params.toString())
    .then(r=>r.json())
    .then(j=>{
...
function renderContactsTable() {
  ...
  tb.innerHTML = contactsState.data.map((row, idx) => {
    const id = `${contactsState.page}-${idx}-${row.gb_id}`;
    const checked = contactsState.selected.has(id) ? 'checked' : '';
    return `
      <tr>
        <td style="padding:8px; text-align:center;"><input type="checkbox" data-id="${id}" data-phone="${escapeHtml(row.gb_id||'')}" data-name="${escapeHtml((row.name||row.participantsCount||'').toString())}" onchange="toggleSelectContact(this)" ${checked}></td>
        <td style="padding:8px;">${escapeHtml(row.name || '')}</td>
        <td style="padding:8px; direction:ltr;">${escapeHtml(row.gb_id || '')}</td>
        <td style="padding:8px;">${escapeHtml(row.participantsCount || '0')}</td>
      </tr>`;
  }).join('');
