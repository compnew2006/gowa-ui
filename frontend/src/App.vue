...
<template>
  <TooltipProvider>
    <div class="min-h-screen bg-background font-sans antialiased">
      <div v-if="hasError" class="fixed inset-0 z-50 flex items-center justify-center bg-background p-6">
        <div class="max-w-md w-full p-8 text-center border rounded-xl bg-card shadow-2xl space-y-6">
          ...
          <div class="space-y-2">
            ...
            <p class="text-muted-foreground">The application encountered a fatal error and could not continue.</p>
          </div>
          <div v-if="errorInfo" class="p-3 bg-muted rounded text-[11px] font-mono text-left overflow-auto max-h-32 opacity-70">
            {{ errorInfo }}
          </div>
