...
const filteredPendingChats = computed(() => {
  ...
  return contactsStore.pendingChats.filter(chat =>
...
function openPendingChat(chat: Contact) {
  router.push({ name: 'chat-conversation', params: { contactId: chat.id }, query: { tab: 'pending' } })
}
  ...
  <div class="flex flex-col h-full bg-[#0a0a0b] light:bg-gray-50">
    ...
    <div class="p-6 space-y-4">
      <div class="flex items-center gap-2">
        ...
        />
        <Button variant="outline" :disabled="isLoading" @click="loadPendingChats">
      ...
      <div class="rounded-xl border border-white/[0.08] bg-white/[0.02] light:bg-white light:border-gray-200 overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-if="isLoading">
              <td colspan="2" class="px-4 py-8 text-center text-white/50 light:text-gray-500">Loading pending chats...</td>
            </tr>
            <tr
            ...
            <tr v-if="!isLoading && filteredPendingChats.length === 0">
              <td colspan="2" class="px-4 py-8 text-center text-white/50 light:text-gray-500">No pending chats found.</td>
            </tr>
          </tbody>
        </table>
      </div>
