...
func (a *App) CreateCatalog(r *fastglue.Request) error {
	...
	metaCatalogID, err := provider.CreateCatalog(ctx, waAccount, req.Name)
	if err != nil {
		if err == whatsapp.ErrNotSupported {
			return r.SendErrorEnvelope(fasthttp.StatusBadRequest, "Catalogs are not supported for this account type", nil, "")
		}
		a.Log.Error("Failed to create catalog", "error", err, "provider", account.ProviderType)
...
func (a *App) DeleteCatalog(r *fastglue.Request) error {
	...
	if err := provider.DeleteCatalog(ctx, waAccount, catalog.MetaCatalogID); err != nil {
		if err == whatsapp.ErrNotSupported {
			return r.SendErrorEnvelope(fasthttp.StatusBadRequest, "Catalogs are not supported for this account type", nil, "")
		}
		a.Log.Error("Failed to delete catalog", "error", err, "provider", account.ProviderType)
		// Continue with local deletion even if the provider fails
	}
...
func (a *App) SyncCatalogs(r *fastglue.Request) error {
	...
	metaCatalogs, err := provider.ListCatalogs(ctx, waAccount)
	if err != nil {
		if err == whatsapp.ErrNotSupported {
			return r.SendErrorEnvelope(fasthttp.StatusBadRequest, "Catalogs are not supported for this account type", nil, "")
		}
		a.Log.Error("Failed to fetch catalogs", "error", err, "provider", account.ProviderType)
...
func (a *App) CreateCatalogProduct(r *fastglue.Request) error {
	...
	metaProductID, err := provider.CreateProduct(ctx, waAccount, catalog.MetaCatalogID, productInput)
	if err != nil {
		if err == whatsapp.ErrNotSupported {
			return r.SendErrorEnvelope(fasthttp.StatusBadRequest, "Catalogs are not supported for this account type", nil, "")
		}
		a.Log.Error("Failed to create product", "error", err, "provider", account.ProviderType)
...
func (a *App) UpdateCatalogProduct(r *fastglue.Request) error {
	...
	if err := provider.UpdateProduct(ctx, waAccount, product.MetaProductID, productInput); err != nil {
		if err == whatsapp.ErrNotSupported {
			return r.SendErrorEnvelope(fasthttp.StatusBadRequest, "Catalogs are not supported for this account type", nil, "")
		}
		a.Log.Error("Failed to update product", "error", err, "provider", account.ProviderType)
...
func (a *App) DeleteCatalogProduct(r *fastglue.Request) error {
	...
	if err := provider.DeleteProduct(ctx, waAccount, product.MetaProductID); err != nil {
		if err == whatsapp.ErrNotSupported {
			return r.SendErrorEnvelope(fasthttp.StatusBadRequest, "Catalogs are not supported for this account type", nil, "")
		}
		a.Log.Error("Failed to delete product", "error", err, "provider", account.ProviderType)
