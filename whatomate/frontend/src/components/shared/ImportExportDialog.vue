...
async function loadExportConfig() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        error,
        t("common.failedLoad", { resource: t("common.configuration") }),
      ),
    );
  } finally {
...
async function handleImport() {
  ...
  try {
    ...
    if (result.errors > 0) {
      toast.warning(
        t("importExport.importPartialErrors", { errors: result.errors }),
      );
    }
  } catch (error) {
...
const allColumns = computed(() => [
  ...importRequiredColumns.value,
  ...importOptionalColumns.value,
]);

function downloadSampleCsv() {
  const headers = allColumns.value
    .map((c) => translateColumnLabel(c))
    .join(",");
...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="sm:max-w-lg">
      <DialogHeader>
        <DialogTitle>{{
          $t("importExport.title", { resource: tableLabel })
        }}</DialogTitle>
        <DialogDescription>{{
          $t("importExport.description", { resource: tableLabel.toLowerCase() })
        }}</DialogDescription>
      </DialogHeader>
                ...
                <div class="space-y-2">
                  ...
                  >
                    ...
                    <Label
                      ...
                      >{{ translateColumnLabel(col) }}</Label
                    >
                  </div>
                </div>
              </ScrollArea>
          ...
          <template v-else>
            ...
            <div class="space-y-2">
              ...
              <div v-if="importOptionalColumns.length > 0" class="text-sm">
                <span class="font-medium"
                  >{{ $t("importExport.optionalColumns") }}:</span
                >
                <span class="text-muted-foreground ml-1">
                  {{
            ...
            >
              ...
              <div class="text-sm text-muted-foreground space-y-1">
                ...
                <p>
                  {{ $t("importExport.updated") }}: {{ importResult.updated }}
                </p>
                <p v-if="importResult.skipped > 0">
                  {{ $t("importExport.skipped") }}: {{ importResult.skipped }}
                </p>
                <p v-if="importResult.errors > 0" class="text-amber-500">
                  {{ $t("importExport.errors") }}: {{ importResult.errors }}
                </p>
              </div>
              <ScrollArea
                v-if="importResult.messages && importResult.messages.length > 0"
                class="h-24 text-xs"
              >
                <div
                  v-for="(msg, i) in importResult.messages"
                  :key="i"
