...
func TestApp_RefreshFacebookAccountPages_UpdatesMetadataAndTokens(t *testing.T) {
	...
	org := testutil.CreateTestOrganization(t, app.DB)
	user := createAccountsAuthorizedUser(t, app, org.ID)
	account := createOAuthFacebookAccount(t, app, org.ID, user.ID, []map[string]any{{
		"id":        "page-1",
		"name":      "Stale Page Name",
		"connected": true,
	...
	saved := reloadFacebookOAuthAccount(t, app, account.ID)
	assert.Equal(t, float64(1), saved.Data["page_count"])
	pages := facebookAccountPagesFromData(t, saved)
	require.Len(t, pages, 1)
	assert.Equal(t, "page-1", pages[0]["id"])
	assert.Equal(t, "Managed Page", pages[0]["name"])
	assert.True(t, pages[0]["connected"].(bool))
	assert.Equal(t, map[string]string{"page-1": "page-token-1"}, decryptFacebookPageTokens(t, app, saved.PageTokens))
}

func TestApp_ConnectFacebookAccountPage_RestoresSelectedPageToken(t *testing.T) {
	app, server := newFacebookOAuthCallbackTestApp(t, facebookOAuthGraphScenario{})
	defer server.Close()
...
func TestApp_DisconnectFacebookAccountPage_RemovesOnlySelectedToken(t *testing.T) {
	app := newTestApp(t)
	org := testutil.CreateTestOrganization(t, app.DB)
	user := createAccountsAuthorizedUser(t, app, org.ID)
	account := createOAuthFacebookAccount(t, app, org.ID, user.ID, []map[string]any{{
	...
	saved := reloadFacebookOAuthAccount(t, app, account.ID)
	assert.Equal(t, map[string]string{"page-2": "page-token-2"}, decryptFacebookPageTokens(t, app, saved.PageTokens))
	pages := facebookAccountPagesFromData(t, saved)
	require.Len(t, pages, 2)
	assert.False(t, pages[0]["connected"].(bool))
	assert.True(t, pages[1]["connected"].(bool))
}

func TestApp_RemoveFacebookAccountPage_RemovesMetadataAndToken(t *testing.T) {
	app := newTestApp(t)
	org := testutil.CreateTestOrganization(t, app.DB)
	user := createAccountsAuthorizedUser(t, app, org.ID)
	account := createOAuthFacebookAccount(t, app, org.ID, user.ID, []map[string]any{{
...
func TestApp_DisconnectFacebookAccountPage_CrossOrgReturnsNotFound(t *testing.T) {
	app := newTestApp(t)
	ownerOrg := testutil.CreateTestOrganization(t, app.DB)
	otherOrg := testutil.CreateTestOrganization(t, app.DB)
	owner := createAccountsAuthorizedUser(t, app, ownerOrg.ID)
	otherUser := createAccountsAuthorizedUser(t, app, otherOrg.ID)
	account := createOAuthFacebookAccount(t, app, ownerOrg.ID, owner.ID, []map[string]any{{
		"id":        "page-1",
		"name":      "Managed Page",
		"connected": true,
...
func newFacebookOAuthCallbackTestApp(t *testing.T, scenario facebookOAuthGraphScenario) (*handlers.App, *httptest.Server) {
	t.Helper()
...
func assertFacebookAccountCount(t *testing.T, app *handlers.App, expected int64) {
	t.Helper()
	var count int64
...
func createOAuthFacebookAccount(t *testing.T, app *handlers.App, orgID, userID uuid.UUID, pages []map[string]any, pageTokens map[string]string) models.FacebookAccount {
	t.Helper()
...
func reloadFacebookOAuthAccount(t *testing.T, app *handlers.App, id uuid.UUID) models.FacebookAccount {
	t.Helper()
	var account models.FacebookAccount
...
func decryptFacebookPageTokens(t *testing.T, app *handlers.App, encrypted string) map[string]string {
	t.Helper()
