...
onMounted(async () => {
  ...
  if (rememberedRaw) {
    try {
      const remembered = JSON.parse(rememberedRaw) as Partial<RememberedCredentials>
      if (typeof remembered.email === 'string' && typeof remembered.password === 'string') {
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (rememberCredentials.value) {
      ...
      localStorage.setItem(REMEMBER_CREDENTIALS_KEY, JSON.stringify(rememberedCredentials))
    } else {
      localStorage.removeItem(REMEMBER_CREDENTIALS_KEY)
    }
      ...
      <form @submit.prevent="handleLogin">
        <div class="px-8 pb-4 space-y-4">
          ...
          <div class="flex items-center space-x-2">
            ...
            />
            <Label for="remember-credentials" class="cursor-pointer text-sm font-normal text-white/70 light:text-gray-700">
              {{ $t('auth.rememberMe') }}
            </Label>
          </div>
