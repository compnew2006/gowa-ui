...
import { Archive, Check, ChevronsUpDown, Loader2, RotateCw } from 'lucide-vue-next'
...
const totalPages = computed(() => Math.max(1, Math.ceil(totalClosedChats.value / pageSizeValue.value)))
...
const pageEnd = computed(() => {
  if (totalClosedChats.value === 0) return 0
  return Math.min(currentPage.value * pageSizeValue.value, totalClosedChats.value)
})
...
async function loadClosedChats() {
  ...
  try {
    const response = await contactsStore.fetchClosedChats({
      ...
      closed_by: selectedAgentId.value !== 'all' ? selectedAgentId.value : undefined,
    ...
    if (response.chats.length === 0 && response.total > 0 && currentPage.value > 1) {
      currentPage.value -= 1
...
watch([searchQuery, selectedAgentId, closedFrom, closedTo], () => {
  debouncedFilterLoad()
})
    ...
    <div class="p-6 space-y-4">
      <div class="grid gap-2 md:grid-cols-2 xl:grid-cols-6">
        ...
        <Popover v-model:open="agentComboboxOpen">
          ...
          <PopoverContent class="w-[var(--radix-popover-trigger-width)] p-0">
            <Command>
              <CommandInput :placeholder="$t('closedChats.searchAgent')" data-testid="closed-chats-agent-search" />
              <CommandList>
                <CommandEmpty>{{ $t('closedChats.noAgentFound') }}</CommandEmpty>
                <CommandGroup>
                  <CommandItem
                    value="all"
                  ...
                  >
                    ...
                    <div class="flex min-w-0 flex-col">
                      ...
                      <span class="truncate text-xs text-muted-foreground">{{ agent.id }}</span>
                    </div>
                  </CommandItem>
                </CommandGroup>
              </CommandList>
      ...
      <div class="flex items-center justify-between gap-2">
        <p class="text-xs text-white/60 light:text-gray-500">
          {{ $t('closedChats.resultsSummary', { start: pageStart, end: pageEnd, total: totalClosedChats }) }}
        </p>
      ...
      <div class="flex items-center justify-end gap-2">
        ...
        <span class="text-xs text-white/70 light:text-gray-600" data-testid="closed-chats-page-label">
          {{ $t('closedChats.pageLabel', { page: currentPage, total: totalPages }) }}
        </span>
        <Button
          variant="outline"
          size="sm"
          data-testid="closed-chats-next-page"
          :disabled="isLoading || currentPage >= totalPages || totalClosedChats === 0"
          @click="goToNextPage"
        >
          {{ $t('closedChats.next') }}
        </Button>
