...
![Campaigns](/Gowa-UI/images/13-campaigns.png)

## Overview

Campaigns allow you to send bulk WhatsApp messages to multiple contacts at once. Whether you're sending promotional offers, updates, or notifications, Gowa-UI handles the delivery efficiently while respecting WhatsApp's rate limits.
...
Campaigns respect each account's **messages-per-minute budget** (Settings → Accounts → Campaign Send Pacing, or the server-wide default). The worker spaces sends to stay inside the budget, protecting the number from WhatsApp ban waves. Recipients are never dropped — only delayed (at most 10 minutes past their claim) when the budget is exhausted.
...
![Campaign Details](/Gowa-UI/images/14-campaign-details.png)
