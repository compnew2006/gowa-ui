...
$page_title = "استخراج بيانات | Kingmaster";
...
function saveCampaign(id) {
     ...
     window.location.href = "api/save_data.php?camp_id=" + id+"&tool=ddbs";
}
...
function loadContacts() {
  ...
  fetch('api/ss.php?' + params.toString())
    .then(r=>r.json())
    .then(j=>{
...
function renderContactsTable() {
  ...
  tb.innerHTML = contactsState.data.map((row, idx) => {
    ...
    return `
      <tr>
        <td style="padding:8px; text-align:center;"><input type="checkbox" data-id="${id}" data-phone="${escapeHtml(row.phone||'')}" data-name="${escapeHtml((row.fb_id||'').toString())}" onchange="toggleSelectContact(this)" ${checked}></td>
        <td style="padding:8px; direction:rtl;">${escapeHtml(row.phone || '')}</td>
        <td style="padding:8px;">${escapeHtml(row.fb_id || '')}</td>
      </tr>`;
  }).join('');
