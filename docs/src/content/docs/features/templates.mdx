...
Templates are reusable message blueprints that let your team send consistent notifications, updates, and OTP messages. They are stored per organization and managed through the [Templates API](/api-reference/templates) — there is no external approval workflow.
