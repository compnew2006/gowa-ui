...
onMounted(async () => {
  // Refresh the user object from the server first. The localStorage snapshot
  // may be stale (e.g. is_super_admin flipped server-side since last login),
  // and acting on the stale value causes a 403 from /organizations. This await
  // ensures the super-admin check below reflects the *current* server state.
  // refreshUserData swallows errors; the interceptor handles 401 globally.
