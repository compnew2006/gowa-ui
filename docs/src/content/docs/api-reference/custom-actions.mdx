...
<Aside type="caution">
  Managing custom actions requires the `custom_actions:read`, `custom_actions:write`, or `custom_actions:delete` permissions respectively. Running an action requires `custom_actions:execute`. Listing actions requires either `custom_actions:read` or `custom_actions:execute`, so agents can see the action buttons in chat.
</Aside>
