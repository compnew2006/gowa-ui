...
const (
	...
	// DeadLetterStreamName stores jobs that exceeded retry limits or are permanently invalid.
...
func (c *RedisConsumer) Consume(ctx context.Context, handler JobHandler) error {
	...
	for {
		...
		if time.Since(lastPendingClaim) >= PendingClaimInterval {
			if err := c.claimPendingMessages(ctx, handler); err != nil {
				c.log.Warn("Failed periodic pending-claim cycle", "error", err)
			}
			lastPendingClaim = time.Now()
		}
				...
				if err := c.processMessage(ctx, msg, handler); err != nil {
					...
					if isPermanentProcessError(err) {
						if dlqErr := c.moveToDeadLetter(ctx, msg, err.Error(), 0); dlqErr != nil {
							c.log.Error("Failed to move permanently invalid message to DLQ", "error", dlqErr, "message_id", msg.ID)
						}
					}
					// Don't ACK transient failures - they'll be reclaimed later
					continue
				}
	...
	for _, p := range pending {
		if p.RetryCount >= MaxDeliveryAttempts {
			if err := c.movePendingToDeadLetter(ctx, p.ID, fmt.Sprintf("max delivery attempts exceeded (%d)", p.RetryCount), p.RetryCount); err != nil {
				c.log.Error("Failed to dead-letter stale message", "error", err, "message_id", p.ID)
			}
			continue
		}
			...
			if err := c.processMessage(ctx, msg, handler); err != nil {
				...
				if isPermanentProcessError(err) {
					if dlqErr := c.moveToDeadLetter(ctx, msg, err.Error(), p.RetryCount); dlqErr != nil {
						c.log.Error("Failed to move permanently invalid claimed message to DLQ", "error", dlqErr, "message_id", msg.ID)
					}
				}
				continue
			}
...
func (c *RedisConsumer) movePendingToDeadLetter(ctx context.Context, messageID, reason string, attempts int64) error {
	messages, err := c.client.XRangeN(ctx, StreamName, messageID, messageID, 1).Result()
	if err != nil {
		return fmt.Errorf("failed to load pending message for DLQ: %w", err)
	}

	if len(messages) == 0 {
		if ackErr := c.client.XAck(ctx, StreamName, ConsumerGroup, messageID).Err(); ackErr != nil {
...
func (c *RedisConsumer) moveToDeadLetter(ctx context.Context, msg redis.XMessage, reason string, attempts int64) error {
	...
	if err := c.client.XDel(ctx, StreamName, msg.ID).Err(); err != nil {
		c.log.Warn("Failed to delete dead-lettered message from source stream", "message_id", msg.ID, "error", err)
	}

	c.log.Warn("Message moved to dead-letter stream", "message_id", msg.ID, "reason", reason, "attempts", attempts)
	return nil
}
