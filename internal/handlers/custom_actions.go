...
func (a *App) ListCustomActions(r *fastglue.Request) error {
	...
	if err := a.requirePermission(r, userID, models.ResourceCustomActions, models.ActionRead); err != nil {
		return nil
	}
...
func (a *App) CreateCustomAction(r *fastglue.Request) error {
	...
	if err := a.requirePermission(r, userID, models.ResourceCustomActions, models.ActionWrite); err != nil {
		return nil
	}
...
func (a *App) DeleteCustomAction(r *fastglue.Request) error {
	...
	if err := a.requirePermission(r, userID, models.ResourceCustomActions, models.ActionDelete); err != nil {
		return nil
	}
...
func (a *App) ExecuteCustomAction(r *fastglue.Request) error {
	...
	if !a.HasPermission(userID, models.ResourceCustomActions, models.ActionWrite, orgID) &&
...
func (a *App) executeWebhookAction(action models.CustomAction, ctxData map[string]interface{}) (*ActionResult, error) {
	...
	for k, v := range config.Headers {
		headers[k] = replaceVariables(v, ctxData)
	}

	// Replace variables in body or use default
	var body string
	if config.Body != "" {
		body = replaceVariables(config.Body, ctxData)
	} else {
		// Default body with all context
		bodyJSON, _ := json.Marshal(ctxData)
...
func (a *App) executeURLAction(action models.CustomAction, ctxData map[string]interface{}) (*ActionResult, error) {
	// Parse config from JSONB (already a map)
	configBytes, err := json.Marshal(action.Config)
	if err != nil {
		return nil, err
	...
	// Store the redirect token (one-time use, short-lived).
...
func (a *App) executeJavaScriptAction(action models.CustomAction, ctxData map[string]interface{}) (*ActionResult, error) {
	configBytes, err := json.Marshal(action.Config)
	if err != nil {
		return nil, err
	}
	...
	if contact, ok := ctxData["contact"]; ok {
		_ = vm.Set("contact", contact)
	}
	if user, ok := ctxData["user"]; ok {
		_ = vm.Set("user", user)
	}
	if org, ok := ctxData["organization"]; ok {
	...
	timeout := resolveJavaScriptTimeout(config.TimeoutMS)
	val, err := runJavaScriptWithTimeout(vm, wrapped, timeout)
	if err != nil {
