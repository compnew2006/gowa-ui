...
async function fetchItems() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        error,
        t("common.failedLoad", { resource: t("resources.apiKeys") }),
      ),
    );
  } finally {
...
async function createAPIKey() {
  ...
  try {
    ...
    await fetchItems();
    toast.success(
      t("common.createdSuccess", { resource: t("resources.APIKey") }),
    );
  } catch (error) {
    toast.error(
      getErrorMessage(
        error,
        t("common.failedCreate", { resource: t("resources.APIKey") }),
      ),
    );
  } finally {
...
async function deleteAPIKey() {
  ...
  try {
    ...
    await fetchItems();
    toast.success(
      t("common.deletedSuccess", { resource: t("resources.APIKey") }),
    );
    closeDeleteDialog();
  } catch (error) {
    toast.error(
      getErrorMessage(
        error,
        t("common.failedDelete", { resource: t("resources.APIKey") }),
      ),
    );
  }
}
...
<template>
  <div class="flex h-full flex-col bg-background text-foreground">
          ...
          <Card>
            <CardHeader>
              <div class="flex items-center justify-between flex-wrap gap-4">
                <div>
                  ...
                  <CardDescription>{{
                    $t("apiKeys.yourApiKeysDesc")
                  }}</CardDescription>
                </div>
                <SearchInput
                  v-model="searchQuery"
                  :placeholder="$t('apiKeys.searchApiKeys') + '...'"
                  class="w-64"
                />
              </div>
            </CardHeader>
            <CardContent>
              <DataTable
                ...
                :empty-title="
                  searchQuery
                    ? $t('apiKeys.noMatchingApiKeys')
                    : $t('apiKeys.noApiKeysYet')
                "
                :empty-description="
                  searchQuery
                    ? $t('apiKeys.noMatchingApiKeysDesc')
                    : $t('apiKeys.noApiKeysYetDesc')
                "
              ...
              >
                <template #cell-name="{ item: key }"
                  ><span class="font-medium">{{ key.name }}</span></template
                >
                <template #cell-key="{ item: key }"
                  ><code class="bg-muted px-2 py-1 rounded text-sm"
                    >whm_{{ key.key_prefix }}...</code
                  ></template
                >
                <template #cell-last_used="{ item: key }">{{
                  formatDateTime(key.last_used_at)
                }}</template>
                <template #cell-expires="{ item: key }">{{
    ...
    <CrudFormDialog
      ...
      :create-title="$t('apiKeys.createTitle')"
      :create-description="$t('apiKeys.createDesc')"
      :create-submit-label="$t('apiKeys.createSubmit')"
      @submit="createAPIKey"
    >
      <div class="space-y-4">
        <div class="space-y-2">
      ...
      <DialogContent>
        ...
        <div class="space-y-4 py-4">
          ...
          <div class="bg-muted p-3 rounded-lg text-sm">
            ...
            <code class="text-xs"
              >curl -H "X-API-Key: {{ newlyCreatedKey?.key }}"
              https://your-api.com/api/contacts</code
            >
          </div>
        </div>
        <DialogFooter
    ...
    <DeleteConfirmDialog
      ...
      :title="$t('apiKeys.deleteApiKey')"
      :item-name="keyToDelete?.name"
