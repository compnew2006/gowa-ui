...
async function saveRule() {
  ...
  if (
    formData.value.response_type !== "transfer" &&
    !formData.value.response_content.trim()
  ) {
  ...
  const validButtons = formData.value.buttons.filter(
  ...
  try {
    ...
    if (editingRule.value) {
      ...
      toast.success(
        t("common.updatedSuccess", { resource: t("resources.KeywordRule") }),
      );
    } else {
      await chatbotService.createKeyword(data);
      toast.success(
        t("common.createdSuccess", { resource: t("resources.KeywordRule") }),
      );
    }
    ...
    toast.error(
      getErrorMessage(
        error,
        t("common.failedSave", { resource: t("resources.keywordRule") }),
      ),
    );
  } finally {
...
async function confirmDeleteRule() {
  ...
  try {
    ...
    toast.success(
      t("common.deletedSuccess", { resource: t("resources.KeywordRule") }),
    );
    closeDeleteDialog();
    await fetchRules();
  } catch (error: any) {
    toast.error(
      getErrorMessage(
        error,
        t("common.failedDelete", { resource: t("resources.keywordRule") }),
      ),
    );
  }
}
...
async function toggleRule(rule: KeywordRule) {
  try {
    ...
    toast.success(
      rule.enabled
        ? t("common.enabledSuccess", { resource: t("resources.KeywordRule") })
        : t("common.disabledSuccess", { resource: t("resources.KeywordRule") }),
    );
  } catch (error: any) {
    toast.error(
      getErrorMessage(
        error,
        t("common.failedToggle", { resource: t("resources.keywordRule") }),
      ),
    );
  }
}
          ...
          <Card>
            ...
            <CardContent>
              <DataTable
                ...
                :empty-title="
                  searchQuery
                    ? $t('keywords.noMatchingRules')
                    : $t('keywords.noRulesYet')
                "
              ...
              >
                ...
                <template #cell-match_type="{ item: rule }">
                  <Badge
                    class="text-xs capitalize bg-blue-500/20 text-blue-400 border-transparent"
                    >{{ rule.match_type }}</Badge
                  >
                </template>
    ...
    <Dialog v-model:open="isDialogOpen">
      <DialogContent class="max-w-md">
        <DialogHeader>
          <DialogTitle
            >{{
              editingRule ? $t("keywords.editRule") : $t("keywords.createRule")
            }}
            {{ $t("keywords.keywordRule") }}</DialogTitle
          >
          <DialogDescription>
        ...
        <div class="space-y-4 py-4">
            ...
            <Select v-model="formData.response_type">
              ...
              <SelectContent>
                <SelectItem value="text">{{
                  $t("keywords.textResponse")
                }}</SelectItem>
                <SelectItem value="transfer">{{
                  $t("keywords.transferToAgent")
                }}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div class="space-y-2">
            <Label for="response">
              {{
                formData.response_type === "transfer"
                  ? $t("keywords.transferMessage")
                  : $t("keywords.responseMessage")
              }}
            </Label>
            ...
            <WhatsAppRichTextEditor
              ...
              :placeholder="
                formData.response_type === 'transfer'
                  ? $t('keywords.transferPlaceholder') + '...'
                  : $t('keywords.responsePlaceholder') + '...'
              "
              :rows="3"
            />
            ...
            <p
              v-if="formData.response_type === 'transfer'"
