...
func createAccountsAuthorizedUser(t *testing.T, app *handlers.App, orgID uuid.UUID) *models.User {
	t.Helper()

	role := testutil.CreateTestRoleWithKeys(t, app.DB, orgID, "accounts-manager", []string{
		"accounts:read",
		"accounts:write",
		"accounts:delete",
	})
...
func TestApp_ListAccounts_OrgIsolation(t *testing.T) {
	...
	user1 := createAccountsAuthorizedUser(t, app, org1.ID)
	user2 := createAccountsAuthorizedUser(t, app, org2.ID)
