...
test.describe("General Tab", () => {
  ...
  test("should show available timezones", async ({ page }) => {
    ...
    // Use exact text match to avoid strict mode violation if multiple options contain "UTC"
    await expect(
      page.locator('[role="option"]').filter({ hasText: /^UTC \(GMT\+0\)$/ }),
    ).toBeVisible();
    await expect(
      page.locator('[role="option"]').filter({ hasText: "Eastern" }),
    ).toBeVisible();
    // Close dropdown
  ...
  test("should show available date formats", async ({ page }) => {
    ...
    await expect(
      page.locator('[role="option"]').filter({ hasText: "YYYY-MM-DD" }),
    ).toBeVisible();
    await expect(
      page.locator('[role="option"]').filter({ hasText: "DD/MM/YYYY" }),
    ).toBeVisible();
    await page.keyboard.press("Escape");
  });
...
test.describe("Settings Tab Navigation", () => {
  ...
  test("should not show close chat rating controls in chat settings", async ({
    page,
  }) => {
