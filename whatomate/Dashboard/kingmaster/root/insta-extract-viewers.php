...
$page_title = "استخراج مشاهدي الاستوري | Kingmaster";
    ...
    <div class="modal-content">
        ...
        <form id="campaignForm">
            ...
            <div class="form-group">
               ...
               <div class="form-group">
                <label class="form-label">
                    <i class="fas fa-tag"></i> معرف المجموعة
                </label>
                <input type="text" class="form-input" id="id-gb-wa" placeholder="أدخل معرف المجموعة" required>
            </div>
      ...
      <table style="width:100%; border-collapse:collapse;">
        <thead>
          <tr style="background:#0f172a;">
            ...
            <th style="padding:10px; border-bottom:1px solid var(--border-color); text-align:right;">الاسم الثاني</th>
          </tr>
        </thead>
...
document.addEventListener('DOMContentLoaded', function() {
    loadAccounts();
    loadIntervals();
    loadCampaigns('Extract Members WA');
    loadContent();
});
...
function closeModal() {
    ...
    editingCampaignId = null;
        ...
        document.getElementById('id-gb-wa').innerHTML = '';
...
document.getElementById('campaignForm').addEventListener('submit', function(e) {
    ...
    if (editingCampaignId) {
        ...
        const data = {
            name: document.getElementById('campaign-name').value,
            accounts: selectedAccounts,
            tools: "Extract Members WA",
            paltform: "WhatsApp",
            id_action: document.getElementById('id-gb-wa').value,
...
function editCampaign(id) {
    ...
    document.getElementById('id-gb-wa').value = campaign.pram1;
...
function saveCampaign(id) {
     ...
     window.location.href = "api/save_data.php?camp_id=" + id+"&tool=Extract-Members-WA";
}
...
function loadContacts() {
  ...
  fetch('api/get_mmbers_wa.php?' + params.toString())
    .then(r=>r.json())
    .then(j=>{
...
function renderContactsTable() {
  ...
  tb.innerHTML = contactsState.data.map((row, idx) => {
    ...
    return `
      <tr>
        <td style="padding:8px; text-align:center;"><input type="checkbox" data-id="${id}" data-phone="${escapeHtml(row.phone||'')}" data-name="${escapeHtml((row.name||row.pushname||'').toString())}" onchange="toggleSelectContact(this)" ${checked}></td>
        <td style="padding:8px;">${escapeHtml(row.name || '')}</td>
        <td style="padding:8px; direction:ltr;">${escapeHtml(row.phone || '')}</td>
        <td style="padding:8px;">${escapeHtml(row.pushname || '')}</td>
      </tr>`;
  }).join('');
