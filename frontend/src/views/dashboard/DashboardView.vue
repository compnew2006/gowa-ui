...
import { useConfigStore, type FeatureFlags } from "@/stores/config";
import {
  MessageSquare,
  Users,
  Bot,
...
const shortcutPermissions: Record<string, string> = {
  chat: "chat",
  campaigns: "campaigns",
  templates: "templates",
  chatbot: "settings.chatbot",
  contacts: "contacts",
  flows: "flows.whatsapp",
  transfers: "transfers",
  agentAnalytics: "analytics.agents",
  metaInsights: "analytics",
...
const shortcutFeatureFlags: Partial<Record<string, keyof FeatureFlags>> = {
  templates: "templates",
  flows: "flows",
  metaInsights: "meta_insights",
  accounts: "business_profile",
};

const isShortcutAccessible = (key: string) => {
  if (!SHORTCUT_REGISTRY.value[key as keyof typeof SHORTCUT_REGISTRY.value]) {
    return false;
  }

  const permission = shortcutPermissions[key];
  if (permission && !authStore.hasPermission(permission, "read")) {
    return false;
  }

  const featureFlag = shortcutFeatureFlags[key];
  if (featureFlag && !configStore.features[featureFlag]) {
    return false;
  }
...
const availableShortcutKeys = computed(() =>
  Object.keys(SHORTCUT_REGISTRY.value).filter((key) =>
    isShortcutAccessible(key),
  ),
);
...
const colorOptions = computed(() => [
  {
    value: "blue",
    label: t("dashboard.colorBlue"),
    bg: "bg-blue-500/20",
    text: "text-blue-400",
  },
  {
    value: "green",
    label: t("dashboard.colorGreen"),
    bg: "bg-emerald-500/20",
    text: "text-emerald-400",
  },
  {
    value: "purple",
    label: t("dashboard.colorPurple"),
    bg: "bg-purple-500/20",
    text: "text-purple-400",
  },
  {
    value: "orange",
    label: t("dashboard.colorOrange"),
    bg: "bg-orange-500/20",
    text: "text-orange-400",
  },
  {
    value: "red",
    label: t("dashboard.colorRed"),
    bg: "bg-red-500/20",
    text: "text-red-400",
  },
  {
    value: "cyan",
    label: t("dashboard.colorCyan"),
    bg: "bg-cyan-500/20",
    text: "text-cyan-400",
  },
]);
...
const getChartComponentData = (widget: DashboardWidget) => {
  ...
  if (
    ...
    groupedSeries &&
    groupedSeries.datasets.length > 0
  ) {
    return {
      ...
      datasets: groupedSeries.datasets.map((ds, i) => ({
        ...
        backgroundColor: chartColors[i % chartColors.length].replace(
          "0.8)",
          "0.1)",
        ),
        fill: false,
      ...
      datasets: [
        {
          ...
          backgroundColor: dataPoints.map(
            (_: any, i: number) => chartColors[i % chartColors.length],
          ),
          borderWidth: 0,
        },
      ],
...
type TableWidgetRow = NonNullable<WidgetData["table_rows"]>[number];

const openTableRowChat = (widget: DashboardWidget, row: TableWidgetRow) => {
  if (widget.data_source !== "messages") return;
...
const openEditWidgetDialog = (widget: DashboardWidget) => {
  ...
  if (widget.display_type === "shortcuts" && widget.config?.shortcuts) {
    selectedShortcuts.value = getVisibleShortcutKeys([
      ...(widget.config.shortcuts as string[]),
    ]);
  } else {
    selectedShortcuts.value = [];
  }
...
const saveWidget = async () => {
  ...
  if (isShortcuts) {
    config = { shortcuts: getVisibleShortcutKeys([...selectedShortcuts.value]) };
  }
  ...
  try {
    if (isEditMode.value && editingWidgetId.value) {
      ...
      toast({
        title: t("common.updatedSuccess", { resource: t("resources.Widget") }),
      });
    } else {
      await widgetsService.create(payload);
      toast({
        title: t("common.createdSuccess", { resource: t("resources.Widget") }),
      });
    }
...
const confirmDeleteWidget = async () => {
  ...
  try {
    ...
    toast({
      title: t("common.deletedSuccess", { resource: t("resources.Widget") }),
    });
  ...
  <div class="flex h-full flex-col bg-background">
    ...
    <header class="border-b border-border bg-background/90 backdrop-blur">
      <div class="flex h-16 items-center px-6">
        <div
          class="mr-3 flex h-8 w-8 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm"
        >
    ...
    <ScrollArea class="flex-1">
      <div class="p-6 space-y-6">
        ...
        <div v-if="isLoading" class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div v-for="i in 4" :key="i" class="widget-surface p-6">
            <div
            ...
            <div class="pt-2">
              <Skeleton class="mb-2 h-8 w-20 bg-muted" />
        ...
        >
          ...
          >
            ...
            <div
              v-if="
                getWidgetById(item.i) && isNumberWidget(getWidgetById(item.i)!)
              "
              class="widget-surface group relative h-full card-depth overflow-hidden p-6"
            >
              <!-- Drag handle indicator -->
              <div
                v-if="isDragMode"
                class="widget-drag-handle absolute top-2 left-2 z-10 cursor-grab text-muted-foreground/40 active:cursor-grabbing"
              >
                ...
                <div class="flex items-center gap-2">
                  ...
                  <div
                    :class="[
                      ...
                      getWidgetColor(getWidgetById(item.i)!.color).bg,
                    ]"
                  >
                    <component
                      :is="getWidgetIcon(getWidgetById(item.i)!.data_source)"
                      :class="[
                        'h-5 w-5',
                        getWidgetColor(getWidgetById(item.i)!.color).text,
                      ]"
                    />
                  </div>
                </div>
              ...
              <div class="pt-2">
                ...
                <div
                  v-if="
                    getWidgetById(item.i)!.show_change && widgetData[item.i]
                  "
            ...
            <div
              v-else-if="
                getWidgetById(item.i) && isChartWidget(getWidgetById(item.i)!)
              "
              class="widget-surface group relative flex h-full flex-col card-depth overflow-hidden p-6"
            >
              <!-- Drag handle indicator -->
              <div
                v-if="isDragMode"
              ...
              <div class="flex flex-row items-center justify-between pb-2">
                <div>
                  ...
                  <p
                    v-if="getWidgetById(item.i)!.description"
                    class="widget-description mt-0.5"
                  >
                    {{ getWidgetById(item.i)!.description }}
                  </p>
                </div>
                <div class="flex items-center gap-2">
                  <div
              ...
              <div class="flex-1 min-h-0 pt-2">
                <template v-if="isWidgetDataLoading">
                  <Skeleton class="h-full w-full bg-muted" />
                </template>
                <template
                  v-else-if="
                    (widgetData[item.i]?.chart_data?.length || 0) > 0 ||
                    (widgetData[item.i]?.data_points?.length || 0) > 0 ||
                    (widgetData[item.i]?.grouped_series?.datasets?.length ||
                      0) > 0
                  "
                >
                  <Line
                    v-if="getWidgetById(item.i)!.chart_type === 'line'"
                    :data="getChartComponentData(getWidgetById(item.i)!)"
                    :options="lineBarChartOptions"
                  />
                  <Bar
                    v-else-if="getWidgetById(item.i)!.chart_type === 'bar'"
                    :data="getChartComponentData(getWidgetById(item.i)!)"
                    :options="lineBarChartOptions"
                  />
                  <Pie
                    v-else-if="getWidgetById(item.i)!.chart_type === 'pie'"
                    :data="getChartComponentData(getWidgetById(item.i)!)"
            ...
            <div
              v-else-if="
                getWidgetById(item.i) && isTableWidget(getWidgetById(item.i)!)
              "
              class="widget-surface group relative flex h-full flex-col card-depth overflow-hidden"
            >
              <!-- Drag handle -->
              <div
                v-if="isDragMode"
              ...
              <div class="flex-1 min-h-0 overflow-auto px-6 pb-6">
                ...
                <template
                  v-else-if="
                    getWidgetById(item.i)!.group_by_field &&
                    widgetData[item.i]?.data_points?.length
                  "
                >
                  <table class="w-full">
                    <thead>
                      <tr class="border-b border-border/80">
                        <th
                          class="py-2 text-left text-xs font-medium uppercase tracking-wide text-muted-foreground"
                        >
                          {{ getWidgetById(item.i)!.group_by_field }}
                        </th>
                        <th
                          class="py-2 text-right text-xs font-medium uppercase tracking-wide text-muted-foreground"
                        >
                          {{ $t("dashboard.count") }}
                        </th>
                      </tr>
                    ...
                    <tbody>
                      <tr
                        v-for="dp in widgetData[item.i]?.data_points"
                        :key="dp.label"
                        class="border-b border-border/50"
                      >
                        <td class="py-2 text-sm text-foreground/80">
                          {{ dp.label }}
                        </td>
                        <td
                          class="py-2 text-right text-sm font-medium text-foreground"
                        >
                          {{ dp.value }}
                        </td>
                      </tr>
                ...
                <template v-else-if="widgetData[item.i]?.table_rows?.length">
                  <div class="space-y-3">
                    <div
                      ...
                      :key="row.id"
                      :class="[
                        'flex items-start gap-3 rounded-[calc(var(--radius)-0.2rem)] border border-border/50 bg-background/60 p-3 transition-colors',
                        getWidgetById(item.i)!.data_source === 'messages'
                          ? 'cursor-pointer hover:bg-accent/70'
                          : 'hover:bg-accent/70',
                      ]"
                      @click="openTableRowChat(getWidgetById(item.i)!, row)"
                    >
                      <div
                        :class="[
            ...
            <div
              v-else-if="
                getWidgetById(item.i) &&
                isShortcutsWidget(getWidgetById(item.i)!)
              "
              class="widget-surface group relative flex h-full flex-col card-depth overflow-hidden"
            >
              <!-- Drag handle -->
              ...
              <div class="flex-1 min-h-0 overflow-y-auto px-6 pb-6">
                <div
                  :class="[
                    'grid gap-3 pt-1',
                    item.w >= 8 ? 'grid-cols-3' : 'grid-cols-2',
                  ]"
                >
                  <template
    ...
    <Dialog v-model:open="isWidgetDialogOpen">
      <DialogContent class="sm:max-w-[500px]">
        ...
        <div class="space-y-4 py-4">
          ...
          <div v-if="widgetForm.display_type === 'shortcuts'" class="space-y-2">
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto pr-1">
              <label
                v-for="key in availableShortcutKeys"
                :key="key"
                class="flex cursor-pointer items-center gap-3 rounded-[calc(var(--radius)-0.2rem)] border border-border/50 bg-background/50 p-2 transition-colors hover:bg-accent/70"
              >
                <input
                  type="checkbox"
                  :value="key"
                  v-model="selectedShortcuts"
                  class="rounded border-border bg-input text-primary focus:ring-primary"
                />
                <div class="flex items-center gap-2">
                  <div
                    :class="[
          ...
          <div v-if="widgetForm.display_type !== 'shortcuts'" class="space-y-2">
            ...
            >
              ...
              <Button
                ...
                class="shrink-0 text-muted-foreground/70 hover:bg-destructive/10 hover:text-destructive"
              >
                <X class="h-4 w-4" />
              </Button>
            </div>
              ...
              <SelectContent>
                ...
                >
                  <span class="flex items-center gap-2">
                    <span
                      :class="['inline-block w-3 h-3 rounded-full', color.bg]"
                    ></span>
                    {{ color.label }}
                  </span>
                </SelectItem>
        ...
        <DialogFooter>
          <Button variant="outline" @click="isWidgetDialogOpen = false">
            {{ $t("common.cancel") }}
          </Button>
