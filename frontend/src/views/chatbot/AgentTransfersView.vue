...
function canAgentHandleTransfer(
  ...
  const instanceId =
    typeof transfer?.instance_id === "string"
      ? transfer.instance_id.trim()
      : "";
  return canAccessInstance(agent.settings, instanceId);
}

const assignableAgents = computed(() => {
  const transfer = transferToAssign.value;
  return agents.value.filter((agent) =>
    canAgentHandleTransfer(agent, transfer),
  );
});
...
async function resumeTransfer(transfer: AgentTransfer) {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(error, t("agentTransfers.failedResumeTransfer")),
    );
  } finally {
      ...
      <template v-if="!isAdminOrManager" #actions>
        <div class="flex items-center gap-4">
          ...
          <Button
            ...
            :disabled="isPicking || transfersStore.queueCount === 0"
          >
    ...
    <ScrollArea class="flex-1">
      <div class="p-6 space-y-6">
        ...
        <div v-if="isLoading" class="space-y-4">
          <Skeleton
            class="h-12 w-full bg-white/[0.08] light:bg-gray-200 rounded-xl"
          />
          <Skeleton
            class="h-64 w-full bg-white/[0.08] light:bg-gray-200 rounded-xl"
          />
        </div>
          ...
          >
            ...
            <div class="px-6 pb-6">
              <div
                ...
                class="text-center py-8 text-white/50 light:text-gray-500"
              >
                <div
                  class="h-16 w-16 rounded-xl bg-red-500/20 flex items-center justify-center mx-auto mb-4"
                >
                  <UserX class="h-8 w-8 text-red-400" />
                </div>
                <p>{{ $t("agentTransfers.noActiveTransfers") }}</p>
                <p class="text-sm mt-2">
                  {{ $t("agentTransfers.clickPickNext") }}
                </p>
              </div>
                  ...
                  <TableRow v-for="transfer in myTransfers" :key="transfer.id">
                    ...
                    <TableCell class="text-right space-x-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>{{
                          $t("agentTransfers.resumeChatbot")
                        }}</TooltipContent>
                      </Tooltip>
                    </TableCell>
                  </TableRow>
            ...
            <TabsContent value="history">
              <Card>
                ...
                <CardContent>
                  ...
                  >
                    <Loader2
                      class="h-8 w-8 mx-auto mb-4 animate-spin text-muted-foreground"
                    />
      ...
      <DialogContent>
        ...
        <div class="space-y-4 py-4">
          <div
            ...
            class="text-sm border rounded-lg p-3 bg-muted/50"
          >
            <p>
