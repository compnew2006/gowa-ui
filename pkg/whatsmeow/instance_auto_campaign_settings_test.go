...
func TestNormalizeAutoCampaignSettings_Defaults(t *testing.T) {
	...
	assert.Equal(t, 0, normalized.MinDelayMinutes)
	assert.Equal(t, 0, normalized.MaxDelayMinutes)
...
func TestEnsureInstanceSettingsDefaults_InjectsAutoCampaign(t *testing.T) {
	...
	assert.Equal(t, 0, autoCampaign.MinDelayMinutes)
	assert.Equal(t, 0, autoCampaign.MaxDelayMinutes)
