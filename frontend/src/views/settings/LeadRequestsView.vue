...
const statusOptions: Array<"all" | LeadRequestStatus> = [
  "all",
  "new",
  "contacted",
  "qualified",
...
const columns = computed<Column<LeadRequest>[]>(() => [
  {
    key: "submitter",
    label: t("leadRequests.columns.submitter"),
    sortable: true,
  },
  ...
  {
    key: "created",
    label: t("leadRequests.columns.created"),
    sortable: true,
    sortKey: "created_at",
  },
...
async function fetchLeadRequests() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        error,
        t("common.failedLoad", { resource: t("resources.leadRequests") }),
      ),
    );
  } finally {
...
async function saveStatus() {
  ...
  try {
    const response = await leadRequestsService.updateStatus(
      selectedLead.value.id,
      selectedStatus.value,
    );
    const updatedLead = ((response.data as any).data ||
      response.data) as LeadRequest;
    selectedLead.value = updatedLead;

    const index = leadRequests.value.findIndex(
    ...
    toast.success(
      t("common.updatedSuccess", { resource: t("resources.leadRequest") }),
    );
  } catch (error) {
    toast.error(
      getErrorMessage(
        error,
        t("common.failedUpdate", { resource: t("resources.leadRequest") }),
      ),
    );
  } finally {
  ...
  <div class="flex h-full flex-col bg-background text-foreground">
            ...
            <CardHeader>
              <div class="flex items-center justify-between flex-wrap gap-4">
                <div>
                  ...
                  <CardDescription>{{
                    $t("leadRequests.inboxDescription")
                  }}</CardDescription>
                </div>
                <div class="flex items-center gap-3 flex-wrap">
                  ...
                  <Select v-model="statusFilter">
                    <SelectTrigger class="w-[180px]">
                      <SelectValue
                        :placeholder="$t('leadRequests.filterStatus')"
                      />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem
              ...
              <DataTable
                ...
                :empty-title="
                  searchQuery || statusFilter !== 'all'
                    ? $t('leadRequests.noMatching')
                    : $t('leadRequests.emptyTitle')
                "
                :empty-description="
                  searchQuery || statusFilter !== 'all'
                    ? $t('leadRequests.noMatchingDescription')
                    : $t('leadRequests.emptyDescription')
                "
                ...
                <template #cell-submitter="{ item: lead }">
                  <div>
                    ...
                    <div class="text-xs text-muted-foreground">
                      {{ lead.country || $t("leadRequests.noCountry") }}
                    </div>
                  </div>
                </template>
    ...
    <Dialog v-model:open="isDialogOpen">
      <DialogContent class="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{{
            selectedLead?.full_name || $t("leadRequests.detailsTitle")
          }}</DialogTitle>
          <DialogDescription>
            {{
              selectedLead?.company_name ||
              $t("leadRequests.detailsDescription")
            }}
          </DialogDescription>
        </DialogHeader>
        ...
        <div v-if="selectedLead" class="space-y-6 py-2">
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="space-y-1">
              ...
              <div class="text-sm text-muted-foreground">
                {{ selectedLead.phone_whatsapp }}
              </div>
            </div>
            <div class="space-y-1">
              <Label>{{ $t("leadRequests.fields.country") }}</Label>
              <div class="text-sm text-muted-foreground">
                {{ selectedLead.country || $t("leadRequests.noCountry") }}
              </div>
            </div>
            <div class="space-y-1">
              <Label>{{ $t("leadRequests.fields.requestedPlan") }}</Label>
              <div class="text-sm text-muted-foreground">
                {{ planLabel(selectedLead.requested_plan) }}
              </div>
            </div>
            <div class="space-y-1">
          ...
          <div class="space-y-2">
            ...
            <div
              class="rounded-lg border bg-muted/30 px-4 py-3 text-sm text-muted-foreground whitespace-pre-wrap min-h-[120px]"
            >
        ...
        <DialogFooter>
          ...
          <Button
            ...
            :disabled="
              isUpdatingStatus ||
              !selectedLead ||
              selectedLead.status === selectedStatus
            "
          >
            <Loader2
