...
func TestLoad_Defaults(t *testing.T) {
	...
	assert.Equal(t, 4096, cfg.Whatsmeow.EventBufferSize)
	require.NotNil(t, cfg.Whatsmeow.EventDispatchEnabled)
	assert.True(t, *cfg.Whatsmeow.EventDispatchEnabled)
...
func TestLoad_EnvironmentVariables(t *testing.T) {
	...
	t.Setenv("WHATOMATE_WHATSMEOW_EVENT_BUFFER_SIZE", "128")
	t.Setenv("WHATOMATE_WHATSMEOW_EVENT_DISPATCH_ENABLED", "false")
	...
	assert.Equal(t, 128, cfg.Whatsmeow.EventBufferSize)
	require.NotNil(t, cfg.Whatsmeow.EventDispatchEnabled)
	assert.False(t, *cfg.Whatsmeow.EventDispatchEnabled)
