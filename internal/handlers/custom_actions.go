...
func (a *App) executeWebhookAction(action models.CustomAction, ctxData map[string]any) (*ActionResult, error) {
...
func (a *App) executeURLAction(action models.CustomAction, ctxData map[string]any) (*ActionResult, error) {
	// Parse config from JSONB (already a map)
	configBytes, err := json.Marshal(action.Config)
	if err != nil {
		return nil, err
...
func (a *App) executeJavaScriptAction(action models.CustomAction, ctxData map[string]any) (*ActionResult, error) {
	configBytes, err := json.Marshal(action.Config)
	if err != nil {
		return nil, err
	}
