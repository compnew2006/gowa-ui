...
func createWebhooksAuthorizedUser(t *testing.T, app *handlers.App, orgID uuid.UUID) *models.User {
	t.Helper()

	role := testutil.CreateTestRoleWithKeys(t, app.DB, orgID, "webhooks-manager", []string{
		"webhooks:read",
		"webhooks:write",
		"webhooks:delete",
	})
...
func TestApp_ListWebhooks_OrgIsolation(t *testing.T) {
	...
	user1 := createWebhooksAuthorizedUser(t, app, org1.ID)
	user2 := createWebhooksAuthorizedUser(t, app, org2.ID)
