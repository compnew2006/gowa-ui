...
const {
  isSubmitting,
  isDialogOpen,
  editingItem: editingContext,
  deleteDialogOpen,
  itemToDelete: contextToDelete,
  formData,
  openCreateDialog,
...
function openEditDialog(context: AIContext) {
  ...
  baseOpenEditDialog(context, (c) => ({
    ...
    api_headers: apiConfig.headers
      ? JSON.stringify(apiConfig.headers, null, 2)
      : "",
...
async function saveContext() {
  ...
  try {
    ...
    const data: any = {
      ...
      trigger_keywords: formData.value.trigger_keywords
        .split(",")
        .map((k) => k.trim())
        .filter(Boolean),
      static_content: formData.value.static_content,
      api_config:
        formData.value.context_type === "api"
          ? {
    ...
    if (editingContext.value) {
      ...
      toast.success(
        t("common.updatedSuccess", { resource: t("resources.AIContext") }),
      );
    } else {
      await chatbotService.createAIContext(data);
      toast.success(
        t("common.createdSuccess", { resource: t("resources.AIContext") }),
      );
    }
    ...
    toast.error(
      getErrorMessage(
        error,
        t("common.failedSave", { resource: t("resources.AIContext") }),
      ),
    );
  } finally {
    isSubmitting.value = false;
...
async function confirmDeleteContext() {
  ...
  try {
    ...
    toast.success(
      t("common.deletedSuccess", { resource: t("resources.AIContext") }),
    );
    closeDeleteDialog();
    await fetchContexts();
  } catch (error: any) {
    toast.error(
      getErrorMessage(
        error,
        t("common.failedDelete", { resource: t("resources.AIContext") }),
      ),
    );
  }
}
...
async function toggleContext(context: AIContext) {
  try {
    await chatbotService.updateAIContext(context.id, {
      enabled: !context.enabled,
    });
    context.enabled = !context.enabled;
    toast.success(
      context.enabled
        ? t("common.enabledSuccess", { resource: t("resources.AIContext") })
        : t("common.disabledSuccess", { resource: t("resources.AIContext") }),
    );
  } catch (error: any) {
    toast.error(
      getErrorMessage(
        error,
        t("common.failedToggle", { resource: t("resources.AIContext") }),
      ),
    );
  }
}
          ...
          <Card>
            <CardHeader>
              <div class="flex items-center justify-between flex-wrap gap-4">
                <div>
                  ...
                  <CardDescription>{{
                    $t("aiContexts.yourContextsDesc")
                  }}</CardDescription>
                </div>
                <SearchInput
                  v-model="searchQuery"
                  :placeholder="$t('aiContexts.searchContexts') + '...'"
                  class="w-64"
                />
              </div>
            </CardHeader>
            <CardContent>
              <DataTable
                ...
                :empty-title="
                  searchQuery
                    ? $t('aiContexts.noMatchingContexts')
                    : $t('aiContexts.noContextsYet')
                "
                :empty-description="
                  searchQuery
                    ? $t('aiContexts.noMatchingContextsDesc')
                    : $t('aiContexts.noContextsYetDesc')
                "
              ...
              >
                ...
                <template #cell-trigger_keywords="{ item: context }">
                  <div class="flex flex-wrap gap-1">
                    <Badge
                      v-for="kw in context.trigger_keywords?.slice(0, 2)"
                      :key="kw"
                      variant="secondary"
                      class="text-xs"
                    >
                    ...
                    <Badge
                      v-if="context.trigger_keywords?.length > 2"
                      variant="outline"
                      class="text-xs"
                    >
                    ...
                    <span
                      v-if="!context.trigger_keywords?.length"
                      class="text-muted-foreground text-sm"
                      >{{ $t("aiContexts.always") }}</span
                    >
                  </div>
                </template>
                ...
                <template #cell-status="{ item: context }">
                  <div class="flex items-center gap-2">
                    <Switch
                      ...
                      @update:checked="toggleContext(context)"
                    />
                    <span class="text-sm text-muted-foreground">{{
                      context.enabled
                        ? $t("aiContexts.active")
                        : $t("aiContexts.inactive")
                    }}</span>
                  </div>
                </template>
                <template #cell-actions="{ item: context }">
                  <div class="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      @click="openDeleteDialog(context)"
                    >
      ...
      <DialogContent class="max-w-2xl">
        <DialogHeader>
          <DialogTitle
            >{{
              editingContext
                ? $t("aiContexts.editContext")
                : $t("aiContexts.createContext")
            }}
            {{ $t("aiContexts.aiContext") }}</DialogTitle
          >
          <DialogDescription>
        ...
        <div class="grid gap-4 py-4 max-h-[60vh] overflow-y-auto">
          ...
          <div class="space-y-2">
            <Label for="trigger_keywords">{{
              $t("aiContexts.triggerKeywords")
            }}</Label>
            <Input
              id="trigger_keywords"
          ...
          <div class="space-y-2">
            <Label for="static_content">{{
              $t("aiContexts.contentPrompt")
            }}</Label>
            <Textarea
              id="static_content"
          ...
          <div
            v-if="formData.context_type === 'api'"
            class="space-y-4 border-t pt-4"
          >
            <p class="text-sm font-medium">
              {{ $t("aiContexts.apiConfiguration") }}
            </p>
            <p class="text-xs text-muted-foreground">
            ...
            <div class="space-y-2">
              <Label for="api_headers">{{
                $t("aiContexts.headersOptional")
              }}</Label>
              <Textarea
                id="api_headers"
            ...
            <div class="space-y-2">
              <Label for="api_response_path">{{
                $t("aiContexts.responsePath")
              }}</Label>
              <Input
