...
func (a *App) willChatbotHandle(account *models.WhatsAppAccount, contact *models.Contact) bool {
	// The chatbot is a 1:1 auto-responder; it must never reply inside group
	// chats where multiple participants would see automated responses.
	if contact.Metadata != nil && contact.Metadata["is_group_chat"] == true {
		return false
	}
