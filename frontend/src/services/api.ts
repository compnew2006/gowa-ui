...
export const contactsService = {
  list: (params?: { search?: string; page?: number; limit?: number; tags?: string; account?: string; exclude_groups?: boolean; sort?: string; sort_dir?: 'asc' | 'desc' }) =>
...
export const messagesService = {
  list: (contactId: string, params?: { page?: number; limit?: number; before_id?: string; account?: string }) =>
    api.get(`/contacts/${contactId}/messages`, { params, validateStatus: (s) => s < 500 }),
  send: (
    contactId: string,
    data: {
      type: string
