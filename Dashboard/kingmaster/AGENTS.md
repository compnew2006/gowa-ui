...
4. serena          → read_memory("summary")    ← if exists
5. Check if skills-map.md exists (e.g., search via `serena → search_for_pattern(substring_pattern = "skills-map\\.md", relative_path = "")` or check file)
   ...
   → if not found: run the generation script (see SKILLS MAP SYSTEM below), then read
   → extract skill routing table for use in Phase 1
...
| New file creation | `bash: touch <path>` then `serena` → `replace_content` |

**After each edit — stage changes first, then verify immediately:**
```bash
git add <edited-file>
```
Then run:
...
```bash
npx playwright test --project=chromium   # minimum smoke
