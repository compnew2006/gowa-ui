...
type incomingMessagePayload struct {
	...
	ButtonID         string
	MediaInfo        *MediaInfo
	FlowResponseData map[string]interface{}
	ReplyToWAMID     string
}
...
func (a *App) storeFlowResponseData(session *models.ChatbotSession, flowResponseData map[string]interface{}) {
...
func evaluateExpression(expr string, data map[string]interface{}) bool {
	...
	// Fast path for plain conditions without logical operators or parentheses.
...
func parseLogicalOperatorAt(expr string, idx int) (expressionTokenType, bool) {
	// Match logical operators case-insensitively with token boundaries.
	if idx+3 <= len(expr) &&
...
func evaluateExpressionRPN(tokens []expressionToken, data map[string]interface{}) bool {
...
func parseNumber(s string) (float64, error) {
	...
	// Preserve the previous tolerant parsing behavior for partially numeric strings.
	var n float64
