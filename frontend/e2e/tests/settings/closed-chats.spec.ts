...
async function mockInstancesAPI(
  page: Page,
  instances: MockInstance[] = MOCK_INSTANCES,
) {
...
function buildClosedChats(total: number): MockClosedChat[] {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    chats.push({
      ...
      instance_id: i % 2 === 0 ? "instance-alpha" : "instance-bravo",
      status: "closed",
      closed_at: closedAt,
      updated_at: closedAt,
...
async function mockClosedChatsAPI(
  ...
  const failSearches = new Set(
    (options?.failSearches || []).map((value) => value.trim().toLowerCase()),
  );
  ...
  await page.route("**/api/chats**", async (route) => {
    ...
    const closedBy = (url.searchParams.get("closed_by") || "")
      .trim()
      .toLowerCase();
    ...
    if (instanceId) {
      filtered = filtered.filter(
        (chat) => chat.instance_id.toLowerCase() === instanceId,
      );
    }

    if (closedFrom) {
      filtered = filtered.filter(
        (chat) => chat.closed_at.slice(0, 10) >= closedFrom,
      );
    }

    if (closedTo) {
      filtered = filtered.filter(
        (chat) => chat.closed_at.slice(0, 10) <= closedTo,
      );
    }
...
test.describe("Closed Chats Settings", () => {
  test("supports page navigation, page size options, and scrollable table", async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId("closed-chats-page-label")).toHaveText(
      "Page 1 of 6",
    );
    ...
    const scrollInfo = await page
      .getByTestId("closed-chats-scroll")
      .evaluate((element) => ({
  ...
  test("filters closed chats by agent and closed date range", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    await mockUsersAPI(page);
  ...
  test("filters closed chats by phone search and clears stale rows on request failure", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    await mockUsersAPI(page);
  ...
  test("filters closed chats by instance using its own page filter", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    await mockUsersAPI(page);
    await mockInstancesAPI(page);
    await mockClosedChatsAPI(page, buildClosedChats(40));

    await page.goto("/settings/closed-chats");

    await page.getByTestId("closed-chats-instance-filter").click();
    await page.getByRole("option", { name: "Instance Alpha" }).click();
    await page.waitForTimeout(450);

    await expect(page.locator("tbody tr")).toHaveCount(20);
    await expect(page.locator("tbody")).toContainText("Contact 2");
    await expect(page.getByText(/^Contact 1$/)).toHaveCount(0);
  });
});
