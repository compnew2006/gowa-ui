...
- Updated the status count badge in [`frontend/src/components/chat/status/StatusStoriesBar.vue`](/Users/noiemany/Downloads/whatomate_GOWA/whatomate/frontend/src/components/chat/status/StatusStoriesBar.vue) to a fixed circular badge and capped the displayed value at `9+` so it keeps its shape.
...
- Passed via Chrome DevTools MCP with mocked API boot:
  ...
  - status badge measured `21 x 21` in the live DOM after the final badge update
